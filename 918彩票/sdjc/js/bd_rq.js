...
var SDJC ={
		...
		dataList:function(flag){//无back向上，有back向下
			$("#bd_rq_list").html('<div style="padding-top:50px;height:200px"><em class="rotate_load" style="margin:auto"></em></div>')
			$.ajax({
				...
				success: function(xml){
					...
					if(row.length){
						row.each(function(){
							...
							var money = SDJC.money_dot(followmoney)
							...
							html+='<p class="bd_p4">跟买总额'+money+'元</p>'
							html+='</div>'
							html+='</li>'
						})
						$("#bd_rq_list").html(html)
					}else{
						$("#bd_rq_list").html('<div class="empty_user"><p>暂无内容</p></div>')
					}
				},
			})
		},
		bindEvent:function(){
			...
			$('#bd_rq_list').delegate('li','click',function(){
				var realuid = $(this).attr('realuid');
				if(agentFrom){
