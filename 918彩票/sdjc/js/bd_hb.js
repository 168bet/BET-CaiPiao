...
var SDJC ={
		...
		dataList:function(index,tag){//无back向上，有back向下
			...
			$("#bd_hb_list").html('<div style="padding-top:50px;height:200px"><em class="rotate_load" style="margin:auto"></em></div>')
			$.ajax({
				...
				success: function(xml){
					...
					if(row.length){
						row.each(function(){
							...
							var l = returnrate.indexOf("%");
							
							var re1 = returnrate.substring(0,returnrate.length-1)
							//var re2 = returnrate.substring(l)
							...
							html+='<p class="bd_p2">回报率：<span class="number"><i>'+re1+'</i>%</span></p>'
							html+='</div>'
							html+='</li>'
						})
						$("#bd_hb_list").html(html)
					}else{
						$("#bd_hb_list").html('<div class="empty_user"><p>暂无内容</p></div>')
					}
				},
			})
		},
		bindEvent:function(){
			...
			$('#bd_hb_list').delegate('li','click',function(){
				var realuid = $(this).attr('realuid');
				if(agentFrom){
