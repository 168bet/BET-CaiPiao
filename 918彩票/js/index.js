...
var white_grade=localStorage.getItem("grade")||"0";
var czss=0;
...
//, 't2015.9188.com ':'3021'
CP.Data = {
		source : {'m.cp.eastday.com':'3020','1ydb.5.9188.com ':'3021','hz.5.9188.com ':'3022','zccp.eastday.com':'3036','ttcp.eastday.com':'3037'}[document.domain]||'3002',//投注source值
		pid : '',//首页双色球大乐透的当前期号
		apps:'',//动态接口
		zlk:'',//资料库
		news:'',//新闻
		data:'',//xml
		white_list_grade:0,//白名单等级
		grade: function(fnc,fnc1){
			$.ajax({
				url:CP.Data.apps+'/trade/getyourgrade.go?mtype=4&appversion=',
				dataType:'xml',
				cache:true,
				success: function(xml) {
					...
					if(code=="0"){//查询成功
						...
						CP.Data.white_list_grade = grade;
...
CP.Util = {
	...
	joinString : function (gid, opt){
		var g = {
				...
				'10' : {'0':'1:4','1':'3:1','2':'7:1','3':'4:1','4':'8:1','5':'2:1','6':'6:1','7':'5:1'},
				'20' : {'0':'11:1','1':'1:1','2':'2:1','3':'3:1','4':'5:1','5':'12:1','6':'10:3','7':'15:3','8':'16:1','9':'10:4'},
				'03' : {'0':'1:1','1':'2:1','2':'1:4','3':'2:3','4':'3:1'},
				'04' : {'0':'6:1','1':'5:1','2':'4:1','3':'3:1','4':'1:1','5':'12:1','6':'7:3','7':'8:3','8':'9:3','9':'7:4','10':'9:1'}
		};
		var opt_ = opt.split(';');
		opt_ = $.map(opt_, function (item) {
			...
			}else if(gid == '54' || gid == '56' || gid == '59' || gid == '55'){//11选5 & 十一运夺金
				gid = '54';
				var Q = item.split('_');
				return Q[1] += (':'+g[gid][Q[0]]);
			}else if(gid == '06'){//安徽快3 & 新快三
				var Q = item.split('_');
				var Q1 = '';
				if(Q[0] == '1'){//三同号单选
					var Q2 = Q[1].split(',');
	 ...
	 * @param {Str} oc 可空
	 */
	showcode : function (lotid, ccodes){//参数：彩种id，投注内容
		var html="";
		...
		for ( var i = 0, l = codes.length; i < l; i++) {
			if(lotid==90 ||lotid==91 ||lotid==92 ||lotid==93 ||lotid==72 || lotid==85 ||lotid==86 ||lotid==87 ||lotid==88 ||lotid==89){
				...
				if (lotid=="01" || lotid=="50"){//双色球，大乐透
					html += CP.Util.matchopencode(lotid, pm, cm, tmpCode[0]);//专为双色球，大乐透服务的方法
				...
				}else if (lotid=="05" || lotid=="06" || lotid=="08" || lotid=="09" || lotid=="10"){//吉林快三  ，安徽快三  ，福彩快三(内蒙)，江苏快三，新k
...
CP.User = {
			...
			$.ajax({
				...
				success : function(xml) {
					...
					if (code == "0") {
						...
						}else if(mobbind !=1) {//未绑定手机号
			        		$('#jc_close').click()
							
							D.confirm_new("为保证您的账户安全，请先绑定手机号码",function(){
			        			window.location.href="/alone/phone.html";
			        		},"确定")
						}else{
							func();
						}
		...
		//检测是否设置过密码
		checksetpassword: function(username,func){
			if($('#content_home').children().length > 0){
				tType = $('#content_home').children().eq(0).attr('id').replace('buy_','')
			}
			...
			$.ajax({
				...
				success:function(xml){
					...
					if(c == '0'){
						...
						if(CP.Data.source == '3021'){//一元得宝
							CP.User.checkId(function(){
								fn(t);
							})
						}else{
							//判断用户是否设置过密码
							CP.User.checksetpassword(nickid,function(){
								fn(t);
							})
						}
					}else{
...
CP.Popup = {
		...
		buybox : function(options){
			...
			var main = function(o){
				...
				if(o.ipacketmoney == '0' || o.payPara.orderType == 'zh' || document.domain == "m.cp.eastday.com" || document.domain == "ttcp.eastday.com"||document.domain == "zccp.eastday.com"){//木有红包|东方网的时候隐藏红包按钮
					if(CP.Data.source=='3021'){//一元得宝
						$('#buy_reveal .popuseRed span').html('宝币兑彩金');
					}else{
						...
						success:function(xml){
							...
							if(Resp.attr('code') == '0'){
								...
								if(isBindIdCard != '1'){//1身份证已绑定
									$('#bdidcard, #mask').show()
									setTimeout(function(){
										$('#bdidcard').addClass('modal-in')
									},10)
									$('#bdidcard span').html('');
									$('#bdidcard').on('click', 'a', function(e){
										$('#bdidcard,#mask').hide();
										location.href='/alone/idcard.html?rnd=123';
									})
									$('#mask').off().on('click', function(e){
										$('#bdidcard,#mask').hide();
										$('#bdidcard').removeClass('modal-in');
									})
									return;
								}
								if(isBindMobile != '1'){//1手机号已绑定
									$('#bdphone, #mask').show()
									setTimeout(function(){
										$('#bdphone').addClass('modal-in')
									},10)
									$('#bdphone span').html('');
									$('#bdphone').on('click', 'a, span', function(e){
										$('#bdphone, #mask').hide()
										location.href='/alone/phone.html'
									})
									$('#mask').off().on('click', function(e){
										$('#bdphone,#mask').hide();
										$('#bdphone').removeClass('modal-in');
									})
									return;
								}
							}
		...
		if (opt_.hid) {//合买跟单
			...
			if (opt_.orderType == 'dg') {//代购
				data = {
						...
						appversion:(opt.gid==10?'4.44':"4.0"),
						imoneyrange: opt.imoneyrange || '',
						iminrange: opt.iminrange || ''
				};
...
var D = {
		...
		confirm_new:function(msg, fn, tag){
			$('#descript').html(msg);
			tag && $('#qr').html(tag) || $('#qr').html('确定');
			$('#iosDialog1').show();
...
//		setTimeout(function () {//加上之后别的页面初始化的时候就改变不了页头的内容了
			if (!obj) {
				...
				$dom.title.off().html(confTemp.title);//设置每个块的头部title
...
CP.Home = function () {
	...
	var b = {
			game:function(){
				if(CP.Data.source=='3021'){//触屏=》一元夺宝
					
					var data = {
							uid : CP.Util.getPara('uid'),
							mobileNo : CP.Util.getPara('mobile'),
							ts : CP.Util.getPara('ts'),
							sign : CP.Util.getPara('sign'),
							target_url : CP.Util.getPara('dn'),
							source: CP.Data.source
					}
					localStorage.setItem('target_url',data.target_url)
					TopAnch.init({
						title:'预约彩票',
						prevShow:true,
						prevFun:function(){//左侧按钮绑定事件
					...
					function body_(){
						...
						$('.mask2').unbind( "click" , body)
					}
					var yydbsso = function(){
						$.ajax({
							url:'/user/yydbsso.go',
							data:data,
							type:'post',
							success: function(xml_){
								var R = $(xml_).find('Resp');
								var code = R.attr('code');
								if(code == '0'){
									localStorage.setItem('db_uid',data.uid);
									var isBindIdCard = R.find('row').attr('isBindIdCard');
									if(isBindIdCard != '1'){//是否绑定身份证
										$('.mask2').show()
										$('.mask2').on('click', body_)
										 
										$('#bdidcard').on('click', 'a, span', function(e){
											$('#bdidcard,#mask').hide()
											$('#bdidcard').removeClass('modal-in')
											e.target.tagName=='A'&&(location.href='/alone/idcard.html')
										})
									}
								}else{
									alert(R.attr('desc'))
					...
					$.ajax({
						...
						success:function(xml){
							var Resp = $(xml).find('Resp')
							if(Resp.attr('code') == '0'){//用户是否登录
								if(data.uid !='' && data.uid != localStorage.getItem('db_uid')){
									yydbsso();
								}
								return;
							}
							if(/用户未登录/.test(Resp.attr('desc'))){
								if(!data.uid){
			...
			init: function(){
				...
				b.game();
				a.setJx();//双色球机选
				$('#handy').on('click','div:eq(0) em',function(){//机选一注
					a.setJx();
				});
				$('#ball').on('click','a',function(){//购买
					...
					CP.User.info(function(options){//投注时判断用户是否登录，如登录可以获取到用户信息
						if (options) {
			...
			login_bind : function () {
				...
				$('#loginBtn').bind('click', function(){b.MiniLogin();});//点击登录按钮去登录
				$('#eye').click(function(){
				...
				$("#username").keyup(function(){
			     ...
			     $("#username").bind(start_ev,function(event){
			         $("#usernamelist").toggle();
			         event.stopPropagation();
			     });
				$(document).bind(start_ev,function(event){
			    ...
			    //忘记密码
			     ...
			     $("#usernamelist").delegate('li>a' ,start_ev,function(){
			         var value=$(this).html();
			         $("#username").val(value);
			     })
			     $("#usernamelist").delegate('li>i' ,start_ev,function(){
	...
	***/
	...
	var init_yy=function(){//白名单用户对彩种描述调用接口
		CP.Data.grade(function(){
			//如果是白名单用户
			$.ajax({
				url:'/trade/start.go?source=3002&mtype=4&come=3002&qtype=4',
				type:'POST',
				dateType:'xml',
				success : function(xml){
					var R = $(xml).find('Resp');
					var lots = R.find('lots');
					var lot = lots.find("lot");
					
					lot.each(function(){
						var index = $(this).attr('index');//序号=》 只有当lot.level=1时才生效
						var gid = $(this).attr('gid');//彩种id
						var jiajiang = $(this).attr('jiajiang');//是否嘉奖 0=不嘉奖  1=嘉奖
						var level = $(this).attr('level');//是否置顶
						var desc = $(this).attr('desc');//彩种描述
						var ishow = $(this).attr('ishow');//彩种是否显示 ： 0-显示，1-不显示
						if(gid == '70'){
							if(desc != ''){
								$('#tag_jcdg').find('p').html(desc);
							}
						}
						if(desc != ''){
							$('#tag_'+gid).find('p').html(desc);//对每个彩种的描述
						}
					});
					$('#ball a:last').html("立即预约");
				}
			});
			
		},function(){
			$("ul.all2 li a").each(function(){
				var id = $(this).attr("id");
				if(id && id!=null){
					id = id.split("_")[1];
					
					$(this).find('p').html(introduction[id]);//对每个彩种的描述
				}
			})
		})
	}
	var openLogin = function (xo) {//打开登录层
		...
		TopAnch.init({//初始化页头
			title: ((CP.Data.source == '3037' || CP.Data.source == '3036')?'彩票账户登录':'9188账户登录'),
			prevShow: true,
			prevFun:function(){//左侧按钮绑定事件
			...
			nextText: '注册',
			nextFun: function(){
				window.location.replace('/alone/newregister.html');
			},*/	
			isBack:true,
			style:true
		...
		var wContact=$(document).height()-$('#regContact').outerHeight();
		var w=wContact-385;
		$('#regContact').css({'margin-top':w});
	};
	var reLogin = function(){
		function back(){
			if(url_){//从哪个url进入的就返回哪个url
...
var hashchange = function () {
	var _G_hash = null;
	var $domContent = $('#content_home');//内容层
	...
	var getPage = function (url) {
		...
		if ($("#home_page").length && url == 'index.html') {//如果为首页
			/***
			...
			***/
			CP.Data.grade(function(){//判断白名单用户
				$('#ball a:last').html("立即预约");
			}, function(){
				$('#ball a:last').html("购买");
			})
			...
			window.homeFun && homeFun();//首次不进来
		} else {
			if(url.indexOf('jczq')>0 || url.indexOf('jcdg')>0 || url.indexOf('jclq')>0 || url.indexOf('bjdc')>0 || url.indexOf('sfgg')>0
					 || url.indexOf('r9')>=0 || url.indexOf('sfc')>=0 || url.indexOf('gddg')>=0 || url.indexOf('activity/game')>=0){
...
//			if (false) {//  CP.Storage.get(url) && CP.Storage.get(url) != ''
		...
		function success(html_) {
			...
			//alert(111)
			$("#home_page").hide();
			pageLoading.hide();//加载中的珠子
			if(document.domain == "m.cp.eastday.com" || document.domain == "zccp.eastday.com" || document.domain == "ttcp.eastday.com"){
				$('#dohm').hide()//东方网的时候 合买按钮隐藏
			}
			$('#content_home').html(html_);
			window.scrollTo(0, 1);
		}
	};
	...
	var change = function () {
		...
		var page = decodeURIComponent(CP.Util.getParaHash('p'));//页面地址
		...
		switch (type) {
			...
			case 'url':
				...
				page = pageRoute;
				CP.Cookie.set('pageRoute', encodeURIComponent(page));//重新设置当前页面的路径
				getPage(page);//获取当前页面路径
				return false;
				break;
			case 'fun':
				$('#content_home').show();
				...
				if (!/CP./.test(fun)) {
					window.location.replace(hostUrl + '#type=index');//重新回到首页
					return;
				}
				var para = CP.Util.getParaHash('in');//获取当前页面
				if (/\,/.test(para)) {
					para = para.split(',');
				}
				...
				try {
					...
					switch (fun.length) {
						case 1:
							...
							break;
						case 2:
							window[fun[0]][fun[1]](para);//这里为方法调用，para相当于一个url
							break;
						case 3:
	...
	var init = function (o) {
		...
		var agent_1 = localStorage.getItem('agent') || false;
		if(agent_){
			localStorage.setItem('agent',agent_);
			CP.Data.source = agent_;
		}else if(agent_1){
			CP.Data.source = agent_1;
		}
		...
		if (window.location.hash == '') {//设置首页的hash值
			window.location.replace('#type=index');
		} else {
			change();
		}
