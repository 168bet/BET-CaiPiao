...
CP.Viewpath = (function () {
	...
	$.ajax({
		url:"/trade/qview.go",
		data:{hid:$_hid,gid:$_gid,state:1,fflag:0},
		dataType:"XML",
		success:function(xml){
			...
			var tmoney=rows.attr("tmoney");
			var mulity=rows.attr("mulity");
			sdParam["shareGod"]=shareGod;
			sdParam["showCode"]=showCode;
			sdParam["minRatio"]=minRatio;
			sdParam["tmoney"]=tmoney;
			sdParam["mulity"]=mulity;
			CP.Viewpath.init();
			var fck = '<a href="javascript:;" class="left loginBtn" id="fenxck" style="margin-top:2rem;">查看分享详情</a>';
			var fx = '<a href="javascript:;" class="left loginBtn" id="fenx" style="margin-top:2rem;">分享赢打赏</a>'
			if(sdParam["showCode"] != undefined && sdParam["shareGod"] == '1'){
				$('#sdfen').html(fck);
			}else if(sdParam["showCode"] != undefined && sdParam["shareGod"] == '0'){
				$('#sdfen').html(fx);
			}
			
			$('#fenx').bind(end_ev, function (){
				$('#layer_bg').show();
				$('#fxy_layer').show();
			});
			$('#fenxck').bind(end_ev, function (event){
				var ev = event || window.event;
				ev.stopPropagation();
				window.location.href ="/sdjc/buy.html?hid="+$_hid
			});
		},
		error:function(){
			CP.Viewpath.init();
	...
	var grade = localStorage.getItem("grade");//白名单等级
	var $dom = {
			hemai : $('#hemai'),//合买层
			zigou : $('#zigou')//代购层
	};
	...
	var o = {
	            ...
	            goBack: function () {
	            	...
	            	CP.Data.grade(function(){
	            		TopAnch.init({
		            		...
		            		nextShow:true,
							nextText:function(){
								return parseInt(grade)>0?"继续预约":"去投注"
							},
							nextFun:function(){
								if($_gid=="98"||$_gid=="99"){
				...
				if (t.itype == '1') {//合买方案
					...
					if(t.acode == undefined){
						if(t.gid==84 ||t.gid==85 ||t.gid==86 ||t.gid==87 ||t.gid==88 ||t.gid==89){
							...
							if(sdParam["shareGod"]==2){
								$('#zigou p').eq(0).html("竞彩-跟买")
							}else{
					...
					if(sdParam['shareGod']=="2"){
						$("#faje").html('<i>跟买金额</i><span class="gray sinceTxt">'+sdParam['tmoney']+'元&nbsp;('+sdParam['mulity']+'倍 )</span>')
						$("#dsbl").html('<i>打赏比例</i><span class="sinceTxt">'+sdParam['minRatio']+'%</span>')
						$("#dsbl").show();
					}else{
						$('#faje span').html(t.tmoney+'元&nbsp;('+hb+t.mulity+'倍 )');//含'+t.rpmoney+'元红包  
					}
					...
					if (t.award == "2") {//自购中奖情况
						...
						if (t.rmoney > 0) {
							if(sdParam['shareGod'] == "2"){
								var m = (t.rmoney*sdParam['minRatio']/100).toFixed(3);
								wininfostr += '方案金额'+ parseFloat(t.rmoney)+'元 ，'+'打赏发单人'+m+'元';
							}else{
							if (t.istate == 3 || t.istate == 4) {
								wininfostr = "<font class='yellow'>该撤单方案中奖"+parseFloat(t.rmoney)+"元</font>";
							} else {
								/*wininfostr = "<font class='yellow'>已中奖</font>";*/
								wininfostr += "<font class='yellow'>"+parseFloat(t.rmoney)+"元</font>&nbsp;"
								if(parseFloat(t.tax) != parseFloat(t.rmoney)){
									wininfostr += "(税后<font class='yellow'>"+ parseFloat(t.tax)+"元</font>)"
								}
							}
							/*wininfostr += "<br/>("
								+ "税前<font class='yellow'>"
					...
					if (t.visible == 'false' && t.shareGod == '2'){
						...
						if(t.source == 6 || t.source == 7){//奖金优化
							$("#zgContent div").html('奖金优化方案&nbsp;<a href="#type=url&p=user/content.html&lotid='+t.gid+'&projid='+t.hid+'">点击查看</a>');
							$('#clasli').show();
							Against.jjyh(t.gid, t.pid, t.hid, t.ccodes);
						}
						else{
							$("#zgContent div").html('<a href="#type=url&p=user/content.html&lotid='+t.gid+'&projid='+t.hid+'">点击查看</a>');
						}
					}
					/*]]自购投注内容*/
				}
			...
			bind : function () {
				...
				$(document).delegate("img.table_icon","click",function(){
					alert("开赛后公开")
				})
				...
				$('#layer_bg').bind(end_ev, function (event){
					var ev = event || window.event;
					ev.stopPropagation();
					$('#layer_bg').hide();
					$('#fxy_layer').hide();
				});
				$('#fxy_layer ul li').bind(end_ev, function (event){
					var ev = event || window.event;
					ev.stopPropagation();
					$(this).addClass('on').siblings().removeClass('on');
				});
				
				$('#fenxBtn').bind(end_ev,function(event){
					var $_hid = CP.Util.getPara('hid', window.location.href, 2);//方案编号
					var $_gid = $_hid.substr(0, 2);//彩种id
	...
	var Against = {
					...
					$.ajax({
						...
						success : function(xml) {
							...
							r.each(function(aa){
								...
								if(sdParam['shareGod']=='2' && sdParam['showCode']=="false"){
									html += '<td '+(aa==0?"width='15%'":"")+' rowspan="2" class="r9last"><p>开赛后</p> <p>公开</p><img src="img/table.png" class="table_icon" style="width:20px;height:20px"></td></tr>';
								}else{
								...
								html += '<tr>'
									...
									for(var i=0;i<ccodes.length;i++){
										...
										html += '<span>'+c1+'('+c2+')</span>';//如c1表示胜平负  c2表示赔率
									}
									html+='</div>'
									html+='</td>'
								}
						...
						success : function(xml) {
							...
							if(lotid == 70){//竞彩足球  混合投注
								...
								r.each(function(a){
									...
									var cctz = [];
									if(codes.indexOf(';')>0){
										cctz = $.map(codes.split(';'),function(val, index){//将对象中的每个值，使用map遍历形成一个新的对象
											if(index == '0'){
												return val.split('|')[1];
											}else{
									...
									for(var i=0;i<cctz.length;i++){
										...
										if(id == cctz[i].split('>')[0]){
											...
											html += '<tr id="'+id+'"><td '+(a==0?"width='15%'":"")+' rowspan="'+(cctzLength2+1)+'">'+wk2+'<br>'+id2+'</td>';
								  			...
								  			if(sdParam['shareGod']=="2" && sdParam['showCode']=="false"){
								  				html += '<td '+(a==0?"width='15%'":"")+' calss="r9last" rowspan="'+(cctzLength2+1)+'"><p>开赛后</p> <p>公开</p><img src="img/table.png" class="table_icon" style="width:20px;height:20px"></td></tr>';
											}else{
												html += '<td '+(a==0?"width='15%'":"")+' calss="r9last" rowspan="'+(cctzLength2+1)+'">'+ban+'<br/>'+quan+'</td></tr>';
											}
											...
											for(var j=0;j<cctzLength;j++){
												...
												if(cctz[i].split('>')[1].split('+')[j].split('=')[0] == "RQSPF"){
													...
													html += '<tr>'
														if(sdParam['shareGod']=='2' && sdParam['showCode']=="false"){//跟买，未显示
														html+=''
													}else{
														html += '<td><div class="tdleft gray">&nbsp;';
														if(result!=""){
								...
								if(code[0]=="RQSPF"){//jczq 让球胜平负 90
									...
									if(c == 0){
										...
										html += '<tr>'
											if(sdParam["shareGode"]=="2" && sdParam["showCode"]=="false"){
												html+=''
											}else{
												html+='<td><div class="tdleft gray">&nbsp;';
												for(var i=0;i<code1.length;i++){
			...
			/***
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lcbetTitle mgTop06">
    		<tbody>
    			<tr><td width="80%">联赛/投注选项</td><td width="20%" style="border-right:none">彩果</td></tr>
    		</tbody>
    	</table>
    	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lcbetTable">
    		<tbody>
    			<tr><td style="padding-left: 8%;">2016年欧洲杯</td><td width="20%" rowspan="2" class="r9last">西班牙</td></tr>
    			<tr><td width="80%"><span>西班牙(6.05)</span><span>法国(6.05)</span><span>比利时(2.16)</span></td></tr>	        			
    		</tbody>
    	</table>
			***/
			//冠亚军
			gyj_clasli:function(lotid, pid, projid, codes, isHm){
				$.ajax({
					...
					success : function(xml) {
						...
						var r = R.find("item");
						var html='';
						html = '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lcbetTitle mgTop06">';
						html += '<tbody><tr><td width="80%">联赛/投注选项</td><td width="20%" style="border-right:none">彩果</td></tr></tbody>';
						html += '</table>';
						html += '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="lcbetTable"><tbody>';
						html+='<tr><td width="80%"><cite style="padding-left: 8%;">2016年欧洲杯</cite></td><td width="20%" rowspan="2" class="r9last" id="sg">西班牙</td></tr>'
							html+='<tr><td width="80%">'
						if(lotid == 98||lotid == 99){
							...
							if(isHm){
								...
								$('#clasli').html(html);
								$("#clasli").removeClass("lcbetText3").addClass("lcbetText2")//冠亚军和和其他玩法的详情页显示不一样所以用样式控制
								$("#sg").html(tmp);
							}
							...
							if(isHm){
								...
								$('#clasli').html(html);
								$("#clasli").removeClass("lcbetText2").addClass("lcbetText3");
							}
						}
			...
			/***
			 * <rows showCode="true" cast="2" istate="2" mulity="1" tmoney="44" rmoney="0" tax="0" award="0" 
			 * rpmoney="0" btime="2016-11-04 17:06:13" imoneyrange="238.08" jjyh="1" 
			 * source="6" minRatio="0" ipay="0" upay="0" shareGod="1" sharedNickid="花生" hideSharedNickid="花*****" visible="true">
				<matchs fs="平稳盈利">
				<row id="161104002" name="周五002" hn="海登海姆" gn="卡尔斯鲁厄" close="-1" hs="" gs="" hhs="" hvs="" jsbf="" ccodes="SPF_3_1.92" />
				<row id="161104003" name="周五003" hn="奥厄" gn="纽伦堡" close="1" hs="" gs="" hhs="" hvs="" jsbf="" ccodes="SPF_0_2.18,SPF_1_3.35" />
				</matchs>
				<detail gg="2*1" missmatch="0">
				<row str="海登海姆(胜),奥厄(负)" bs="10" />
				<row str="海登海姆(胜),奥厄(平)" bs="12" />
				</detail>
				<jindu node="1" percent="80" paint="约单中" kjtime="(预计:11-05 03:50)" pjtime="" />
			</rows>
			 * 
			***/
			/***
			 * 
			jjyh1:function(){//新版奖金优化
				if((lotid==90 || lotid==70 || lotid==72 || lotid==91 || lotid==92 || lotid==93) && parseInt(expect)>=20130514){
					$.ajax({
						type : 'GET',
						data : data,
						dataType : "xml",
						url : '/trade/qview.go?hid='+projid+'&gid='+lotid+'&state=1&fflag=0',
						success : function(xml) {
							var rows = $("xml").find("rows");
							
							var matchs = rows.find("matchs");//场次内容信息
							var r1 = matchs.find("row");
							
							var detail = rows.find("detail");//优化对阵节点
							var gg = detail.attr("gg");//过关方式
							var missmatch = detail.attr("missmatch");
							var r2 = detail.find("rows");
							...
							//<row id="150408001" name="周三001" hn="新泻天鹅" gn="鸟栖沙岩"
							//close="-1" hs="" gs="" hhs="" hvs="" jsbf="已完 1:0/1:0" ccodes="SPF_3_2.00,RQSPF_3_4.20" />
							r1.each(function(aa){
								var id = $(this).attr("id");//场次id
								var name = $(this).attr("name");//
								var hn = $(this).attr("hn");//主队
								var gn = $(this).attr("gn");//客队
								var close = $(this).attr("close");//让球数
								var hs = $(this).attr("hs");//主队得分
								var gs = $(this).attr("gs");//客队得分
								var hhs = $(this).attr("hhs");//主队半场得分
								var hvs = $(this).attr("hvs");//客队半场得分
								var jsbf = $(this).attr("jsbf");//完场描述
								var ccodes = $(this).attr("ccodes");//投注内容
								
								var obj = from_code_to_span(gid,ccodes,jjyh);//调用解析ccodes方法
								...
								for(var i in obj){//i表示玩法类型
									if(obj[i].length>1){
										for(var j=0;j<obj[i].length;j++){
											html += '<span>'+i+'('+obj[i][j].split("_")[1]+')</span>';
										}
									}else{
										html += '<span>'+i+'('+obj[i].split("_")[1]+')</span>';
									}
							...
							r2.each(function(){
								...
								var bs = $(this).attr("bs");
								var op = str.split(",")
								html2 += '<tr><td '+(i==0?"width='40%'":"")+'>'+op[0]+"<br/>"+op[1]+'</td>';
								html2 += '<td '+(i==0?"width='30%'":"")+'>'+gg+'</td>';
			...
			objCode:{},
			//gid=70ccodes="SPF_0_3.65,RQSPF_3_4.05,CBF_5:1_150.0,CBF_9:0_100.0,CBF_0:9_250.0,JQS_1_3.30,JQS_7_40.00
			from_code_to_span:function(gid,ccodes,jjyh){
				if(gid==70){//混合投注
					var ccodes2 = ccodes.split(",");
					for(var i=0;i<ccodes2.length;i++){
						var c1 = ccodes2[i];
						var len = c1.indexOf("_");
						var ck = c1.substring(0,len);
						var cv = c1.substring(len+1,c1.length);
															...
															for(var q = 0; q<dd.length; q++){
																...
																if(ee[0] == 'SPF'){
																	...
																	for(var j=0;j<touzhu.length;j++){
																		...
																		var ii = touzhu[j].replace("0","2").replace("3","0");
																		if(result != ""){//开奖结果不等于空
																			if(touzhu[j] == rqresult){//投注内容和结果相同,标红
...
//																		html += '<span><em class="yellow">'+wwc[n].replace("9:0","胜其它").replace("9:9","平其它").replace("0:9","负其它")+'</em>('+spvalue[cbfstr.indexOf(wwc[n])]+')</span>';
																		...
																		var ii = cbfstr.indexOf(touzhu[j]);
																		if(result != ""){
																			if(touzhu[j] == result){
																	...
																	for(var j=0;j<touzhu.length;j++){
																		...
																		var ii = jqsstr.indexOf(touzhu[j]);
																		if(result != ""){
																	...
																	touzhu = ee[1].split('/');
																	for(var j=0;j<touzhu.length;j++){
																		var bq = touzhu[j].split('-')
																		var text='';
																		for(var i=0;i<bq.length;i++){
																			text += bq[i].replace("3","胜").replace("1","平").replace("0","负");
																		}
																		var oracle2 = text;
																		var ii = bqcstr.indexOf(touzhu[j]);
																		if(result != ""){
																			if(touzhu[j] == rqresult){
