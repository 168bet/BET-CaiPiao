CP.JC = (function() {
	...
	var contrast = {
			...
			jclq : {
				...
				sfc : {maxPass:8, gid:96, location:{'3':'0','0':'1'}},
				hh : {maxPass:8, gid:71},
				minLength:2,//最小场次
				maxLength:15,//最多15场
		        maxBs:50000//最大投注倍数
    		...
    		render : function () {
    			...
    			***/
    			...
    			if(!againstData.length){//对阵列表中是否有数据
    				...
    				/*兼容篮彩样式[[*/
    				if(lotteryType == 'jclq'){
    					var cl_ = {'sf':'jclqVs','rf':'jclqVs','dxf':'jclqDxf','hh':'jclqhh'}[lotteryPlayName];
    					$item.attr('class',cl_);
    				}else{
    ...
    var o = {
            ...
            goBack: function () {
            	...
            	TopAnch.init({
    				...
    				headerSelect: (lotteryType == 'jczq' ? [{name:'胜平负',fun:o.changePlayType,cur:2},{name:'让球胜平负'},{name:'混投'},{name:'比分'},{name:'总进球'},{name:'半全场'}] :
    					(lotteryType == 'jclq' ? [{name:'胜负',fun:o.changePlayType,cur:3},{name:'让分'},{name:'大小分'},{name:'混投'}] :
        ...
        changePlayType : function(index){
			...
			switch(lotteryType){
				...
				case 'jclq':lotteryPlayName={'0':'sf','1':'rf','2':'dxf','3':'hh'}[index];break;
