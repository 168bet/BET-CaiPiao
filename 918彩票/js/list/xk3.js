	...
	var o = {
				...
				goBack: function () {
					TopAnch.init({
						...
						menu:[{'name':'投注选号','url':'javascript:;','cur':true},
						      {'name':'开奖结果','url':'#type=url&p=kj/result.html?in_=10','cur':false},
						      {'name':'玩法帮助','url':'#type=url&p=wf/xk3.html','cur':false}],
						      style:false
					});
				}
			},
			...
			addList : function(){
				if(!g.qihao_id){
					...
					window.location.href = "#type=fun&fun=CP.XK3.openList";
					o.setLocal();
				}
			},
			/*添加到投注列表]]*/
			...
			openList : function(){
				...
				TopAnch.init({
					...
					prevShow:true,
					prevFun:function(){//左侧按钮绑定事件					
						window.location.href = '#type=url&p=list/xk3.html';
					},
					isBack:true,
					nextShow:false,
					style:false
			...
			setLocal : function(){
				...
				g.codes=codes.join(";");
				CP.Storage.set('xk3',g.codes,true); //存list(json)到sessionStorage
			},
			//sessionStorage数据恢复到投注列表
			fromLocal : function(){
				var _json = CP.Storage.get('xk3',true);
				if(_json)
				{
					try{
						g.codes = _json;
	        ...
	        info : function () {
	        	function main(){
	        		$.ajax({
						//url : CP.Data.apps+'/trade/info.go?gid=10',
	        			url:CP.Data.apps+'/tradeios/lotteryInfo.go?gid=10',
						type : "POST",
						dataType : "xml",
				...
				function eachClock(){
					...
					if(diff >= 0){
						...
						if(timeout[2]>=10){
							msg = timeout[2]+':'+CP.Util.pad(timeout[3],2);
						}else{
...
//					clearInterval(timer2); //清除时时彩的定时器
			...
			getArgument : function(t){
				var buy = {};
				console.log(g.codes+"~~~")
				var code = CP.Util.joinString('10', g.codes);
				console.log(g.codes)
				console.log(code)
				switch(t){
					/* 1:自购 3:追号 */
			...
			dopay : function(){
				...
				sessionStorage.removeItem('xk3');//购买的时候情况sessionstorage
			},
			/*购买弹窗]]*/
			
			/*购买验证[[*/
			dobuy : function(){
				console.log(g.count_zs+"~"+g.qishu+"~"+g.beishu)
				var info = '';
	...
	var bind = function(){
		...
		$('#hand').bind(start_ev,function(){location.href='#type=url&p=list/xk3.html';});
		$('#dobuy').bind(start_ev,function(){o.dobuy();});//代购
		$('#clearAll').bind(start_ev,function(){//清空投注列表
			$('#bet_list').html('');
			$("#tbox_qishu").val("1");
			$("#tbox_beishu").val("1");
			CP.Storage.remove('xk3',true);
			o.countAll();//统计总金额 注数
			if(g.qishu>1){
