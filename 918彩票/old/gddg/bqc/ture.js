...
CP.Gddg.Bqc_t = function(){
	...
	var w = {
			init : function(){
				var html = localStorage.getItem('dg_bqc_SelectNum');
			...
			href_ : function(go){
				...
				dom.item.find('article[v=y]').each(function(){
					...
					$(this).find('.jjyhbuy .cur').each(function(){
						...
						var t3 = $(this).attr('v');
						t3 = {'胜胜':'3-3','胜平':'3-1','胜负':'3-0',
								'平胜':'1-3','平平':'1-1','平负':'1-0',
								'负胜':'0-3','负平':'0-1','负负':'0-0'}[t3]||t3;
						t += 'BQC|'+ t1 +'='+ t3 +'|1*1_'+ t2 +';';//拼code
						ttt += t3 +',';
					});
				...
				localStorage.setItem('dg_bqc_PollNum',t);
				location.href = go;
			}
	};
	var bind = function(){
		...
		dom.jjyhinput.on('click','input:eq(0)',function(){//添加赛事
			w.href_('#class=url&xo=gddg/bqc/more.html');
		...
		dom.gddgFloat.on('click','a:eq(0)',function(){//
			...
			if(l<=0){
        		...
        		w.href_('/#class=url&xo=gddg/pay/dg_fqhm.html&money='+m+'&gid=92');
        	}
			...
			if(l<=0){
				...
				w.href_('/#class=url&xo=gddg/pay/paydg.html&money='+m+'&bk=0&gid=92');
        	}
		});
