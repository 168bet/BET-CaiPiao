...
CP.Gddg.Jq_t = function(){
	...
	var w = {
			init : function(){
				var html = localStorage.getItem('dg_jq_SelectNum');
			...
			href_ : function(go){
				...
				dom.item.find('article[v=y]').each(function(){
					...
					$(this).find('.jjyhbuy .cur').each(function(){
						...
						t += 'JQS|'+ t1 +'='+ $(this).attr('v') +'|1*1_'+ t2 +';';//拼code
						ttt += $(this).attr('v') +',';
					});
				...
				localStorage.setItem('dg_jq_PollNum',t);
				location.href = go;
			}
	};
	var bind = function(){
		...
		dom.jjyhinput.on('click','input:eq(0)',function(){//添加赛事
			w.href_('#class=url&xo=gddg/jq/more.html');
		...
		dom.gddgFloat.on('click','a:eq(0)',function(){//合买
			...
			if(l<=0){
        		...
        		w.href_('/#class=url&xo=gddg/pay/dg_fqhm.html&money='+m+'&gid=93');
        	}
			...
			if(l<=0){
				...
				w.href_('/#class=url&xo=gddg/pay/paydg.html&money='+m+'&bk=0&gid=93');
        	}
		});
