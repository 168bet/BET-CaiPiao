...
CP.Gddg.Rq = function(){
	...
	var PollNum = localStorage.getItem('dg_rqspf_PollNum');
	var w = {
			dz : function(){
				$.ajax({
...
//							}
							r.each(function(){
								...
								if((32 & isale) > 0){//是否停售
									if(close.indexOf('-')>=0){
										tmp_close='<i class="blue">'+(close=='0'?'':'('+(close.indexOf('-')>=0?close:'+'+close)+')')+'</i>';
									}else{
										tmp_close='<i class="red">'+(close=='0'?'':'('+(close.indexOf('-')>=0?close:'+'+close)+')')+'</i>';
									}
									if(PollNum && PollNum_t.indexOf(itemid)>=0){
										html += '<ul class="sfcxs" t="'+itemid+'" v="y" z="'+hn+'" k="'+gn+'" close="'+close+'">';
										...
										html += '<span v="3" '+(pc.indexOf('3')>=0?'class="cur"':'')+' sp="'+z_s+'"><em>'+hn+'</em>'+tmp_close+'<cite>胜</cite></span><span class="spfvs '+(pc.indexOf('1')>=0?'cur':'')+'" v="1" sp="'+p+'"><em>VS</em><cite>平</cite></span><span v="0" '+(pc.indexOf('0')>=0?'class="cur"':'')+' sp="'+k_s+'"><em>'+gn+'</em><cite>胜</cite></span>';
									}else{
										html += '<ul class="sfcxs" t="'+itemid+'" z="'+hn+'" k="'+gn+'" close="'+close+'">';
										html += '<li class="li_weige"><em>'+itemid.substr(-3)+'</em><p style=\'color:'+cl+'\'>'+mname+'</p><cite>'+end_time+' 截止</cite><i class="xzup xzdown"></i></li>';
										html += '<li>';
										html += '<p class="spfzpk spfzpk2">';
										html += '<span v="3" sp="'+z_s+'"><em>'+hn+'</em>'+tmp_close+'<cite>胜</cite></span><span class="spfvs" v="1" sp="'+p+'"><em>VS</em><cite>平</cite></span><span v="0" sp="'+k_s+'"><em>'+gn+'</em><cite>胜</cite></span>';
									}
...
//		});
		...
		jQuery('.deleted').Touch(function(){
			...
			localStorage.removeItem('dg_rqspf_PollNum');
			w.my_play();
		});
		jQuery('#isOk_').click(function(e){
			if(parseInt($('#c_').html())>=1){
				...
				jQuery('#item section').find('div:eq(1) ul[v=y]').each(function(){
					var z = $(this).attr('z'),k = $(this).attr('k'),close = $(this).attr('close'),tmp_close = '';
					if(close.indexOf('-')>=0){
						tmp_close='<i class="blue">'+(close=='0'?'':'('+(close.indexOf('-')>=0?close:'+'+close)+')')+'</i>';
					}else{
						tmp_close='<i class="red">'+(close=='0'?'':'('+(close.indexOf('-')>=0?close:'+'+close)+')')+'</i>';
					...
					c += '<p><span>'+z+''+tmp_close+'  vs  '+k+'</span><cite>奖金</cite></p>';
				...
				localStorage.setItem('dg_rqspf_SelectNum',c);
				localStorage.removeItem('dg_rqspf_PollNum');
				window.location.href='#class=url&xo=gddg/rq/ture.html';
			}else{
				D.msg('请至少选择1场比赛');
			}
		});
	...
	var init = function(){
		dom.downline.animate({left:'21.7%'},500);
		w.dz();
		bind();
	};
	init();
