...
*Date: 2014.9.24
*/
var zs_ = decodeURIComponent(CP.Util.getParaHash("zs"));//注数
var bs_ = decodeURIComponent(CP.Util.getParaHash("bs"));//倍数
var m_ = decodeURIComponent(CP.Util.getParaHash("money"));//总金额
var gid_ = decodeURIComponent(CP.Util.getParaHash("gid"));//当前彩种id
var pid_ = decodeURIComponent(CP.Util.getParaHash("pid"));//当前彩种id
...
var Jc_hm = {
		...
		bind: function(m){//m 方案总金额
			...
			$('#my_').next().Touch(function(){
				...
				location.href='#class=url&xo=trade/paybd.html&notes='+zs_+'&pid='+pid_+'&isPublic='+p+'&countMoney='+m_+'&bs='+bs_+'&rg='+rg+'&bd='+bd+'&xyText='+desc+'&wrate='+wrate+'&bk=1&gid='+gid_;
			});
		},
		my_: function(){
			var rg = $('#rg').val();
...
$(function(){
	if(zs_ != '' && gid_ != '' && bs_ != ''){
		Jc_hm.initail(zs_,bs_,m_,gid_);//初始化加载
	}else{
		history.go(-1);
	}
});