...
//				  a.nav.on('click','li',function(){
				  a.nav.find('li').Touch(function(){
					  ...
					  var t1 = $(this).width();
					  a.nav_c.animate({left:t+'px',width:t1+'px'},200,function(){cc.addClass('cur').siblings().removeClass('cur');});
				  ...
				  a.pay_.on('click',function(){//投注
						...
						if(zs == 0){
							...
							var code = '';
							code = c.code(n,zs);
							var plsSelectNum = localStorage.getItem('pls_SelectNum');
							(plsSelectNum && (plsSelectNum += code)) || (plsSelectNum = code);
							localStorage.setItem('pls_SelectNum',plsSelectNum);
							location.href = '#class=url&xo=p3/ture.html';
						}
					});
			  ...
			  info : function(){//填充开奖号码列表
				  $.ajax({
						url : '/trade/info.go?gid='+$('#lotid').val(),
						type : "POST",
						dataType : "xml",
