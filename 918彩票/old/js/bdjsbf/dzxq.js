...
var XX = {
		...
		bindEvent:function(){
			...
			***/
			...
			$('.fcbackIco2').bind('click',function(){
				if(flag){
					window.location.href="/bdjsbf/?flag="+flag;
				}else{
					if (history.length == 0) {
						return false;
					} else {
		...
		loadHeader:function(){
			...
			//XX.getServerTime();
			$.ajax({
				url:'/zlk/jsbf/bd/onerounddata/'+XX.itemid+".xml",
				type:'GET',
				DataType:'XML',
				success: function(xml){
		...
		bssj:function(){
			var html="";
			$.ajax({
				url:'/zlk/jsbf/bd/oneroundeventdata/'+XX.itemid+".xml",
				type:'GET',
				DataType:'XML',
				success: function(xml){
					var rows = $(xml).find("rows");
							...
							row.each(function(){
								...
								if(teamType==0){
									if(eventType==0){//进球
										...
										html+='<div class="timeleft"><span><em>'+pn+'</em><cite class="redpico"></cite></span><b>'+time+'&prime;</b></div>'
									...
									if(eventType==0){//进球
										...
										html+='<div class="timeright"><b>'+time+'&prime;</b><span><cite class="redpico"></cite>'+pn+'</span></div>';
									}else if(eventType==5){//两黄变红
		...
		symd:function(){
			...
			var tbhtmlB="";
			$.ajax({
				url:'/zlk/jsbf/bd/roundPlayerdata/'+XX.itemid+'.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					var rows = $(xml).find("rows");
					...
					if(SFPlayerA){
						...
						$("#sfmd .dzxqList ul:eq(0)").html("暂无数据")
					...
					if(SFPlayerB){
						...
						$("#sfmd .dzxqList ul:eq(1)").html("暂无数据")
					...
					if(TBPlayerA){
						...
						$("#tbmd .dzxqList ul:eq(0)").html("暂无数据")
					...
					if(TBPlayerB){
						...
						$("#tbmd .dzxqList ul:eq(1)").html("暂无数据")
		...
		jstj:function(){
			//var html="";
			$.ajax({
				url:'/zlk/jsbf/bd/oneroundjstjdata/'+XX.itemid+'.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					var rows = $(xml).find("rows");
		...
		lsjf:function(){
			...
			var kkfnum=0
			$.ajax({
				url:'/zlk/jsbf/bd/zjdata/'+XX.itemid+'.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					var rows = $(xml).find("rows");
						...
						***/
						...
						if(hn==gname || gn==gname){
							...
							if(hn==gname){
								...
								htmlB+='<li><b><cite>'+ln+'</cite>'+time+'</b><span class="blue">'+hn.substring(0,6)+'</span><em>'+hsc+':'+asc+'</em><span class="">'+gn.substring(0,6)+'</span><em class="red">'+sf+'</em></li>';
							}else if(gn==gname){
								if(hsc<asc){
									ksnum++;
									sf="胜";
		...
		pl:function(){
			var html = $("#pl").html();
			$.ajax({
				url:'/zlk/jsbf/bd/oudata/'+XX.itemid+'.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					var rows = $(xml).find("rows");
					...
					$("#pl ul li em").bind("click",function(){
						...
						$.ajax({
							url:'/zlk/jsbf/bd/odds/'+XX.itemid.split("_")[0]+'/OP/'+XX.qc+'/'+XX.qc+'_'+v+'.xml',
							type:'GET',
							DataType:'XML',
							success: function(xml){
								var Rows = $(xml).find("Rows");
