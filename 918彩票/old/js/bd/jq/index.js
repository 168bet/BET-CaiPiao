 ...
 * Date: 2014-7-2
 */
var PollNum = localStorage.getItem('bd_jq_PollNum');
var Bjdc_jq = {
		initial: function(){
			Bjdc_jq.add_C();//初始化添加对阵内容
			
			Bjdc_jq.bind_();//初始化的一些绑定事件
		},
		add_C: function(){
			D.load();
			$('.loading').hide();
			$.ajax({
				url:'/data/app/bd/jqs.xml',
				type:'GET',
				DataType:'XML',
...
//							var ic = $(this).attr('ic');//是否取消  1y-0n
					...
					Bjdc_jq.my_play();
					Bjdc_jq.bind_delay();//必须有对阵之后的绑定事件
				}
			});
		},
		bind_delay: function(){
		...
		bind_: function(){
			...
			jQuery('#isOk_').click(function(){
				if(parseInt($('#c_').html())>0){
					...
					localStorage.removeItem('bd_jq_PollNum');
					window.location.href='#class=url&xo=bjdc/jq/ture.html&pid='+$('#expect').val();
				}else{
					D.tx('请至少选择1场比赛');
				}
			});
