...
var Bjdc_jq_t = {
		initial: function(){
			var html = localStorage.getItem('bd_jq_SelectNum');
		...
		bind: function(){
			...
			jQuery('#clearTab').Touch(function(){//清空
				localStorage.removeItem('bd_jq_PollNum');
			...
			jQuery('#tjss').click(function(){//添加赛事
				Bjdc_jq_t.href_('#class=url&xo=bjdc/jq/index.html');
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c == '0'){
					...
					Bjdc_jq_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid+'&pid='+pid);
				}
			});
			jQuery('#payment').click(function(){//代购
					...
					if(c == '0'){
						...
						var bs = $('#bs_').val();
						var gid = $('#lotid').val();
						Bjdc_jq_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid+'&pid='+pid);
					}
			});
		...
		href_: function(u){
			...
			localStorage.setItem('bd_jq_PollNum',c);
			location.href=u;
		},
		local_: function(){//bd_spf_SelectNum重新赋值
			var c = jQuery('#content').html();
			localStorage.setItem('bd_jq_SelectNum',c);
		},
		c: function(){
