...
var Bjdc_t = {
		initial: function(){
			var html = localStorage.getItem('bd_spf_SelectNum');
		...
		gg_: function(){//一些事件触发改变过关方式
			...
			if(!l){
				...
				var ll = {'1':'单关','2':'2串1','3':'3串1','4':'4串1','5':'5串1','6':'6串1','7':'7串1','8':'8串1','9':'9串1','10':'10串1','11':'11串1','12':'12串1','13':'13串1','14':'14串1','15':'15串1'}[l];
				$('#selectPlay').val(ll);
				$('#selectPlay').attr('v',l);
		...
		bind: function(){
			...
			jQuery('#tjss').click(function(){//添加赛事
				Bjdc_t.href_('#class=url&xo=bjdc/index.html');
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c == '0'){
					...
					Bjdc_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid+'&pid='+pid);
				}
			});
			jQuery('#payment').click(function(){//代购
					...
					if(c == '0'){
						...
						var bs = $('#bs_').val();
						var gid = $('#lotid').val();
						Bjdc_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid+'&pid='+pid);
					}
			});
		...
		href_: function(u){
			...
			localStorage.setItem('bd_spf_PollNum',c);
			location.href=u;
		},
