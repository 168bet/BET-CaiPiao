 ...
 * Date: 2014-7-2
 */
var PollNum = localStorage.getItem('bd_spf_PollNum');


var Bjdc = {
		initial: function(){
			Bjdc.add_C();//初始化添加对阵内容
			Bjdc.bind_();//初始化的一些绑定事件
		},
...
//							var ic = $(this).attr('ic');//是否取消  1y-0n
							...
							if(PollNum && PollNum_t.indexOf(itemid)>=0){
								var pc = PollNum_c[PollNum_t.indexOf(itemid)];
								html += '<span v="3" '+(pc.indexOf('3')>=0?'class="cur"':'')+' sp="'+z_s+'"><em>'+hn.substr(0,5)+'<i class="green">'+(close=='0'?'':'('+(close.indexOf('-')>=0?close:'+'+close)+')')+'</i></em><cite>胜</cite></span><span class="spfvs '+(pc.indexOf('1')>=0?'cur':'')+'" v="1" sp="'+p+'"><em>VS</em><cite>平</cite></span><span v="0" '+(pc.indexOf('0')>=0?'class="cur"':'')+' sp="'+k_s+'"><em>'+gn.substr(0,5)+'</em><cite>胜</cite></span>';
							}else{
								if (close.substr(0,1)=='-')
									close='<i class="blue">('+close+')';
								else if(close.substr(0,1)=='0')
									close='';
								else
									close='<i class="red">(+'+close+')';
									
								html += '<span v="3" sp="'+z_s+'"><em>'+hn.substr(0,5)+close+'</i></em><cite>胜</cite></span><span class="spfvs" v="1" sp="'+p+'"><em>VS</em><cite>平</cite></span><span v="0" sp="'+k_s+'"><em>'+gn.substr(0,5)+'</em><cite>胜</cite></span>';
							}
		...
		bind_: function(){
			...
			jQuery('.deleted').Touch(function(){
				...
				localStorage.removeItem('bd_spf_PollNum');
				Bjdc.my_play();
			});
			jQuery('#isOk_').click(function(){
				if(parseInt($('#c_').html())>0){
					...
					localStorage.setItem('bd_spf_SelectNum',c);
					localStorage.removeItem('bd_spf_PollNum');
					window.location.href='#class=url&xo=bjdc/ture.html&pid='+$('#expect').val();
				}else{
					D.tx('请至少选择1场比赛');
				}
			});
