...
var Bjdc_bf_t = {
		initial: function(){
			var html = localStorage.getItem('bd_bf_SelectNum');
		...
		bind: function(){
			...
			jQuery('.bf_').find('a:eq(0)').click(function(){//取消比分层
				...
				Bjdc_bf_t.public_bf($(this).parent().parent());
			});
			...
			jQuery('.bf_ div.competitions span').click(function(){//选择比分
				...
				Bjdc_bf_t.public_bf($(this).parent().parent(),bl);
			});
			...
			jQuery('#tjss').click(function(){//添加赛事
				Bjdc_bf_t.href_('#class=url&xo=bjdc/bf/index.html');
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c == '0'){
					...
					Bjdc_bf_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid+'&pid='+pid);
				}
			...
			jQuery('#payment').click(function(){//代购
					...
					if(c == '0'){
						...
						var bs = $('#bs_').val();
						var gid = $('#lotid').val();
						Bjdc_bf_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid+'&pid='+pid);
					}
				});
		...
		local_: function(){//bd_bf_SelectNum重新赋值
		...
		href_: function(u){
			...
			localStorage.setItem('bd_bf_PollNum',c);
			location.href=u;
		},
