 ...
 * Date: 2014-7-22
 */
var PollNum = localStorage.getItem('jclq_sf_PollNum');
var Jclq_sf = {
		initial: function(){
			Jclq_sf.add_C();//初始化添加对阵内容
			
			Jclq_sf.bind_();//初始化的一些绑定事件
		},
		add_C: function(){
			$('.loading').hide();
			D.load();
			$.ajax({
				url:'/data/app/jclq/sf.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					...
					rs.each(function(aa){
						...
						var r = $(this).find('row');
						html += '<section class="jclqVs"><div class="sfcTitle">'+addesc+'&nbsp;<label>'+r.length+'</label>场比赛';
						...
						r.each(function(){
							...
							if((1 & isale) > 0){
								if(PollNum && PollNum_t.indexOf(itemid)>=0){
									...
									pc = pc.split('/');
									html += '<ul class="sfcxs" t="'+itemid+'" p="'+sf+'" v="y" c="'+pc+'">';
								}else{
									html += '<ul class="sfcxs" t="'+itemid+'" p="'+sf+'">';
								}
								...
								if(PollNum && PollNum_t.indexOf(itemid)>=0){
									var pc = PollNum_c[PollNum_t.indexOf(itemid)];
									html += '<span v="0" sp="'+k_sf+'" '+(pc.indexOf('0')>=0?'class="cur"':'')+'><em>'+gn+'</em><cite>胜</cite></span><b>VS</b><span v="3" sp="'+z_sf+'" '+(pc.indexOf('3')>=0?'class="cur"':'')+'><em>'+hn+'</em><cite>胜</cite></span>';
								}else{
									html += '<span v="0" sp="'+k_sf+'"><em>'+gn+'</em><cite>胜</cite></span><b>VS</b><span v="3" sp="'+z_sf+'"><em>'+hn+'</em><cite>胜</cite></span>';
								}
					...
					Jclq_sf.my_play();
					Jclq_sf.bind_delay();//必须有对阵之后的绑定事件
				}
			});
		...
		bind_: function(){
			...
			jQuery('#isOk_').click(function(){
				if(parseInt($('#c_').html())>1){
					...
					localStorage.setItem('jclq_sf_SelectNum',c);
					localStorage.removeItem('jclq_sf_PollNum');
					window.location.href='#class=url&xo=jclq/sf/ture.html';
				}else{
					D.tx('请至少选择2场比赛');
				}
			});
