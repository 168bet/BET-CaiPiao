...
var Jclq_t = {
		initial: function(){
			...
			var html = localStorage.getItem('jclq_rfsf_SelectNum');
		...
		bind: function(){
			...
			//	window.location.href='#class=url&xo=jclq/index.html';
				Jclq_t.href_("#class=url&xo=jclq/index.html");
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c < 2){
					...
					Jclq_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid);
				}
			...
			jQuery('#payment').click(function(){//代购
					...
					if(c < 2){
						...
						var bs = $('#bs_').val();
						var gid = $('#lotid').val();
						Jclq_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid);
					}
		...
		href_: function(u){
			...
			localStorage.setItem('jclq_rfsf_PollNum',c);
			location.href=u;
		},
		local_: function(){//jclq_rfsf_SelectNum重新赋值
