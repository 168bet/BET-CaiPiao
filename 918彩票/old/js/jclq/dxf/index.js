 ...
 * Date: 2014-7-22
 */
var PollNum = localStorage.getItem('jclq_dxf_PollNum');
var Jclq_dxf = {
		initial: function(){
			Jclq_dxf.add_C();//初始化添加对阵内容
			
			Jclq_dxf.bind_();//初始化的一些绑定事件
		},
		add_C: function(){
			$('.loading').hide();
			D.load();
			$.ajax({
				url:'/data/app/jclq/dxf.xml',
				type:'GET',
				DataType:'XML',
				success: function(xml){
					...
					rs.each(function(aa){
						...
						var r = $(this).find('row');
						html += '<section class="jclqDxf"><div class="sfcTitle">'+addesc+'&nbsp;'+r.length+'场比赛';
						...
						r.each(function(){
							...
							if((8 & isale) > 0){
								if(PollNum && PollNum_t.indexOf(itemid)>=0){
									...
									pc = pc.split('/');
									html += '<ul class="sfcxs" t="'+itemid+'" p="'+dxf+'" v="y" c="'+pc+'">';
								}else{
									html += '<ul class="sfcxs" t="'+itemid+'" p="'+dxf+'">';
								}
								html += '<li><em>'+name.substr(-3)+'</em><p style=\'color:'+cl+'\'>'+mname+'</p><cite>'+end_time+' 截止</cite></li><li>';
								html += '<p class="spfzpkNum"><span>'+gn+'</span><span class="spfvs">VS</span><span>'+hn+'</span></p>';
								html += '<p class="spfzpk">';
								
								if(PollNum && PollNum_t.indexOf(itemid)>=0){
									var pc = PollNum_c[PollNum_t.indexOf(itemid)];
									html += '<span v="3" sp="'+k_sf+'" '+(pc.indexOf('3')>=0?'class="cur"':'')+'>总分&gt;'+zclose+'</span><span v="0" sp="'+z_sf+'" '+(pc.indexOf('0')>=0?'class="cur"':'')+'>总分&lt;'+zclose+'</span>';
								}else{
									html += '<span v="3" sp="'+k_sf+'">总分&gt;'+zclose+'</span><span v="0" sp="'+z_sf+'">总分&lt;'+zclose+'</span>';
								}
					...
					Jclq_dxf.my_play();
					Jclq_dxf.bind_delay();//必须有对阵之后的绑定事件
				}
			});
			...
			jQuery('#isOk_').click(function(){
				if(parseInt($('#c_').html())>1){
					...
					jQuery('#content_ section').find('div:eq(1) ul[v=y]').each(function(){
						var n = $(this).find('li:eq(1) p:eq(0)').html();
						var dz = $(this).find('li:eq(1) p:eq(1)').html();
						var sp = $(this).find('li:eq(1) p:eq(2)').html();
						var code = $(this).attr('c');
						
		            
		                c += '<li t="'+$(this).attr('t')+'" c="'+code+'" v="y">';
		                c += '<cite class="errorBg"><em class="error2"></em></cite>';
		                c += '<div class="spfzpkNum">'+n+'</div>';
		                c += '<div class="spfzpk">'+dz+'</div>';
					...
					localStorage.removeItem('jclq_dxf_PollNum');
					window.location.href='#class=url&xo=jclq/dxf/ture.html';
				}else{
					D.tx('请至少选择2场比赛');
				}
			});
