...
var Jczq_rq_t = {
		initial: function(){
			var html = localStorage.getItem('rqspf_SelectNum');
		...
		bind: function(){
			...
			jQuery('#tjss').click(function(){//添加赛事
				Jczq_rq_t.href_('#class=url&xo=jczq/rqspf/index.html');
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c < 2){
					...
					Jczq_rq_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid);
				}
			...
			jQuery('#payment').click(function(){//代购
				...
				if(c < 2){
					...
					if(c < 2){
						...
						Jczq_rq_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid);
					}
			});
		...
		href_: function(u){
			...
			localStorage.setItem('rqspf_PollNum',c);
			location.href=u;
		},
		local_: function(){
