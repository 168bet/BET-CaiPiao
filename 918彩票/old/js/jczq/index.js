 ...
 * Date: 2014-7-2
 */
var PollNum = localStorage.getItem('spf_PollNum');
var Jczq = {
		initial: function(){
			Jczq.add_C();//初始化添加对阵内容
			Jczq.bind_();//初始化的一些绑定事件
		},
		add_C: function(){
			$('.loading').hide();
			D.load();
...
//							var ic = $(this).attr('ic');//是否取消  1y-0n
							...
							if(PollNum && PollNum_t.indexOf(itemid)>=0){
								...
								html += '<span v="3" sp="'+z_s+'"><em>'+hn.substr(0,5)+'</em><cite>胜</cite></span><span class="spfvs" v="1" sp="'+p+'"><em>VS</em><cite>平</cite></span><span v="0" sp="'+k_s+'"><em>'+gn.substr(0,5)+'</em><cite>胜</cite></span>';
							}
		...
		bind_delay: function(){
			...
			***/
			/***
			$('.li_weige').Touch(function(){//联赛历史战绩
				$(this).find('li:eq(0) i').toggleClass('xzdown');
		...
		bind_: function(){
			...
			jQuery('#isOk_').click(function(){
				if(parseInt($('#c_').html())>1){
					...
					localStorage.removeItem('spf_PollNum');
					window.location.href='#class=url&xo=jczq/ture.html';
				}else{
					D.tx('请至少选择2场比赛');
				}
			});
