...
var Jczq_bqc_t = {
		initial: function(){
			var html = localStorage.getItem('jczq_bqc_SelectNum');
		...
		bind: function(){
			...
			jQuery('.bf_').find('a:eq(0)').click(function(){//取消比分层
				...
				Jczq_bqc_t.public_bf($(this).parent().parent());
			});
			...
			jQuery('.bf_ div.competitions span').click(function(){//选择比分
				...
				Jczq_bqc_t.public_bf($(this).parent().parent(),bl);
			});
			...
			jQuery('#tjss').click(function(){//添加赛事
				Jczq_bqc_t.href_('#class=url&xo=jczq/bqc/index.html');
			});
			jQuery('#fqhm').click(function(){//发起合买
				...
				if(c < 2){
					...
					Jczq_bqc_t.href_('#class=url&xo=trade/buyconfirm.html&zs='+zs+'&bs='+bs+'&money='+m+'&bk=1&gid='+lotid);
				}
			});
			jQuery('#payment').click(function(){//代购
				...
				if(c < 2){
					...
					if(c == '0'){
						...
						Jczq_bqc_t.href_('#class=url&xo=trade/paybd.html&notes='+zs+'&bs='+bs+'&countMoney='+z+'&bk=0&gid='+gid);
					}
			});
		...
		local_: function(){//jczq_bqc_SelectNum重新赋值
		...
		href_: function(u){
			...
			localStorage.setItem('jczq_bqc_PollNum',c);
			location.href=u;
		},
