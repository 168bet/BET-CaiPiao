...
CP.Qlc_Over = function () {
		...
		var c = {
			local : function(){
				var qlcSelectNum = localStorage.getItem('qlc_SelectNum');//永久
				qlcSelectNum && a.list.html(qlcSelectNum);
				c.count_();
				...
				localStorage.setItem('qlc_PollNum',l);
			},count_ : function () {
				var zs = 0;
				...
				var l = '';
				l = a.list.html();
				localStorage.setItem('qlc_SelectNum',l);
			},jixuan_ : function () {
				var h_ = '';
				var kd_ = '-'+$(window).width()+'px';
				var zx = Random(30);
				h_ = zeroStr(zx[3],2)+' '+zeroStr(zx[6],2)+' '+zeroStr(zx[9],2)+' '+zeroStr(zx[13],2)+' '+zeroStr(zx[24],2)+' '+zeroStr(zx[28],2)+' '+zeroStr(zx[17],2);
				h_ = '<p v=\'1\' w=直选  style=\'left:'+kd_+'\'><cite class="errorBg"><em class="error2"></em></cite><span class="revise_ww"><span>'+h_+'</span><b>直选&nbsp;共1注2元</b></span></p>';
				a.list.append(h_);
