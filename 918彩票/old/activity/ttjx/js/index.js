...
var HH = {
		...
		getMessage:function(date){
			$.getScript('http://static.9188.com/zlk/jcsp/jcsp_'+date+'.js',function(data,textStatus,jqxhr) {
			...
			$.ajax({
				 url: "http://static.9188.com/zlk/jcsp/jcsp_"+date+".js",
				 dataType: "script",
				 async:false,
				 type:"post",
				 success: function(t){
		...
		***/
		...
		getMessage:function(date){
			...
			$.getScript('http://static.4g.9188.com/zlk/jcsp/jcsp_'+date+'.js',function(data,textStatus,jqxhr) {
				var data = query();//返回的数据是个方法
				HH.lctj(data.lctj);//临场推荐
				HH.rmss(data.rmss);//热门赛事
				HH.ttwd(data.ttwd);//天天稳胆
		...
		getHistoryDate:function(){
			...
			for(var i=0;i<10;i++){
				...
				if(newTime.getTime()>1412784000000){
					...
					listring += '<li id=\"'+newDate1+'\" onclick=\"HH.loadMessage(' + newDate1 + ')\" onmouseover=\"$(this).addClass(\'cur\');\" onmouseout=\"$(this).removeClass(\'cur\');\">'+ newDate +'</li>';
				}
			}
		...
		showHistoryPeriod:function(){
			...
			$('#listDate li').each(function () {
				...
				$.ajax({
					url:'http://static.4g.9188.com/zlk/jcsp/jcsp_'+id+'.js',
					type:"get",
					processData:false,
					cache:false,
					dataType:"script",
