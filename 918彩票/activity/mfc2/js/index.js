		...
		$.ajax({
			...
			success: function(xml) {
				...
				if(code=="0"){//登录,如果登录了显示：“竞猜记录”层
					...
					//$("#jcjlInfo").html('<p>'+nickid+ '<cite>(累计竞猜<em id="sum">'+sumnums+'</em>注)</cite></p><p>今日可免费选择<cite id="num">'+todaynums+'</cite>注双色球，剩余<em class="red" id="remain">'+remainnums+'</em>注</p><p>签到</p>')
					//$("#jcjlInfo").show();
					g.fg=1
					is_qd();
		...
		$.ajax({
			...
			success: function(xml) {
				...
				if(code==0){
					...
					var balance = userinfo.attr("balance");//金豆信息
					
					$("#golden_info").html(balance);
				}else{
