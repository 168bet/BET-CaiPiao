namespace newball.gs
{
    ...
    public class reportshowend : Page
    {
        ...
        private void GetValue()
        {
            ...
            this.sumCreditSql = "SELECT count(*) FROM ball_order WHERE datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and username = '" + this.username + "' and tzType in (" + MyFunc.GettzType(this.tzType) + ")";
            this.sumCashSql = "SELECT count(*) FROM ball_order WHERE datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and username = '" + this.username + "' and tzType in (" + MyFunc.GettzType(this.tzType) + ")";
        }
