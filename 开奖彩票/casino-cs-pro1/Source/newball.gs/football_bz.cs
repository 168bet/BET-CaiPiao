        ...
        private string SumAllMember()
        {
            ...
            if (reader.Read())
            {
                ...
                if (num3 < 360)
                {
                    ...
                    {
                        reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0 and gdid in (" + text5 + ") and ballid in (230,231) group by tztype order by tztype asc");
                    }
                    else
                    {
                        reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*(1-csgd-cszdl-csdls)),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0 and gdid in (" + text5 + ") and ballid  in (230,231) group by tztype order by tztype desc");
                    }
                    ...
                    {
                        string text7 = text2;
                        text2 = text7 + "<td align=center class=tznumer>" + text3 + "   <br>  <a href='tzinfo.aspx?gameid=" + set.Tables[0].Rows[0]["id"].ToString().Trim() + "&tztype=20&marker=C'>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></a></td> ";
                    }
                    else
                    {
