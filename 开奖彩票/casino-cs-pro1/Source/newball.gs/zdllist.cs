namespace newball.gs
{
    ...
    public class zdllist : Page
    {
        ...
        private void countmem()
        {
            ...
            string sql = "SELECT COUNT(*) FROM agence WHERE classid=3 ";
        ...
        public void DelZdl(string id)
        {
            ...
            {
                DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
                int num = (int) base2.ExecuteScalar("SELECT COUNT(*) FROM agence WHERE zdlid=" + id + " AND classid=4 ");
                if (num > 0)
                {
                    base2.Dispose();
                ...
                else if (base2.ExecuteNonQuery("DELETE FROM agence WHERE userid=" + id + " AND classid=3") > 0)
                {
                    base2.ExecuteNonQuery("UPDATE agence SET memcount=memcount-1 WHERE userid=" + id);
                    base2.Dispose();
        ...
        {
            return (this.OpItem(flag, zid) + "/<a href=zdlmsg.aspx?id=" + zid + ">详情</a>/<a href=zdlset.aspx?id=" + zid + "&gid=" + this.DropDownListGd.SelectedValue.Trim() + ">设定</a>");
        }
        ...
        public string OpItem(string flag, string id)
        {
            if (flag.Trim().ToUpper() == "TRUE")
            {
                return ("<a href=zdllist.aspx?action=Close&id=" + id + "&flag=" + this.DropDownListAccountStatus.SelectedValue + "&gdid=" + this.DropDownListGd.SelectedValue + " onClick=\"return confirm('确定要停用总代理吗?')\">停用</a>");
            }
            return ("<a href=zdllist.aspx?action=Open&id=" + id + "&flag=" + this.DropDownListAccountStatus.SelectedValue + "&gdid=" + this.DropDownListGd.SelectedValue + " onClick=\"return confirm('确定要启用总代理吗?')\">启用</a>");
        }
        ...
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            string sql = "SELECT COUNT(1) FROM agence WHERE classid=3 ";
            if (this.DropDownListGd.SelectedValue.Trim() != "")
            {
                sql = sql + " AND gdid=" + this.DropDownListGd.SelectedValue;
            }
