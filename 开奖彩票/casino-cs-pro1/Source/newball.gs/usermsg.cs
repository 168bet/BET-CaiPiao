    ...
    public class usermsg : Page
    {
        ...
        private void ButtonSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string text10 = this.DropDownListRate.SelectedItem.Text;
                string selectedValue = this.DropDownListRate.SelectedValue;
                string text12 = this.DropDownListPlType.SelectedValue;
                string text13 = this.RadioButton1.Checked ? "信用" : "现金";
                string text14 = this.DropDownListABC.SelectedValue;
                if ((((text2 == "") || (input == "")) || ((text8 == "") || (s == ""))) || (((text12 == "") || (text13 == "")) || (((text14 == "") || (text10 == "")) || (selectedValue == ""))))
                {
                ...
                else
                {
                    ...
                    {
                        MyFunc.showmsg("会员商名称不能大于8个字符(4个汉字)");
                        base.Response.End();
                    }
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (num >= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (text14.ToString().Trim().ToUpper() != base2.ExecuteScalar("select abc from member where userid ='" + text2 + "'").ToString().Trim().ToUpper())
                                        {
                                            base2.ExecuteNonQuery("DELETE FROM userhs where userid = '" + text2 + "'");
                                            base2.ExecuteNonQuery("insert userhs select  '" + text2.ToString().Trim() + "',w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15,l1,l2,l3,l4,l5,l6,l7,l8,l9,l10,l11,l12,l13,l14,l15,maxC1,maxC2,maxC3,maxC4,maxC5,maxC6,maxC7,maxC8,maxC9,maxC10,maxC11,maxC12,maxC13,maxC14,maxC15,maxZ1,maxZ2,maxZ3,maxZ4,maxZ5,maxZ6,maxZ7,maxZ8,maxZ9,maxZ10,maxZ11,maxZ12,maxZ13,maxZ14,maxZ15,W28,L28,maxC28,maxZ28,maxZ18,maxZ19,maxZ20,maxZ21,maxZ22,maxZ23,maxZ24,maxC18,maxC19,maxC20,maxC21,maxC22,maxC23,maxC24,w18,w19,w20,w21,w22,w23,w24,l18,l19,l20,l21,l22,l23,l24 from hs join agence on hs.userid = agence.userid where hs.userid='" + text9 + "' and hs.type='" + text14 + "'");
                                        }
                                        if (base2.ExecuteNonQuery("UPDATE member SET truename='" + input + "'" + text + ",tel='" + text6 + "',usemoney=" + s + ",moneysort='" + text10 + "',moneyrate=" + selectedValue + ",pltype='" + text12 + "',usertype='" + text13 + "',abc='" + text14 + "' WHERE userid=" + text2) > 0)
                                        {
                                            base2.Dispose();
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                if ((base.Request.QueryString["id"] != null) && (base.Request.QueryString["id"].ToString().Trim() != ""))
                {
                    ...
                    SqlDataReader reader2 = db.ExecuteReader("SELECT userid,username,userpass,zdlid,zdlname,dlsid,dlsname,gdid,gdname,usemoney,moneysort,usertype,pltype,tel,truename,abc FROM member WHERE userid=" + text);
                    if (!reader2.Read())
                    {
                        reader2.Close();
                        db.Dispose();
                    ...
                    this.LabelGd.Text = reader2["gdname"].ToString().Trim();
                    this.LabelZdl.Text = reader2["zdlname"].ToString().Trim();
                    this.LabelDls.Text = reader2["dlsname"].ToString().Trim();
                    this.TextBoxGdid.Text = reader2["gdid"].ToString().Trim();
                    this.TextBoxZdlid.Text = reader2["zdlid"].ToString().Trim();
