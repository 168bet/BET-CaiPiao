namespace newball.gs
{
    ...
    public class gdlist : Page
    {
        ...
        {
            string sql = "SELECT userid,username,memcount,truename,regtime,isuseable,usemoney,gdid FROM agence WHERE classid=2 AND gdid = '" + this.Session["adminuserid"].ToString() + "'";
            if (flag != "")
            {
        ...
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            string sql = "SELECT COUNT(1) FROM agence WHERE classid=2 AND gdid = '" + this.Session["adminuserid"].ToString() + "'";
            if (flag != "")
            {
                sql = sql + "AND isuseable=" + flag;
            }
