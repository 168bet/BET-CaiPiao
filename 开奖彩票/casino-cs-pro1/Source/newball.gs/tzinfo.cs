namespace newball.gs
{
    ...
    public class tzinfo : Page
    {
        ...
        private string GetSqlStr()
        {
            ...
            {
                return ("SELECT (1-csdls-cszdl-csgd) as csgd,tzmoney*(1-csdls-cszdl-csgd) as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE tztype='" + this.tzType.Value + "'  AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT (1-csdls-cszdl-csgd) as csgd,tzmoney*(1-csdls-cszdl-csgd) as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE tzteam like '%" + this.ballid.Value + "%'  AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT (1-csdls-cszdl-csgd) as csgd,tzmoney*(1-csdls-cszdl-csgd) as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE ballid in (" + this.ballid.Value + ")  AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
        ...
        private string GetSumSqlStr()
        {
            if (this.tzType.Value == "20")
            {
                return ("SELECT count(*) FROM ball_order WHERE tztype='" + this.tzType.Value + "' AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0 ");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT count(*) FROM ball_order WHERE tzteam like '" + this.ballid.Value + "' AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT count(*) FROM ball_order WHERE ballid in (" + this.ballid.Value + ") AND gdid in (" + this.Session["adminarrgd"].ToString() + "-1) and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
