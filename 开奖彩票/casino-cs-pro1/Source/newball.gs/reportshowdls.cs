namespace newball.gs
{
    ...
    public class reportshowdls : Page
    {
        ...
        private void Deal(string sql)
        {
            ...
            text = text + "<tr class=dlsreport><td width=8%>代理商</td><td width=5%>笔数</td><td width=14%>会员投注金额</td><td width=13%>会员</td><td width=13%>代理商金额</td><td width=13%>代理商</td><td width=4%>成数</td><td width=13%>总代理金额</td><td width=13%>总代理</td><td width=4%>成数</td></tr>\n";
            while (reader.Read())
            {
                num++;
