namespace newball.gs
{
    ...
    public class football_rqgg : Page
    {
        ...
        private string SumAllMember()
        {
            ...
            {
                string text5 = sql;
                sql = text5 + " and (select count(*) from " + connStr + "..ball_order where datediff(day,updatetime,getdate()) = 0 and charindex(ball_pl7view.team1,content)>0 and charindex(ball_pl7view.team2,content)>0 and charindex(ball_pl7view.matchname,content)>0 and tztype = 8 and gdid in (" + this.Session["adminarrgd"].ToString() + "-1)) > 0";
            }
            ...
            while (reader.Read())
            {
                ...
                text2 = text7 + "<td align=center><a href='tzinfo.aspx?gameid=" + reader["ballid"].ToString().Trim() + "&tztype=8&marker=none'>" + MyFunc.SumStaticGs(reader["ballid"].ToString().Trim(), "8", "none") + "</a></td></tr>";
            }
            if (num == 0)
            {
