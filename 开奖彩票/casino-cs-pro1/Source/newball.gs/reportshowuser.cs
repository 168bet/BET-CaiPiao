namespace newball.gs
{
    ...
    public class reportshowuser : Page
    {
        ...
        private void Deal(string sql)
        {
            ...
            text = text + "<tr class=dlsreport><td width=10%>会员</td><td width=5%>笔数</td><td width=20%>金额</td><td width=20%>会员</td><td width=20%>代理商金额</td><td width=20%>代理商</td><td width=5%>成数</td></tr>\n";
            while (reader.Read())
            {
                num++;
