            ...
            if (reader.Read())
            {
                ...
                if (num3 < 360)
                {
                    ...
                    for (int i = 0; i < 7; i++)
                    {
                        ...
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0 and gdid in (" + text6 + ") and ballid=" + set.Tables[0].Rows[num]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
