namespace newball.dls
{
    ...
    public class reportShowNoJs : Page
    {
        ...
        private void Deal(string flag)
        {
            ...
            if (flag == "all")
            {
                sql = "SELECT od.csdls,od.tzmoney*od.csdls as csres,convert(numeric(5,2),od.hsuser_w) as hsuser_w,convert(numeric(5,2),od.hsuser_l) as hsuser_l,dbo.GettzType_Func(od.tzType) as tzTypeName,od.tzType as tzType,od.updatetime as updatetime,od.username as username,od.content as content,od.tzmoney as tzmoney,isnull((od.win-od.lose),0) as result,od.abc as abc,od.iscancel as iscancel FROM ball_order as od inner join member as mb on od.userid = mb.userid where od.isjs = 0 and od.iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',od.updatetime) = 0 and od.dlsid='" + this.Session["adminuserid"].ToString() + "'";
                text3 = "select count(*) from ball_order where isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and dlsid='" + this.Session["adminuserid"].ToString() + "'";
            }
            else
            {
                ...
                text3 = "select count(*) from ball_order where tztype='1' AND isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and dlsid='" + this.Session["adminuserid"].ToString() + "'";
            }
