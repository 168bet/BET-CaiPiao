    ...
    public class index : Page
    {
        ...
        private void ButtonLogin_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                reader = db.ExecuteReader("SELECT userid,username,userpass,classid,gdid,zdlid,islock FROM agence ag WHERE username='" + text + "' AND classid=4 AND isuseable = 1 and (select top 1 gdid from agence where userid = ag.gdid) in (" + MyFunc.GetGongSiID().ToString() + ")");
                if (!reader.Read())
                {
                    reader.Close();
                    db.Dispose();
                    MyFunc.showmsg("网络繁忙,请稍后登陆.....");
                    base.Response.End();
                }
                ...
                else
                {
                    ...
                    this.Session["pre_id"] = reader["zdlid"].ToString().Trim();
