    ...
    public class myReportForm : Page
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.Page.IsPostBack)
            {
                ...
                reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and dlsid='" + this.Session["adminuserid"].ToString() + "'");
                reader.Read();
                ...
                {
                    this.showTime2.Text = "[" + DateTime.Today.ToString("yyyy-MM-dd") + "]";
                    this.showNo.Text = "[<a href='reportShowNoJs.aspx?tztype=all'><font color=red>" + reader["rowcounts"].ToString() + "注注单未结算</font></a>]";
                }
                reader.Close();
                reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where tzType = '8' AND isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and dlsid='" + this.Session["adminuserid"].ToString() + "'");
                reader.Read();
                if (reader["rowcounts"].ToString() != "0")
                {
