    ...
    public class useradd : Page
    {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (num >= 0)
                            {
                                ...
                                {
                                    int num5 = (int) base2.ExecuteScalar("SELECT COUNT(1) FROM member WHERE username='" + input + "'");
                                    if (num5 > 0)
                                    {
                                        base2.Dispose();
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                DataBase db = new DataBase(MyFunc.GetConnStr(2));
                SqlDataReader reader = db.ExecuteReader("SELECT gdid,gdname FROM agence WHERE userid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                reader.Read();
                this.TextBoxGdid.Text = reader["gdid"].ToString().Trim();
                this.TextBoxGdName.Text = reader["gdname"].ToString().Trim();
                reader.Close();
                SqlDataReader reader2 = db.ExecuteReader("SELECT userid,username FROM agence WHERE zdlid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                ...
                if (this.DropDownListDls.Items.Count < 1)
                {
                    db.Dispose();
                    MyFunc.showmsg("您还没有代理商,请选添加代理商");
                    base.Response.End();
                }
                else
                {
