namespace newball.zdl
{
    ...
    public class tzinfo1 : Page
    {
        ...
        private string GetSqlStr()
        {
            ...
            {
                return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order2 WHERE tztype='" + this.tzType.Value + "'  AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order2 WHERE tzteam like '%" + this.ballid.Value + "%'  AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order2 WHERE ballid in (" + this.ballid.Value + ")  AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
        ...
        private string GetSumSqlStr()
        {
            if (this.tzType.Value == "20")
            {
                return ("SELECT count(*) FROM ball_order2 WHERE tztype='" + this.tzType.Value + "' AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT count(*) FROM ball_order2 WHERE tzteam like '" + this.ballid.Value + "' AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT count(*) FROM ball_order2 WHERE ballid in (" + this.ballid.Value + ") AND dlsid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack && (base.Request.QueryString["gameid"] != null))
            {
