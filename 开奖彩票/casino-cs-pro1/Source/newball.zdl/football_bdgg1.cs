namespace newball.zdl
{
    ...
    public class football_bdgg1 : Page
    {
        ...
        {
            string s = "<font style='FONT-SIZE:9pt'>&nbsp;&nbsp;波胆过关</font>\n<table id='m_tab' border='0' cellspacing='1' cellpadding='2' width='510' class='tableBorder1'>\n<tr align='center' class='dlsheader' height=25 >\n";
            s = s + "<td width='60'nowrap>时间</td><td width='130'nowrap>联塞</td><td width='70'>主队/客队</td><td width='150'>波胆过关</td></tr>" + this.SumAllMember();
        ...
        private string SumAllMember()
        {
            ...
            {
                string text5 = sql;
                sql = text5 + " and (select count(*) from " + connStr + "..ball_order where datediff(day,updatetime,getdate()) = 0 and charindex(ball_pl4view.team1,content)>0 and charindex(ball_pl4view.team2,content)>0 and charindex(ball_pl4view.matchname,content)>0 and tztype = 16 and zdlid='" + this.Session["adminuserid"].ToString() + "') > 0";
            }
            ...
            while (reader.Read())
            {
                ...
                text2 = text7 + "<td align=center><a href='tzinfo.aspx?gameid=" + reader["ballid"].ToString().Trim() + "&tztype=16&marker=none'>" + MyFunc.SumStaticZdl(reader["ballid"].ToString().Trim(), "16", "none") + "</a></td></tr>";
            }
            if (num == 0)
            {
