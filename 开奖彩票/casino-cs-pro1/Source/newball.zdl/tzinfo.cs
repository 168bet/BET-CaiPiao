namespace newball.zdl
{
    ...
    public class tzinfo : Page
    {
        ...
        private string GetSqlStr()
        {
            ...
            {
                return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE tztype='" + this.tzType.Value + "'  AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE tzteam like '%" + this.ballid.Value + "%'  AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT cszdl,tzmoney*cszdl as csres,hsuser_w,hsuser_l,abc,updatetime,username,tzType,content,tzmoney,win-lose as result FROM ball_order WHERE ballid in (" + this.ballid.Value + ")  AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
        ...
        private string GetSumSqlStr()
        {
            if (this.tzType.Value == "20")
            {
                return ("SELECT count(*) FROM ball_order WHERE tztype='" + this.tzType.Value + "' AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            if (this.tzType.Value == "16")
            {
                return ("SELECT count(*) FROM ball_order WHERE tzteam like '" + this.ballid.Value + "' AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
            }
            return ("SELECT count(*) FROM ball_order WHERE ballid in (" + this.ballid.Value + ") AND zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and DATEDIFF(dd, GETDATE(), updatetime)=0");
        }
