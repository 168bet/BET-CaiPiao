namespace newball.zdl
{
    ...
    public class dlsadd : Page
    {
        ...
        protected TextBox TextBoxGdName;
        ...
        protected HtmlInputHidden TextBoxZdlName;
                        ...
                        try
                        {
                            if (int.Parse(s) >= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    else if (int.Parse(base2.ExecuteScalar("SELECT COUNT(*) FROM agence WHERE username='" + input + "'").ToString()) > 0)
                                    {
                                        base2.Dispose();
        ...
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            int num = int.Parse(base2.ExecuteScalar("SELECT gdbl FROM agence WHERE userid=" + id).ToString());
            base2.Dispose();
            return num;
        }
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.TextBoxZdlName.Value = this.Session.Contents["adminusername"].ToString().Trim();
                this.LabelZdl.Text = this.Session.Contents["adminusername"].ToString().Trim();
                DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
                SqlDataReader reader = base2.ExecuteReader("SELECT gdid,gdname FROM agence WHERE userid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                reader.Read();
                this.TextBoxGdid.Text = reader["gdid"].ToString().Trim();
                this.TextBoxGdName.Text = reader["gdname"].ToString().Trim();
