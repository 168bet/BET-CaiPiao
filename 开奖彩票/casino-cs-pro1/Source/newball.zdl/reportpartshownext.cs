namespace newball.zdl
{
    ...
    public class reportpartshownext : Page
    {
        ...
        private void GetValue()
        {
            ...
            this.creditSql = "SELECT od.cszdl,od.tzmoney*od.cszdl as csres,convert(numeric(5,2),od.hsuser_w) as hsuser_w,convert(numeric(5,2),od.hsuser_l) as hsuser_l,dbo.GettzType_Func(od.tzType) as tzTypeName,od.tzType as tzType,od.updatetime as updatetime,od.username as username,od.content as content,od.tzmoney as tzmoney,isnull((od.win-od.lose),0) as result,od.abc as abc,od.iscancel as iscancel,od.isdanger as isdanger FROM ball_order as od inner join member as mb on od.userid = mb.userid where mb.usertype = '信用' and mb.zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and od.zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and datediff(s,'" + this.startTime + "',od.balltime) > 0 and datediff(d,od.balltime,'" + this.endTime + "') >= 0 and od.tzType in (" + this.tzType + ")";
            this.cashSql = "SELECT od.cszdl,od.tzmoney*od.cszdl as csres,convert(numeric(5,2),od.hsuser_w) as hsuser_w,convert(numeric(5,2),od.hsuser_l) as hsuser_l,dbo.GettzType_Func(od.tzType) as tzTypeName,od.tzType as tzType,od.updatetime as updatetime,od.username as username,od.content as content,od.tzmoney as tzmoney,isnull((od.win-od.lose),0) as result,od.abc as abc,od.iscancel as iscancel,od.isdanger as isdanger FROM ball_order as od inner join member as mb on od.userid = mb.userid where mb.usertype = '现金' and mb.zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and od.zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and datediff(s,'" + this.startTime + "',od.balltime) > 0 and datediff(d,od.balltime,'" + this.endTime + "') >= 0 and od.tzType in (" + this.tzType + ")";
            this.sumCreditSql = "SELECT count(*) FROM ball_order WHERE userid in (select userid from member where usertype='信用' and zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "') and zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + this.tzType + ")";
            this.sumCashSql = "SELECT count(*) FROM ball_order WHERE userid in (select userid from member where usertype='现金' and zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "') and zdlid = '" + this.Session.Contents["adminuserid"].ToString() + "' and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + this.tzType + ")";
        }
