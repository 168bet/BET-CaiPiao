namespace newball.zdl
{
    ...
    public class football_guodan : Page
    {
        protected double abcadd = 0;
        public string sql1 = "";
        public string sTime = "N";
        public string time_reload;
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            if (int.Parse(base2.ExecuteScalar("select count(*) from ball_order2 where  dxc=1 and DATEDIFF(dd, GETDATE(), updatetime)=0 and userid=" + this.Session.Contents["adminuserid"]).ToString()) > 0)
            {
        ...
        Label_01DC:
            ...
            if (reader.Read())
            {
                ...
                int num4 = ((((span.Days * 0x15180) + (span.Hours * 0xe10)) + (span.Minutes * 60)) + span.Seconds) + 40;
                if (num4 < 1)
                {
            ...
            if (num3 == 0)
            {
                string sql = "select ballid,min(curpl) as curpl,isnull(sum(tzmoney),0) as tzmoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 and tztype=8 and dlsid=" + this.Session.Contents["adminuserid"] + " group by ballid";
                reader = base2.ExecuteReader(sql);
                while (reader.Read())
                {
        ...
        {
            string s = "<font style='FONT-SIZE:9pt'>&nbsp;&nbsp;特别号</font>\n<form name=topMenu id=topMenu method=post runat=server><table id=glist_table border=0 cellspacing=1 cellpadding=0 class=ra_listbet_tab width=850>\n  <tr class=ra_listbet_title><td width=40>时间</td><td width=50>期数</td><td >号码</td>\n";
        ...
        private void saveOrder(string type, string ballid, string team, string money, string cstype)
        {
            ...
            else
            {
                ...
                reader.Close();
                sql = "INSERT INTO ball_order2(orderid,ballid,tztype,tzmoney,curpl,tzteam,rqteam,rqc,zdbf,content,userid,username,gdid,zdlid,dlsid,hszdl_w,hsdls_w,hsuser_w,hszdl_l,hsdls_l,hsuser_l,hsgd_w,hsgd_l,csgd,cszdl,csdls,tzip,updatetime,moneyrate,balltime,abc,isdanger,zdmoney,ds,dxc)VALUES(" + text + "," + ballid + "," + type + "," + text9 + "," + textArray[1] + ",'" + team + "','" + textArray[2] + "',1,'" + cstype + "','" + textArray[4] + "'," + text10 + ",'" + text11 + "'," + text20 + "," + text21 + "," + this.Session.Contents["pre_id"].ToString() + "," + text25 + "," + text27 + "," + text29 + "," + text26 + "," + text28 + "," + text30 + "," + text34 + "," + text35 + "," + text31 + "," + text32 + "," + text33 + ",'" + text19 + "',GetDate()," + text15 + ",'" + textArray[8] + "','" + text17 + "',0," + text9 + ",'" + base.Request.Form["amount"].ToString() + "','1');";
                text7 = textArray[4];
        ...
        private string SumAllMember()
        {
            ...
            if (reader.Read())
            {
                ...
                if (num4 < 360)
                {
                    ...
                    this.sTime = ((((span2.Hours * 0x5a0) + (span2.Minutes * 60)) + span2.Seconds) + 40).ToString();
                    object obj2 = text2;
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 6]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 6]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                        }
                        ...
                        string text7 = text2;
                        text2 = text7 + "<td align=center background=" + MyFunc.GetRGB(textArray[index]) + " class=nums><font color=" + MyFunc.GetRGB(int.Parse(textArray[index])) + "><b>" + textArray[index++] + "</b></font></td>";
                        if (reader2.Read())
                        {
                            string text8 = text2;
                            text2 = text8 + "<td align=center bgcolor=#FFFFFF ><font color=#CC0000>" + text3 + "</font>   <br>  <a href='tzinfo1.aspx?gameid=" + set.Tables[0].Rows[i + 6]["id"].ToString().Trim() + "&tztype=8&marker=C'><span class=tznumer>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></span></a></td> ";
                        ...
                        if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x10]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x10]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                        }
                        ...
                        {
                            string text10 = text2;
                            text2 = text10 + "<td align=center bgcolor=#FFFFFF><font color=#CC0000>" + text3 + "</font>   <br>  <a href='tzinfo1.aspx?gameid=" + set.Tables[0].Rows[i + 0x10]["id"].ToString().Trim() + "&tztype=8&marker=C'>  <span class=tznumer><font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</span></font></a></td> ";
                            num3 += double.Parse(reader2["summoney"].ToString());
                        }
                        else
                        {
                        ...
                        if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x1a]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x1a]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                        }
                        ...
                        {
                            string text12 = text2;
                            text2 = text12 + "<td align=center bgcolor=#FFFFFF ><font color=#CC0000>" + text3 + "</font>   <br>  <a href='tzinfo1.aspx?gameid=" + set.Tables[0].Rows[i + 0x1a]["id"].ToString().Trim() + "&tztype=8&marker=C'>  <span class=tznumer><font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></span></a></td> ";
                            num3 += double.Parse(reader2["summoney"].ToString());
                        }
                        else
                        {
                        ...
                        if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x24]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x24]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                        }
                        ...
                        {
                            string text14 = text2;
                            text2 = text14 + "<td align=center bgcolor=#FFFFFF ><font color=#CC0000>" + text3 + "</font>   <br>  <a href='tzinfo1.aspx?gameid=" + set.Tables[0].Rows[i + 0x24]["id"].ToString().Trim() + "&tztype=8&marker=C'>  <span class=tznumer><font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></span></a></td> ";
                            num3 += double.Parse(reader2["summoney"].ToString());
                        }
                        else
                        {
                        ...
                        if (i < 9)
                        {
                            if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x2e]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                            }
                            else
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i + 0x2e]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                            }
                            ...
                            {
                                string text16 = text2;
                                text2 = text16 + "<td align=center bgcolor=#FFFFFF ><font color=#CC0000>" + text3 + "</font>   <br>  <a href='tzinfo1.aspx?gameid=" + set.Tables[0].Rows[i + 0x2e]["id"].ToString().Trim() + "&tztype=8&marker=C'>  <span class=tznumer><font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></span></a></td> ";
                                num3 += double.Parse(reader2["summoney"].ToString());
                            }
                            else
                            {
                        ...
                        if (i < 6)
                        {
                            if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                            }
                            else
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csdls),0) as summoney from ball_order2 where DATEDIFF(dd, GETDATE(), updatetime)=0 " + text5 + "  and ballid=" + set.Tables[0].Rows[i]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                            }
                    ...
                    text2 = string.Concat(textArray24);
                    if (this.Session.Contents["adminclassid"].ToString() == "2")
                    {
                        text2 = text2 + "<tr bgColor=#ffffff height=25><TD height=30 colSpan=11>";
                        try
                        {
                            index = int.Parse(base2.ExecuteScalar("select top 1 isnull(ds,0) from ball_order2 where dxc=1 and DATEDIFF(dd, GETDATE(), updatetime)=0 and userid=" + this.Session.Contents["adminuserid"]).ToString());
                        }
                        catch
                        {
                            index = 0;
                        ...
                        text2 = text2 + "</TD><td colspan=3><table bgcolor=ffffff><tr><TD id=closeM align=right ><FONT color=red>即将封盘</FONT> 倒数:" + "<SPAN id=close_msg>&nbsp;</SPAN>秒</TD></tr></table></form>";
                        this.time_reload = "time_reload();";
                    }
                    else
                    {
                        text2 = text2 + "<table><tr><TD id=closeM align=right >" + "<SPAN id=close_msg>&nbsp;</SPAN></TD></tr></table></form>";
                    }
                }
                else
                {
        ...
        private void TopMenuProcess()
        {
            ...
            {
                text = base.Request.Form["ltype"].ToString().Trim();
                this.Session["football_b_ltype"] = text;
            }
            else if (this.Session["football_b_ltype"] != null)
            {
                text = this.Session["football_b_ltype"].ToString().Trim();
            }
            if (base.Request.Form["retime"] != null)
            {
                text2 = base.Request.Form["retime"].ToString().Trim();
                this.Session["football_b_retime"] = text2;
            }
            else if (this.Session["football_b_retime"] != null)
            {
                text2 = this.Session["football_b_retime"].ToString().Trim();
            }
            if (text.ToUpper() == "A")
            {
                this.abcadd = 0.01;
