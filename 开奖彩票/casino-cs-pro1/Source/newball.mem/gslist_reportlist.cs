namespace newball.mem
{
    ...
    public class gslist_reportlist : Page
    {
        ...
        private void ShowMonthContent()
        {
            ...
            while (reader.Read())
            {
                text3 = text3 + reader["userid"].ToString().Trim() + ",";
            }
            reader.Close();
            text3 = text3 + "0";
            for (int i = 0; i < 30; i++)
            {
                sql = "SELECT isnull(sum(mgd),0) as gdmoney,isnull(sum(mdls),0) as dlsmoney,isnull(sum(mzdl),0) as zdlmoney,count(*) as tzcount,isnull(sum(tzmoney),0) as tzmoney,isnull(sum(win-lose),0) as result FROM ball_order WHERE gdid in(" + text3 + ") AND datediff(day,updatetime,'" + DateTime.Today.AddDays((double) -i).ToShortDateString() + "') = 0";
