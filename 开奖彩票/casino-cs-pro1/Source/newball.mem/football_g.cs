            ...
            if (reader.Read())
            {
                ...
                if (num3 < 360)
                {
                    ...
                    for (int i = 0; i < 12; i++)
                    {
                        ...
                        string text6 = text2;
                        text2 = text6 + "<td align=center>" + MyFunc.GettwelveName(textArray[index]) + "</td><td align=center>" + MyFunc.GettwelveNum(textArray[index++]) + "</td>";
                        if (reader2.Read())
                        {
                            string text7 = text2;
                            text2 = text7 + "<td align=center class=tznumer>" + text3 + "   <br>  <a href='tzinfo.aspx?gameid=" + set.Tables[0].Rows[i]["id"].ToString().Trim() + "&tztype=18&marker=C'>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></a></td> ";
                        }
                        else
                        {
                        ...
                        if (i < 3)
                        {
                            if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and ballid in (" + set.Tables[0].Rows[i + 12]["id"].ToString().Trim() + "," + set.Tables[0].Rows[i + 0x1b]["id"].ToString().Trim() + ") group by tztype order by tztype asc");
                            }
                            else
                            {
                                reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and ballid in (" + set.Tables[0].Rows[i + 12]["id"].ToString().Trim() + "," + set.Tables[0].Rows[i + 0x1b]["id"].ToString().Trim() + ") group by tztype order by tztype desc");
                            }
                            text3 = double.Parse(MyFunc.GetPlType(set.Tables[0].Rows[i + 12]["pl"].ToString().Trim(), this.Session.Contents["ABC"].ToString().Trim(), set.Tables[0].Rows[i + 0x1a]["give"].ToString().Trim(), "H", "0").ToString("F2")).ToString();
                            text2 = text2 + "<td align=center>" + textArray[index++] + "</td>";
                            if (reader2.Read())
                            {
                                string text8 = text2;
                                text2 = text8 + "<td align=center class=tznumer>" + text3 + "   <br>  <a href='tzinfo.aspx?gameid=" + set.Tables[0].Rows[i + 12]["id"].ToString().Trim() + "," + set.Tables[0].Rows[i + 0x1b]["id"].ToString().Trim() + "&tztype=17&marker=C'>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></a></td> ";
                            }
                            else
                            {
                                text2 = text2 + "<td align=center class=tznumer>" + text3 + "   <br>    <font color=#000088>0</font></a></td> ";
                        ...
                        if (this.Session["sessionSelectStaticCS"].ToString() == "全部")
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and ballid=" + set.Tables[0].Rows[i + 15]["id"].ToString().Trim() + " group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and ballid=" + set.Tables[0].Rows[i + 15]["id"].ToString().Trim() + " group by ballid order by ballid desc");
                        }
                        ...
                        string text9 = text2;
                        text2 = text9 + "<td align=center>" + MyFunc.GettwelveName(textArray[index]) + "</td><td align=center>" + MyFunc.GettwelveNum(textArray[index++]) + "</td>";
                        if (reader2.Read())
                        {
                            string text10 = text2;
                            text2 = text10 + "<td align=center class=tznumer>" + text3 + "   <br>  <a href='tzinfo.aspx?gameid=" + set.Tables[0].Rows[i + 15]["id"].ToString().Trim() + "&tztype=19&marker=C'>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></a></td></tr> ";
                        }
                        else
                        {
