namespace newball.mem
{
    ...
    public class zdllist_reportlist : Page
    {
        ...
        private void ShowMonthContent()
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                sql = "SELECT isnull(sum(mdls),0) as dlsmoney,isnull(sum(mzdl),0) as zdlmoney,count(*) as tzcount,isnull(sum(tzmoney),0) as tzmoney,isnull(sum(win-lose),0) as result FROM ball_order WHERE zdlid = '" + this.userId + "' AND datediff(day,updatetime,'" + DateTime.Today.AddDays((double) -i).ToShortDateString() + "') = 0";
