namespace newball.mem
{
    ...
    public class userlist_reportlist : Page
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!this.Page.IsPostBack)
            {
                ...
                cell2.InnerHtml = cell2.InnerHtml + "&nbsp;&nbsp;管理员：";
                HtmlTableCell cell3 = this.tableHeader.Rows[1].Cells[0];
                cell3.InnerHtml = cell3.InnerHtml + "<b></font color=gray>" + this.Session["adminusername"].ToString() + "</font color=gray></b>";
                HtmlTableCell cell4 = this.tableHeader.Rows[1].Cells[0];
                cell4.InnerHtml = cell4.InnerHtml + "&nbsp;->->&nbsp;<A href='javascript:window.print();'>打印&nbsp;&nbsp;</A><a href='javascript:window.history.back();'>返回</a>";
        ...
        private void ShowMonthContent()
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                sql = "SELECT isnull(sum(mdls),0) as dlsmoney,isnull(sum(mzdl),0) as zdlmoney,count(*) as tzcount,isnull(sum(tzmoney),0) as tzmoney,isnull(sum(win-lose),0) as result FROM ball_order WHERE userid = '" + this.userId + "' AND datediff(day,updatetime,'" + DateTime.Today.AddDays((double) -i).ToShortDateString() + "') = 0";
