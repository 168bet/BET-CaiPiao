namespace newball.mem
{
    ...
    public class userlist_reportcontent : Page
    {
        ...
        private void ShowBallOrderContent()
        {
            ...
            double num3 = 0;
            double num4 = 0;
            sql = "SELECT isnull(1-csdls-cszdl-csgd,0) as cszdl,isnull(tzmoney*(1-csdls-cszdl-csgd),0) as csres,convert(numeric(5,2),hsuser_w) as hsuser_w,convert(numeric(5,2),hsuser_l) as hsuser_l,dbo.GettzType_Func(tzType) as tzTypeName,tzType,updatetime,username,content,tzmoney,isnull((win-lose),0) as result,abc,iscancel FROM ball_order where datediff(day,'" + this.upDateTime + "',updatetime) = 0 and userid='" + this.userId + "'";
