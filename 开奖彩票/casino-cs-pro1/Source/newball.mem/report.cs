        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    SqlDataReader reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0");
                    reader.Read();
                    ...
                    reader.Close();
                    reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where tzType = '8' AND isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0");
                    reader.Read();
                    if (reader["rowcounts"].ToString() != "0")
                    {
