namespace newball.mem
{
    ...
    public class mgrConfig : Page
    {
        ...
        {
            string sql = "INSERT INTO affiche (le,content,updatetime,kaisai,qishu) VALUES ('" + this.le.SelectedValue + "', '" + this.content.Text.Trim().Replace("'", " ") + "', '" + DateTime.Now.ToString() + "', '" + this.kaisai.Text.Trim().Replace("'", " ") + "','" + this.qishu.Text.Trim().Replace("'", " ") + "')";
        ...
        {
            string sql = "UPDATE affiche SET le = '" + this.le.SelectedValue + "',content = '" + this.content.Text.Trim().Replace("'", " ") + "',updatetime = '" + DateTime.Now.ToString() + "',kaisai= '" + this.kaisai.Text.Trim().Replace("'", " ") + "',qishu= '" + this.qishu.Text.Trim().Replace("'", " ") + "'  WHERE id = '" + this.configid.Value + "'";
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            base2.ExecuteNonQuery(sql);
            base2.CloseConnect();
            base2.Dispose();
        ...
        {
            string sql = "SELECT le,content,kaisai,qishu FROM affiche WHERE id='" + varconfigid + "'";
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
