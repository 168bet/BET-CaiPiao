namespace newball.mem
{
    ...
    public class reportshow : Page
    {
        ...
        private void GetValue()
        {
            ...
            this.creditSql = "select gdid,(select username from agence where userid =ball_order.gdid) as gdname,count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mgd) as mgd,sum(mzdl) as mzdl,sum(mdls) as mdls FROM ball_order WHERE userid in (select userid from member where usertype='信用') and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ") group by gdid";
            this.cashSql = "select gdid,(select username from agence where userid =ball_order.gdid) as gdname,count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mgd) as mgd,sum(mzdl) as mzdl,sum(mdls) as mdls FROM ball_order WHERE userid in (select userid from member where usertype='现金') and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ") group by gdid";
            this.allSql = "SELECT count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mgd) as mgd,sum(mzdl) as mzdl,sum(mdls) as mdls FROM ball_order WHERE datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ")";
            this.creditPartSql = "SELECT dbo.GettzType_Func(tzType) as tzTypeName,tzType,count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mzdl) as mzdl,sum(mdls) as mdls,sum(mgd) as mgd FROM ball_order WHERE userid in (select userid from member where usertype='信用') and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ") group by tzType";
            this.cashPartSql = "SELECT dbo.GettzType_Func(tzType) as tzTypeName,tzType,count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mzdl) as mzdl,sum(mdls) as mdls,sum(mgd) as mgd FROM ball_order WHERE userid in (select userid from member where usertype='现金') and datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ") group by tzType";
            this.allPartSql = "SELECT count(*) as tzNumber,sum(tzmoney) as tzmoney,sum(win-lose) as result,sum(mgd*(1-csgd-cszdl-csdls)) as mgs,sum(mgd) as mgd,sum(mzdl) as mzdl,sum(mdls) as mdls FROM ball_order WHERE datediff(s,'" + this.startTime + "',balltime) > 0 and datediff(d,balltime,'" + this.endTime + "') >= 0 and tzType in (" + MyFunc.GettzType(this.tzType) + ")";
        }
        ...
        private void ShowCashContent(DataBase db)
        {
            ...
            text = text + "<tr class=blueheader><td width=10%>股东</td><td width=10%>投注数</td><td width=10%>投注额</td><td width=10%>会员</td><td width=10%>代理商</td><td width=10%>总代理</td><td width=10%>股东</td><td width=10%>公司</td></tr>\n";
            while (reader.Read())
            {
                ...
                text = ((((((text2 + "<td><a href='reportshowzdl.aspx?search=cash" + this.pathStr + "&gdid=" + reader["gdid"].ToString() + "'><font color=blue>" + MyFunc.NumBerFormat(reader["tzmoney"].ToString()) + "</font></a></td>") + "<td>" + MyFunc.NumBerFormat(reader["result"].ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(reader["mdls"].ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(reader["mzdl"].ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(reader["mgd"].ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(reader["mgs"].ToString()) + "</td>") + "</tr>\n";
                ...
                num6 += double.Parse(reader["mgs"].ToString());
                num7 += double.Parse(reader["result"].ToString());
            }
            reader.Close();
            text = ((((((((text + "<tr class=reportTotalnum align=right height=22><td class=reportTotalchar>总 数</td>") + "<td>" + num.ToString() + "</td>") + "<td>" + MyFunc.NumBerFormat(num2.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num7.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num3.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num4.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num5.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num6.ToString()) + "</td>") + "</tr></table>\n";
        ...
        private void ShowCashPartContent(DataBase db)
        {
            ...
            text = "<table border=0 cellspacing=1 cellpadding=0 class=tableborder1 width=100%>\n";
            text = text + "<tr class=blueheader><td width=10%>投注方式</td><td width=10%>投注数</td><td width=10%>投注额</td><td width=10%>会员</td><td width=10%>代理商</td><td width=10%>总代理</td><td width=10%>股东</td><td width=10%>公司</td></tr>\n";
            while (reader.Read())
            {
            ...
            reader.Close();
            text = ((((((((text + "<tr class=reportTotalnum align=right height=22><td class=reportTotalchar>总 数</td>") + "<td>" + num.ToString() + "</td>") + "<td>" + MyFunc.NumBerFormat(num2.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num7.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num3.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num4.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num5.ToString()) + "</td>") + "<td>" + MyFunc.NumBerFormat(num6.ToString()) + "</td>") + "</tr>\n</table>\n";
            this.tableMiddle.Rows[1].Cells[0].InnerHtml = text;
        }
        ...
        private void ShowCreditContent(DataBase db)
        {
            ...
            text = text + "<tr class=dlsreport><td width=8%>股东</td><td width=8%>投注数</td><td width=14%>投注额</td><td width=14%>会员</td><td width=14%>代理商</td><td width=14%>总代理</td><td width=14%>股东</td><td width=14%>公司</td></tr>\n";
            while (reader.Read())
            {
        ...
        private void ShowCreditPartContent(DataBase db)
        {
            ...
            text = "<table border=0 cellspacing=1 cellpadding=0 class=tableNoborder1 width=100%>\n";
            text = text + "<tr class=dlsreport><td width=8%>投注方式</td><td width=8%>投注数</td><td width=14%>投注额</td><td width=14%>会员</td><td width=14%>代理商</td><td width=14%>总代理</td><td width=14%>股东</td><td width=10%>公司</td></tr>\n";
            while (reader.Read())
            {
                string text2 = ((text + "<tr bgcolor=white align=right height=22>") + "<td align=center>" + reader["tzTypeName"].ToString() + "</td>") + "<td>" + reader["tzNumber"].ToString() + "</td>";
