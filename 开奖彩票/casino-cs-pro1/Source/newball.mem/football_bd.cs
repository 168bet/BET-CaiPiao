            ...
            if (reader.Read())
            {
                ...
                if (num3 < 360)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and tzteam like '%" + set.Tables[0].Rows[num]["id"].ToString().Trim() + "%' group by ballid order by ballid asc");
                        }
                        else
                        {
                            reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0  and tzteam like '%" + set.Tables[0].Rows[num]["id"].ToString().Trim() + "%' group by ballid order by ballid desc");
                        }
                        ...
                        {
                            string text13 = text2;
                            text2 = text13 + "<td align=center class=tznumer>" + text3 + "   <br>  <a href='tzinfo.aspx?gameid=" + set.Tables[0].Rows[num]["id"].ToString().Trim() + "&tztype=" + text5 + "&marker=C'>  <font color=#000088>" + double.Parse(reader2["summoney"].ToString()).ToString() + "</font></a></td></tr> ";
                        }
                        else
                        {
