    ...
    public class index : Page
    {
        ...
        private void ButtonLogin_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                reader = db.ExecuteReader("SELECT userid,username,userpass,classid FROM agence WHERE username='" + text + "' AND classid=1 AND isuseable=1");
                if (!reader.Read())
                {
                    reader.Close();
                    db.Dispose();
                ...
                else
                {
                    ...
                    reader.Close();
                    reader = db.ExecuteReader("SELECT userid FROM agence WHERE classid=0 order by userid asc");
                    if (reader.Read())
                    {
                    ...
                    text6 = MyFunc.GetPlace(db, MyFunc.ChgIP(base.Request.UserHostAddress.ToString().Trim()));
                    db.Dispose();
                    MyTeam.OnlineList.OnlineList.NewUserLogin(this.Session.Contents["adminusername"].ToString().Trim(), this.Session.SessionID.ToString().Trim(), DateTime.Now.ToString(), ip, this.Session.Contents["adminclassid"].ToString().Trim(), place + "|" + text6);
