namespace newball.odds.subadmin
{
    ...
    public class BallJsFrame : Page
    {
        ...
        public int tatal = 0;
        public string[] total = new string[300];
        ...
        private void JsHmSx(string tztype)
        {
            ...
            string sql = "SELECT * FROM ball_order WHERE tztype=" + tztype + " AND iscancel=0 AND datediff(hh,balltime,getdate())<12";
            ...
            reader.Close();
            this.cancelcount = int.Parse(base3.ExecuteScalar("SELECT COUNT(1) FROM ball_order WHERE tztype=" + tztype + " and isjs=0  AND datediff(hh,balltime,getdate())<12 ").ToString());
            this.zdcount = int.Parse(base3.ExecuteScalar("SELECT COUNT(1) FROM ball_order WHERE tztype=" + tztype + " and isjs=1  AND datediff(hh,balltime,getdate())<12 ").ToString());
            base3.Dispose();
            base2.Dispose();
        }
        ...
        private void JsHmZt(string tztype)
        {
            ...
            while (reader.Read())
            {
                ...
                if (num > 0)
                {
                    num16 = num + ((num7 * num10) / 100);
                    num17 = num + ((num7 * num12) / 100);
                    num18 = num + ((num7 * num14) / 100);
                    num += (num7 * num8) / 100;
                }
                if (num2 > 0)
                {
                    num16 = -num2 + ((num7 * num11) / 100);
                    num17 = -num2 + ((num7 * num13) / 100);
                    num18 = -num2 + ((num7 * num15) / 100);
                    num2 -= (num7 * num9) / 100;
                }
                if ((num == 0) && (num2 == 0))
                {
        ...
        private void JsZm16Ds(string tztype)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    if (this.total[int.Parse(reader["ballid"].ToString().Trim())].ToString() == "1")
                    {
                        num = num6 * num7;
                        num3 = 1;
                    }
        ...
        private void JsZm16Ds1(string tztype)
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            DataBase base3 = new DataBase(MyFunc.GetConnStr(2));
            string sql = "SELECT * FROM ball_order2 WHERE tztype=" + tztype + " AND iscancel=0 AND datediff(hh,balltime,getdate())<12";
            SqlDataReader reader = base3.ExecuteReader(sql);
            while (reader.Read())
            {
                string text2 = reader["rqteam"].ToString().Trim();
                ...
                base2.ExecuteNonQuery("UPDATE ball_order2 SET win=" + num.ToString() + ",lose=" + num2.ToString() + ",isjs=1,truewin=" + num3.ToString() + ",iscancel=0,mdls=" + num16.ToString() + ",mzdl=" + num17.ToString() + ",mgd=" + num18.ToString() + " WHERE orderid=" + reader["orderid"].ToString().Trim());
            }
            reader.Close();
            this.cancelcount = int.Parse(base3.ExecuteScalar("SELECT COUNT(1) FROM ball_order2 WHERE tztype=" + tztype + " and isjs=0  AND datediff(hh,balltime,getdate())<12 ").ToString());
            this.zdcount = int.Parse(base3.ExecuteScalar("SELECT COUNT(1) FROM ball_order2 WHERE tztype=" + tztype + " and isjs=1  AND datediff(hh,balltime,getdate())<12 ").ToString());
            base3.Dispose();
            base2.Dispose();
        }
        ...
        private void JsZm2qz(string tztype)
        {
            ...
            while (reader.Read())
            {
                ...
                if (num20 > 0)
                {
                    ...
                    num = (((num6 + 1) * int.Parse(num27.ToString())) * num20) - num7;
                    num3 = 2;
                }
                else
                {
        ...
        private void JsZm3qz(string tztype)
        {
            ...
            while (reader.Read())
            {
                ...
                string text5 = "三全中<font color=red>" + num20 + "</font>注";
                if (num20 > 0)
                {
                    double num27 = num7 / ((double) num21);
                    num = (((num6 + 1) * int.Parse(num27.ToString())) * num20) - num7;
        ...
        private void JsZm3z2(string tztype)
        {
            ...
            while (reader.Read())
            {
                ...
                if (text3.IndexOf("^") == -1)
                {
                    ...
                    if (num19 != textArray.Length)
                    {
                        if (num19 >= 2)
                        {
                            num20 = ((num19 * (num19 - 1)) / 2) * (textArray.Length - num19);
                        }
                        if (num19 >= 3)
                        {
                            num22 = ((num19 * (num19 - 1)) * (num19 - 2)) / 6;
                        }
                    }
                    else if (num19 >= 3)
                    {
                ...
                else
                {
                    ...
                    if (text4 == "1")
                    {
                        ...
                        if (num19 != (textArray2.Length - 1))
                        {
                            if (num19 >= 1)
                            {
                                num20 = num19 * ((textArray2.Length - 1) - num19);
                            }
                            if (num19 >= 2)
                            {
                                num22 = (num19 * (num19 - 1)) / 2;
                            }
                        }
                        else if (num19 >= 2)
                        {
                    ...
                    else
                    {
                        for (int index = 1; index < textArray2.Length; index++)
                        {
                            for (int num29 = 0; num29 < 6; num29++)
                            {
                                if (textArray2[index].ToString().Trim() == this.result[num29].ToString().Trim())
                                {
                                    num19++;
                                    break;
                                }
                ...
                string text5 = string.Concat(new object[] { "中二<font color=red>", num20, "</font>注<br>中三<font color=red>", num22, "</font>注" });
                if ((num20 > 0) || (num22 > 0))
                {
                    double num30 = num7 / ((double) num21);
                    double num31 = num7 / ((double) num21);
                    num = ((((num6 + 1) * int.Parse(num30.ToString())) * num20) + (((double.Parse(reader["zdbf"].ToString().Trim()) + 1) * int.Parse(num31.ToString())) * num22)) - num7;
                    num3 = 2;
                }
                else
                {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((this.Session.Contents["adminclassid"] != null) && (this.Session.Contents["adminclassid"].ToString().Trim() != "5"))
            {
                MyFunc.goToLoginPage();
                base.Response.End();
                return;
            ...
            if (base.Request.Form["ballid"].ToString().Trim() == "0")
            {
                ...
                switch (base.Request.Form["jstype"].ToString().Trim())
                {
                    ...
                    case "21":
                        this.JsZm16Ds(tztype);
                        this.kyglContent = "parent.document.all['ht_rq'].innerHTML = '半波注单结算完成.有效:" + this.zdcount.ToString() + "  无效:" + this.cancelcount.ToString() + "';alert('半波注单结算完成');parent.location.href='GameFen.aspx';";
                        goto Label_0EC5;
                }
