namespace newball.gd
{
    ...
    public class football_bdgg : Page
    {
        ...
        private string SumAllMember()
        {
            ...
            {
                string text5 = sql;
                sql = text5 + " and (select count(*) from " + connStr + "..ball_order where datediff(day,updatetime,getdate()) = 0 and charindex(ball_pl4view.team1,content)>0 and charindex(ball_pl4view.team2,content)>0 and charindex(ball_pl4view.matchname,content)>0 and tztype = 16 and gdid='" + this.Session["adminuserid"].ToString() + "') > 0";
            }
            ...
            while (reader.Read())
            {
                ...
                text2 = text7 + "<td align=center><a href='tzinfo.aspx?gameid=" + reader["ballid"].ToString().Trim() + "&tztype=16&marker=none'>" + MyFunc.SumStaticGd(reader["ballid"].ToString().Trim(), "16", "none") + "</a></td></tr>";
            }
            if (num == 0)
            {
