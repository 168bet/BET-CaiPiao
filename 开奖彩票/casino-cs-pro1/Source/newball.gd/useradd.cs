    ...
    public class useradd : Page
    {
        ...
        private void ButtonSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string text8 = this.Session.Contents["adminuserid"].ToString().Trim();
                string text9 = this.Session.Contents["adminusername"].ToString().Trim();
        ...
        public void DropDownListZdl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataBase db = new DataBase(MyFunc.GetConnStr(2));
                SqlDataReader reader = db.ExecuteReader("SELECT userid,username FROM agence WHERE zdlid=" + this.DropDownListZdl.SelectedValue + " AND classid=4");
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                DataBase db = new DataBase(MyFunc.GetConnStr(2));
                SqlDataReader reader = db.ExecuteReader("SELECT userid,username FROM agence WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim() + " AND classid=3");
                while (reader.Read())
                {
                ...
                {
                    db.Dispose();
                    MyFunc.showmsg("你现在还没有总代理,请先添加总代理");
                    base.Response.End();
                }
                else
                {
