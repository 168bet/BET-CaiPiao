        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                else
                {
                    this.TextBoxBorder(true);
                    DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
                    SqlDataReader reader = base2.ExecuteReader("SELECT userid,username,userpass,zdlid,zdlname,truename,tel,usemoney,isuseable,maxmem FROM agence WHERE userid=" + base.Request.QueryString["id"].ToString().Trim() + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                    if (reader.Read())
                    {
                        ...
                        reader.Close();
                        int num = int.Parse(base2.ExecuteScalar("SELECT ISNULL(SUM(usemoney),0) FROM agence WHERE zdlid=" + this.TextBoxZdlID.Text + " AND isuseable=1").ToString());
                        int num2 = int.Parse(base2.ExecuteScalar("SELECT ISNULL(SUM(usemoney),0) FROM agence WHERE zdlid=" + this.TextBoxZdlID.Text + " AND isuseable=0").ToString());
