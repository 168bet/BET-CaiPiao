namespace newball.gd
{
    ...
    public class dlsmsg : Page
    {
        ...
        private void ButtonSave_Click(object sender, EventArgs e)
        {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            if (int.Parse(s) >= 0)
                            {
                                ...
                                int num5 = int.Parse(base2.ExecuteScalar("SELECT maxmem FROM agence WHERE userid=" + this.Session.Contents["adminuserid"].ToString().Trim()).ToString());
                                if (num5 > 0)
                                {
                                    int num6 = int.Parse(base2.ExecuteScalar("SELECT ISNULL(SUM(maxmem),0) FROM agence WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim() + " AND classid=4 AND userid<>" + text7).ToString());
            ...
            return;
        Label_04FF:
            num7 = int.Parse(base2.ExecuteScalar("SELECT ISNULL(usemoney,0) FROM agence WHERE userid=" + text9 + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim()).ToString());
            ...
            else if (base2.ExecuteNonQuery("UPDATE agence SET truename='" + input + "'" + text10 + ",usemoney=" + s + ",isuseable=" + text6 + ",tel='" + text8 + "',maxmem=" + text11 + " WHERE userid=" + text7 + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim()) > 0)
            {
                base2.Dispose();
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                if (((base.Request.Form["action"] == null) && (base.Request.QueryString["id"] != null)) && (base.Request.QueryString["id"].ToString().Trim() != ""))
                {
                    ...
                    SqlDataReader reader = base2.ExecuteReader("SELECT userid,username,userpass,truename,usemoney,bl,gdbl,zdlbl,gsbl,isuseable,tel,zdlid,zdlname,maxmem FROM agence WHERE userid=" + base.Request.QueryString["id"].ToString().Trim() + " AND classid=4 AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                    if (reader.Read())
                    {
                    ...
                    this.Label1.Text = "使用状况：启用:" + num.ToString() + "　停用:" + num2.ToString() + "　可用:" + num3.ToString();
                    base2.Dispose();
                }
            }
        }
