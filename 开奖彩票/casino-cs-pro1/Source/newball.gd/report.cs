namespace newball.gd
{
    ...
    public class report : Page
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!this.Page.IsPostBack)
                {
                    ...
                    SqlDataReader reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and gdid='" + this.Session["adminuserid"].ToString() + "'");
                    reader.Read();
                    ...
                    reader.Close();
                    reader = base2.ExecuteReader("select count(*) as rowcounts from ball_order where tzType = '8' AND isjs = 0 and iscancel = 0 and datediff(day,'" + DateTime.Today.ToShortDateString() + "',updatetime) = 0 and gdid='" + this.Session["adminuserid"].ToString() + "'");
                    reader.Read();
                    if (reader["rowcounts"].ToString() != "0")
                    {
        ...
        private void searchDeal()
        {
            ...
            path = (((((path + "&startTime=" + this.startTime.Value) + "&endTime=" + this.endTime.Value) + "&reportType=" + this.reportType.SelectedValue) + "&tzCase=" + this.tzCase.SelectedValue) + "&tzType=" + this.tzType.SelectedValue) + "&ballType=";
