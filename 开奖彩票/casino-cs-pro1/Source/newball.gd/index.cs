    ...
    public class index : Page
    {
        ...
        private void ButtonLogin_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                reader = db.ExecuteReader("SELECT userid,username,userpass,classid,gdid,islock FROM agence WHERE username='" + text + "' AND classid=2 AND isuseable = 1 and gdid in (" + MyFunc.GetGongSiID().ToString() + ")");
                if (!reader.Read())
                {
                    reader.Close();
                    db.Dispose();
