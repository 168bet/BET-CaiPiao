        ...
        private string SumAllMember()
        {
            ...
            if (reader.Read())
            {
                ...
                if (num3 < 360)
                {
                    ...
                    {
                        reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0 and gdid = '" + this.Session["adminuserid"].ToString() + "' and ballid in (230,231) group by tztype order by tztype asc");
                    }
                    else
                    {
                        reader2 = base2.ExecuteReader("select max(tztype) as tztype,max(tzteam) as tzteam,count(1) as orderno,isnull(sum(tzmoney*csgd),0) as summoney from ball_order where DATEDIFF(dd, GETDATE(), updatetime)=0 and gdid = '" + this.Session["adminuserid"].ToString() + "' and ballid  in (230,231) group by tztype order by tztype desc");
                    }
