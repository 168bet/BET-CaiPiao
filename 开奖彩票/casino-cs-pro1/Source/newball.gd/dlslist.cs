namespace newball.gd
{
    ...
    public class dlslist : Page
    {
        ...
        private void countmem()
        {
            ...
            string sql = "SELECT COUNT(*) FROM agence WHERE classid=4 AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim();
        ...
        {
            string sql = "SELECT userid,username,memcount,usemoney,truename,regtime,isuseable,zdlid,zdlname FROM agence WHERE classid=4 AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim();
            if (flag != "")
            {
        ...
        public void DelDls(string id, string zdlid)
        {
            ...
            {
                DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
                int num = (int) base2.ExecuteScalar("SELECT COUNT(*) FROM member WHERE dlsid=" + id + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                if (num > 0)
                {
                    base2.Dispose();
                ...
                else if (base2.ExecuteNonQuery("DELETE FROM agence WHERE userid=" + id + " AND classid=4 AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim()) > 0)
                {
                    base2.ExecuteNonQuery("UPDATE agence SET memcount=memcount-1 WHERE zdlid=" + zdlid + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                    base2.Dispose();
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.LabelGd.Text = this.Session.Contents["adminusername"].ToString();
                ...
                DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
                SqlDataReader reader = base2.ExecuteReader("SELECT userid,username FROM agence WHERE classid=3 AND  gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
        ...
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            string sql = "SELECT COUNT(1) FROM agence WHERE classid=4 AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim();
            if (flag != "")
            {
                sql = sql + " AND isuseable=" + flag;
            }
