namespace newball.gd
{
    ...
    public class userlist : Page
    {
        ...
        private void countmem()
        {
            ...
            string sql = "SELECT COUNT(*) FROM member WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim();
        ...
        private void DataGridDataBind(string flag, string field, int page)
        {
            ...
            {
                sql = sql + " AND zdlid=" + this.DropDownListZdl.SelectedValue;
                if ((this.DropDownListDls.SelectedValue != "") && (this.DropDownListDls.SelectedValue != "0"))
                {
                    sql = sql + " AND dlsid=" + this.DropDownListDls.SelectedValue;
                }
            }
        ...
        public string DelItem(string str, string id, string dlsid)
        {
            ...
            {
                return (" / <a href=userlist.aspx?action=del&id=" + id + "&did=" + dlsid + "&dlsid=" + this.DropDownListDls.SelectedValue + "&zdlid=" + this.DropDownListZdl.SelectedValue + "&flag=" + this.DropDownListAccountStatus.SelectedValue + " onClick=\"return confirm('确定要删除该会员吗?')\">删除</a>");
            }
            return "";
        }
        ...
        {
            string sql = "SELECT userid,username FROM agence WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim() + " AND classid=4";
            if (this.DropDownListZdl.SelectedValue != "")
            {
                sql = sql + " AND zdlid=" + this.DropDownListZdl.SelectedValue;
            }
        ...
        {
            return ("<a href=userlist.aspx?action=js&id=" + uid + "&dlsid=" + this.DropDownListDls.SelectedValue + "&zdlid=" + this.DropDownListZdl.SelectedValue + "&flag=" + this.DropDownListAccountStatus.SelectedValue + ">结帐</a>/<a href=userlist.aspx?action=re&id=" + uid + "&dlsid=" + this.DropDownListDls.SelectedValue + "&zdlid=" + this.DropDownListZdl.SelectedValue + "&flag=" + this.DropDownListAccountStatus.SelectedValue + ">回复</a>/" + this.OpItem(flag, uid, did) + "/<a href=usermsg.aspx?id=" + uid + ">详情</a>/<a href=userset.aspx?id=" + uid + ">设定</a>");
        }
        ...
        public string OpItem(string flag, string id, string dlsid)
        {
            if (flag.Trim().ToUpper() == "TRUE")
            {
                return ("<a href=userlist.aspx?action=Close&id=" + id + "&did=" + dlsid + "&dlsid=" + this.DropDownListDls.SelectedValue + "&zdlid=" + this.DropDownListZdl.SelectedValue + "&flag=" + this.DropDownListAccountStatus.SelectedValue + " onClick=\"return confirm('确定要停用该会员吗?')\">停用</a>");
            }
            return ("<a href=userlist.aspx?action=Open&id=" + id + "&did=" + dlsid + "&dlsid=" + this.DropDownListDls.SelectedValue + "&zdlid=" + this.DropDownListZdl.SelectedValue + "&flag=" + this.DropDownListAccountStatus.SelectedValue + " onClick=\"return confirm('确定要启用该会员吗?')\">启用</a>");
        }
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                if ((base.Request.QueryString["action"] != null) && (base.Request.QueryString["id"] != null))
                {
                    ...
                    {
                        base2.ExecuteNonQuery("UPDATE member SET jsdatetime=GetDate() WHERE userid=" + base.Request.QueryString["id"].ToString().Trim() + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                    }
                    ...
                    {
                        base2.ExecuteNonQuery("UPDATE member SET jsdatetime=null WHERE userid=" + base.Request.QueryString["id"].ToString().Trim() + " AND gdid=" + this.Session.Contents["adminuserid"].ToString().Trim());
                    }
                }
                this.DropDownListZdl.Items.Clear();
                this.DropDownListZdl.Items.Add(new ListItem("请选择", "0"));
                SqlDataReader reader = base2.ExecuteReader("SELECT userid,username FROM agence WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim() + " AND classid=3");
                while (reader.Read())
                {
                ...
                this.DlsList();
                if ((base.Request.QueryString["zdlid"] != null) && (base.Request.QueryString["zdlid"].ToString().Trim() != ""))
                {
                    this.DropDownListZdl.SelectedValue = base.Request.QueryString["zdlid"].ToString().Trim();
                }
                else
                {
        ...
        {
            DataBase base2 = new DataBase(MyFunc.GetConnStr(2));
            string sql = "SELECT COUNT(1) FROM member WHERE gdid=" + this.Session.Contents["adminuserid"].ToString().Trim();
            if (flag != "")
            {
                sql = sql + " AND isuseable=" + flag;
            }
            ...
            int num2 = 0;
            this.LabelCount.Text = "共有会员 " + num.ToString() + " 个";
            if (num > 0)
            {
