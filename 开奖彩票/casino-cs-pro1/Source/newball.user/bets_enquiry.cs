    ...
    public class bets_enquiry : Page
    {
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!base.IsPostBack)
            {
                ...
                this.kyglName = this.Session.Contents["username"].ToString().Trim();
                if ((base.Request.QueryString["date"] != null) && (base.Request.QueryString["date"].ToString().Trim() != ""))
                {
                    ...
                    sql = "SELECT updatetime,tztype,orderid,content,tzmoney,(win-lose) AS result,iscancel,isdanger,zdmoney,GetDate() as thisdatetime FROM ball_order WHERE userid=" + this.Session.Contents["userid"].ToString().Trim() + " AND datediff(day,balltime,'" + base.Request.QueryString["date"].ToString().Trim() + "')=0 AND isjs=1 ORDER BY updatetime DESC";
                }
                else
                {
                    this.kyglTT = "可嬴金额";
                    sql = "SELECT updatetime,tztype,orderid,content,tzmoney,(tzmoney*curpl) AS result,iscancel,isdanger,zdmoney,GetDate() as thisdatetime FROM ball_order WHERE userid=" + this.Session.Contents["userid"].ToString().Trim() + " AND isjs=0 ORDER BY updatetime DESC";
                }
                ...
                reader2.Close();
                base2.Dispose();
                this.kyglList = this.kyglList + "<TR style='BACKGROUND-COLOR: #e9e9e9'><TD class='tdR' colSpan='4'><IMG src='images/print.gif' border='0'> <A href='javascript:window.print()'> 打印</A>&nbsp;</TD>";
