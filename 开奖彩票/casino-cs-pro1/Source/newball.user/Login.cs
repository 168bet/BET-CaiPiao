namespace newball.user
{
    ...
    public class Login : Page
    {
        ...
        private void submitdeal()
        {
            ...
            if ((text.Length > 15) || (text2.Length > 15))
            {
            ...
            if ((text != reader["username"].ToString().ToLower().Trim()) || (text2 != reader["userpass"].ToString().ToLower().Trim()))
            {
                reader.Close();
                db.Dispose();
                MyFunc.showmsg("帐号或密码错误!");
            ...
            switch (reader["abc"].ToString().ToUpper())
            {
                case "A":
