...
//比赛编号,根据比赛表中的gameid跟联赛赛程的gameid做差集过滤
$game_sql = 'SELECT `gameid` FROM `ft_competition_schedule`;';
$old_sql = 'SELECT `gameid` FROM `ft_game`;';
$game_info = $mysqli->query($game_sql);
$old_info = $mysqli->query($old_sql);
$gameids = $old = array();
while ($row = mysqli_fetch_array($game_info)) {
    $gameids[] = $row['gameid'];
}
while ($row = mysqli_fetch_array($old_info)) {
    $old[] = $row['gameid'];
}
$gameids = array_diff($gameids, $old);
