...
class member extends admin
{
	...
	public function search()
    {
		...
		foreach($memberlist as $k=>$v) {
			$memberlist[$k]['avatar'] = get_memberavatar(intval($v['userid']));
		}
		$pages = $this->db->pages;
	...
	public function edit()
    {
		if(isset($_POST['dosubmit']))
        {
			...
			if(!empty($_POST['delavatar'])) {
                ...
                $is_deleted = $obj_avatar->deleteavatar(intval($userid));
			}

  			if(empty($_POST['delavatar']) || $is_deleted)//修复原本删除头像后直接退出导致无法保存的问题	edit by lxt 2016.04.12
            {
				unset($info['userid']);
				unset($info['username']);
        ...
        else
        {
            ...
            // 获取会员头像
			$memberinfo['avatar'] = get_memberavatar(intval($userid));
	...
	public function memberinfo()
    {
		...
		$memberinfo['avatar'] = get_memberavatar($userid, 90);
