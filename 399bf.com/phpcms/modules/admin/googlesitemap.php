	...
	function set () {
		...
		$this_domain = substr($sitecache[$siteid]['domain'], 0,strlen($sitecache[$siteid]['domain'])-1);
   		if($dosubmit) {
			...
			/* 过滤栏目不生成地图
				4 竞彩篮球预测、5 竞彩篮球分析 */
			$filter_catid_arr = [4, 5];
			foreach ($this->categorys as $catid => $cat) {
				if (in_array($catid, $filter_catid_arr)) {
					continue;
				}

				if (substr($cat['url'], 0, 1) == '/') {
					$url = new_html_special_chars(strip_tags($domain . $cat['url']));
				} else {
					$url = new_html_special_chars(strip_tags($cat['url']));
				}
			...
			foreach($new_model as $modelid=>$m) {//每个模块取出num条数据
				...
				$result = $this->content_db->select(array('status'=>99), '*', '', $order = 'inputtime desc');
				foreach ($result as $arr){
