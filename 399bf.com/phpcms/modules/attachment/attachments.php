...
class attachments {
	...
	{
		$CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents(APP_PATH . "phpcms/modules/attachment/config.json")), true);
		$action = $_GET['action'];
		$result = '';
		switch ($action) {
			case 'config':
	...
	public function ueditor_upload()
	{
		...
		$result = $attachment->ueditor_upload('upfile',$site_allowext);
		if ($result) {
			$info = array(
				'state' => 'SUCCESS',
				'url'	=> $this->upload_url . $attachment->uploadedfiles[0]['filepath'],
				'title' => $attachment->uploadedfiles[0]['filename'],
				'original' => $attachment->uploadedfiles[0]['filename'],
				'type'	=> $attachment->uploadedfiles[0]['fileext'],
