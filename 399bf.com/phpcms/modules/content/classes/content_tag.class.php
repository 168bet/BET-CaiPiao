...
class content_tag {
	...
	public function count($data) {
		//针对lists_model的分页统计		edit by lxt 2016.6.30
		switch ($data['action']) {
			case 'lists':
				...
				else
				{
					...
					if(!empty($data['keywords'])){
						$keywords = safe_replace(new_html_special_chars($data['keywords']));
			...
			case 'lists_model':
				...
				$this->db->set_model($modelid); // 初始化模型
				if(isset($data['where'])) {
					$sql = $data['where'];
				} else {
		...
		else
		{
			...
			if(!empty($data['keywords'])){
				...
				foreach ($keywords as $keyword) {
					$keywords_list[] = ' keywords LIKE "%' . addslashes(safe_replace(new_html_special_chars($keyword))) . '%"';
				}
				$keywords_where = ' AND (' . join(" OR ", $keywords_list) . ') ';
				$sql .= $keywords_where;
//				$keywords = safe_replace(new_html_special_chars($data['keywords']));
...
//				$sql .= " AND keywords LIKE '%$keywords%' ";
		...
		//读取hits表数据
		if (isset($data['hits']) && intval($data['hits']) == 1) {
			$hits_ids = array();
			foreach ($return as $v) {
				if (isset($v['id']) && !empty($v['id'])) {
					$hits_ids[] = "'c-" . $this->modelid . "-" . $v['id'] . "'";
				} else {
					continue;
				}
			}
			if (!empty($hits_ids)) {
				...
				$ids = join(',', $hits_ids);
				$sql = 'SELECT `hitsid`,`views` FROM ' . $table_name . ' WHERE `hitsid` IN (' . $ids . ')' . ' AND `catid`=' . $catid;
				$hits = $this->db->query($sql);
				$hits_info = $hits->fetch_all(MYSQLI_ASSOC);
				if (!empty($hits_info)) {
					foreach ($hits_info as $value) {
						$pos = strpos($value['hitsid'],'-',2) + 1;
						$key = substr($value['hitsid'],$pos);
						if (isset($return[$key])) $return[$key] = array_merge($value, $return[$key]);
					}
				}
			}
		}
	 ...
	 * @param $data
	 */
	public function lists_model($data) {
		$modelid = intval($data['modelid']);
		$this->db->set_model($modelid); // 初始化模型
		if(isset($data['where'])) {
			$sql = $data['where'];
	...
	public function relation($data) {
		...
		//增加where条件处理		add by lxt 2016.07.08
		if (isset($data['where'])) {
			$sql .= " AND " . $data['where'];
		}
	...
	public function hits($data) {
		$catid = intval($data['catid']);
		if(!$this->set_modelid($catid)) return false;
		$thumb = $data['thumb'] ? true : false;//新增参数，对最终news表查询时做有无缩略图过滤	add by lxt 2016.05.17
		...
		//新增参数，对最终news表查询时做有无缩略图过滤	add by lxt 2016.05.17
		if ($thumb) {
			$sql .=  (strlen($sql) > 0 ? ' AND ' : '') . '`thumb` <> ""';
		}
	...
	public function position($data) {
		...
		if(!empty($pos_arr)) {
			foreach ($pos_arr as $info) {
				...
				//补充推荐位自由链接
				$array[$key]['url'] = isset($array[$key]['url']) ? $array[$key]['url'] : go($info['catid'],$info['id']);
