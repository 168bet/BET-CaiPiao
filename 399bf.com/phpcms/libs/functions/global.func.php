 ...
 * @param $data      array|string 条件数组或者字符串
 * @param $front     string 连接符
 * @param $in_column bool 字段名称
 * @return string
 */
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	...
	/**
	 * 需要调用后台分页函数的情况：	add by lxt 2016.05.12
	 * {
	 *		1.通过admin.php访问：例外{
	 *			控制器：create_html
	 * 		}
	 * 		2.不是通过admin.php访问：例外{
	 *			模块：attachment
	 * 		}
	 * }
	 */
	if ((defined('IN_ADMIN') && !in_array(ROUTE_C,['create_html'])) || (in_array(ROUTE_M,['attachment']))) {
		return admin_pages($num, $curr_page, $perpage, $urlrule, $array, $setpages);
	}
	...
	if($num > $perpage) {
		...
		$to = $curr_page + $offset;
		$more = 0;
		//echo 'page='.$page.' pages='.$pages.' curr_page='.$curr_page.' offset='.$offset;
		if($page >= $pages) {
			$from = 2;
			$to = $pages-1;
		} else {
        ...
        // del by tangkh 2016.05.05 屏蔽总条数
		//$multipage .= '<a class="a1">'.$num.L('page_item').'</a>';
		if($curr_page>0) {
			$multipage .= ' <li class="prev"><a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="a1"><span aria-hidden="true"><i class="icon-arrow-left arrow-color"></i>
</a></li>';
			if($curr_page==1) {
				$multipage .= ' <li class="active"><span>1</span></li>';
			} elseif($curr_page>3 && $more) {
				$multipage .= ' <li><a href="'.pageurl($urlrule, 1, $array).'">1</a></li><li>...</li>';
			} else {
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <li>...</li><li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li class="next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1"><i class="icon-arrow-right arrow-color"></i>
</a></li>';
			} else {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li class="next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1"><i class="icon-arrow-right arrow-color"></i></a></li>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <li class="active"><span>'.$pages.'</span></li> <li class="next"><a href="'.pageurl($urlrule, $curr_page, $array).'" class="a1"><i class="icon-arrow-right arrow-color"></i></a></li>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li class="next"><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1"><i class="icon-arrow-right arrow-color"></i></a></li>';
		}
 ...
 * @return 分页
 */
function pages_bak_20170224($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		//$multipage .= '<a class="a1">'.$num.L('page_item').'</a>';
		if($curr_page>0) {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="a1"><span aria-hidden="true">'.L('previous').'</a></li>';
			if($curr_page==1) {
				$multipage .= ' <li class="active"><span>1</span></li>';
		...
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' <li>..</li><li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></li>';
			} else {
 ...
 * @return 分页
 */
function admin_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
...
function pageurl($urlrule, $page, $array = array()) {
	...
	//添加关键词参数
	//针对URL规则 {$catid}/|{$catid}/{$page}/
	if(!empty($array['keywords']) &&
		preg_match('@^(\d+/)(\d+/)$|^\d+/$@', $url, $matches)){
		if(isset($matches[2])){
			$url = $matches[1].$array['keywords'].'/'.$matches[2];
		}else{
			$url = $matches[0].$array['keywords'].'/';
		}
	}
	//修复后台分页地址错误
//	if(!defined('IN_ADMIN')){
//		$url = 'http://'.parse_url(get_url(), PHP_URL_HOST).'/'.$url;
//	}
	return $url;
}
 ...
 */
function get_memberavatar($uid, $size='30')
{
    $obj_avatar = pc_base::load_app_class('avatar', 'member');
    $avatar = $obj_avatar->getavatar($uid);
...
function catpos($catid, $symbol=''){
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= '<a class="orange" style="font-weight:normal;" href="'.$url.'">'.$category_arr[$catid]['catname'].'</a>'.$symbol;
	}
	return $pos;
}
...
function get_category($filter = false) {
	$categories = $cache = array();
	$cache = getcache('category_content_1','commons');
	if (count($cache)) {
		foreach ($cache as $cat_id => $data) {
			//用作判断选中的栏目是否属于其子栏目
			$data['child_arr'] = explode(',', $data['arrchildid']);
			//按父级分组
			$parents = explode(',', $data['arrparentid']);
			switch (count($parents)) {
				//二级
				case 2:
					$first = $parents[1];
					$categories[$first]['children'][$cat_id] = isset($categories[$first]['children'][$cat_id]) ? array_merge($categories[$first]['children'][$cat_id], $data) : $data;
					break;
				//三级
				case 3:
					$first = $parents[1];
					$second = $parents[2];
					$categories[$first]['children'][$second]['children'][$cat_id] = $data;
					break;
				//一级
				default:
					$categories[$cat_id] = isset($categories[$cat_id]) ? array_merge($categories[$cat_id], $data) : $data;
					break;
			}
		}
	}
 ...
 * 获取分页函数title属性
 */
function page_title($num)
{
	$num_cn = number_cn($num) && $num > 1 ? '_第' . number_cn($num) . '页' : '';
	return $GLOBALS['seo_title'] . $num_cn;
}
?>