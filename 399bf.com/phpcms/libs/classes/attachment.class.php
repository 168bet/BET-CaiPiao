...
class attachment {
	 ...
	 * @param int $watermark_enable
	 * @return bool
	 */
	public function ueditor_upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1)
	{
		...
		foreach($uploadfiles as $k=>$file) {
			...
			if(@$upload_func($file['tmp_name'], $savefile)) {
				...
				//生成150*150(小)、350*350(中)的缩略图
				$image = new image(1,$this->siteid);
				$image->thumb($savefile,dirname($savefile).'/thumb_150_'.basename($savefile),150,150);
				$image->thumb($savefile,dirname($savefile).'/thumb_350_'.basename($savefile),350,350);
				//判断是否生成缩略图(之前功能)
				$thumb_enable = is_array($thumb_setting) && ($thumb_setting[0] > 0 || $thumb_setting[1] > 0 ) ? 1 : 0;
				$image->thumb_enable = $thumb_enable;
				if($thumb_enable) {
				...
				$aids[] = $this->ueditor_add($uploadedfile);
			}
		}
	...
	public function ueditor_add($uploadedfile)
	{
		$this->att_db = pc_base::load_model('attachment_model');
