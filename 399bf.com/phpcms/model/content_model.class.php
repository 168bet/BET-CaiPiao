...
class content_model extends model
{
	...
	public function add_content($data,$isimport = 0)
	{
		...
		// 标题图片地址更换为图片服务器地址
		/*if(($this->modelid == '1' || $this->modelid == '3') && isset($systeminfo['thumb']))
		{
			$systeminfo['thumb'] = preg_replace_callback(
				"/[^ \"']+(uploadfile[^ \"']+\.[gif|jpg|jpeg|bmp|png])/i",
				create_function(
					'$matches',
					'return PIC_PATH.$matches[1];'
				),
				$systeminfo['thumb']
			);
		}*/
		...
		// 内容图片更换为图片服务器地址
		/*if(($this->modelid == '1' || $this->modelid == '3') && isset($modelinfo['content']))
		{
			$content = stripslashes($modelinfo['content']);
			$content = preg_replace_callback(
				"/(src=[\"|']?)[^ \"']+(uploadfile[^ \"']+\.[gif|jpg|jpeg|bmp|png])/i",
				create_function(
					'$matches',
					'return $matches[1].PIC_PATH.$matches[2];'
				),
				$content
			);
			$modelinfo['content'] = addslashes($content);
		}*/

		// 图片模型组图更换为图片服务器地址
		/*if($this->modelid == '3' && isset($modelinfo['pictureurls']))
		{
			define('PICS_PATH', preg_replace("@/@", '\\/', PIC_PATH));
			$pictureurls = stripslashes($modelinfo['pictureurls']);
			$pictureurls = preg_replace_callback(
				"/(\"url\":\")[^ \"']+(uploadfile[^ \"']+\.[gif|jpg|jpeg|bmp|png])/i",
				create_function(
					'$matches',
					'return $matches[1].PICS_PATH.$matches[2];'
				),
				$pictureurls
			);
			$modelinfo['pictureurls'] = addslashes($pictureurls);
		}*/
	 ...
	 * 修改内容
	 *
	 * @param array $data $_POST['info']数据
	 * @return bool
	 */
