...
$.fn.banqh = function(can){
    ...
    $(can.pic).width("100%");
    $(can.pic).find('li').width($("#ban_pic1").width());
...
//小图切换过程
    function minshow(xtqhnum){
        ...
        // console.log("mingdjl_num:"+mingdjl_num);
        var mingdjl_w=-mingdjl_num*picminw;
        //console.log("mingdjl_w:"+mingdjl_w);
        var mingdjl_h=-mingdjl_num*picminh;

        if(can.mindire==true){
            $(can.pnum).find('ul li').css('float','left');
            if(picminnum>can.min_picnum){
                if(xtqhnum<can.min_picnum){
                    mingdjl_w=0;
                }else{
                    mingdjl_w=-(mingdjl_num-12)*picminw;
                }
...
//大图切换过程
    function show(tpqhnum){
        var liw=$(can.pic).find('ul li').width();
        var gdjl_w=-tpqhnum*liw;
