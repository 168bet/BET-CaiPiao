(function ($) {
    ...
    $('#layer').click(function () {
        ...
        if (bool) {
            ...
            $(this).removeClass('checkbox-off').addClass('checkbox-on');
            $(this).siblings('input').prop('checked', true);
            var index_selector,key_selector,td_arr,row;
            for (i = 3; i <= 5; i++) {
                index_selector = '[data-index="' + i + '"]';
                for (j = 0; j <= 9; j++) {
                    key_selector = '[data-key="' + j + '"]';
            ...
            //类型
            for (i = 1; i <= 3; i++) {
                key_selector = '[data-index="type"][data-key="' + i + '"]';
                td_arr = tr.find('td' + key_selector);
                row = $(key_selector + '.on:last').parent('tr').data('row') == undefined ? $(key_selector + ':first').parent('tr').data('row') + 1 : $(key_selector + '.on:last').parent('tr').data('row');
                td_arr.each(function () {
                    if (Number($(this).parent('tr').data('row')) < Number(row)) {
                        $(this).addClass('lost-layer');
                    }
