<!DOCTYPE html>
<html>
<head>
    <title>竞彩足球混合过关 - 订单投注 - 体育彩票 | 福利彩票 | 中国体育彩票 - 趣彩网(触屏版)</title>
    <meta http-equiv="content-type" content="text/html; charset=UTF-8"/>
    <meta charset="utf-8"/>
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta content="initial-scale=1.0,minimum-scale=1.0,maximum-scale=1.0" name="viewport">
    <meta content="yes" name="apple-mobile-web-app-capable">
    <meta content="black" name="apple-mobile-web-app-status-bar-style">
    <meta content="telephone=no" name="format-detection">
    <link href="/css/main.min.css" rel="stylesheet" type="text/css"/>
    <link href="/css/common.css" rel="stylesheet" type="text/css"/>
    <script src="/scripts/share.touch.min.js" type="text/javascript"></script>
    <script src="/scripts/common.js" type="text/javascript"></script>
    <script src="/scripts/iscroll.js" type="text/javascript"></script>
    <script src="/scripts/onlinePhone.js" type="text/javascript"></script>
</head>
<body>
<div id="tx_c" class="logints" style="display:none;"></div>
<section id="dConfirm" class="zfPop weige_" style="position: fixed;z-index: 1000;display: none"><h4>提示</h4>
    <div class="clearfix pdLeft08 center"></div>
    <div class="zfTrue clearfix">
        <input type="button" value="取消" class="zfqx" id="zfqx"/>
        <input type="button" value="确定" id="zfqd"/>
    </div>
</section>
<div id="Mask"
     style="display: none; position: fixed; left: 0px; top: 0px; width: 100%; height: 100%; background: none repeat scroll 0% 0% gray; opacity: 0.5; z-index: 999;"></div>
<script type="text/javascript">
    var ires = "http://res.qcwddd.com/iqucai.touch/images/";
    var dres = "http://data.qcwddd.com/matchdata/";
</script>

<div style="position: relative;" id="outer">
    <div class="wrap">
        <link href="/css/jczq.css" rel="stylesheet" type="text/css"/>
        <div class="zhezhao" style=" display:none"></div>
        <div class="content_home">
            <header class="tzHeader">
                <div class="fixed2">
                    <div class="hmHeader">
                        <a href="javascript:history.go(-1)" class="back">&lt;</a>
                        <h1 id="wanfa_">
                            竞彩混合过关
                        </h1>
                        <div class="hmPull" style="display:none;">
                            <a class="" href="/buy/jingcai/brqspf">胜平负</a>
                            <a class="" href="/buy/jingcai/spf">让球胜平负</a>
                            <a class="" href="/buy/jingcai/bf">比分</a>
                            <a class="" href="/buy/jingcai/zjq">总进球</a>
                            <a class="" href="/buy/jingcai/bqc">半全场</a>
                            <a class="cur" href="/buy/jingcai/hh">混合过关</a>
                            <a class="" href="/buy/jingcai/hh?oddstype=1">单关固定</a>

                        </div>
                        <div style="position: absolute; right: 2.1rem; top: 0.6rem; font-size: 0.85rem; line-height: 29px; height: 29px;">
                            <span style="height: 29px; line-height: 29px; display: block; float: left; color: red;">加奖10%</span>
                        </div>
                        <div class="pullDown">
                            <em id="pullIco" class="pullIco"></em>
                            <div style="display: none" class="pullText" id="pullText">

                                <a href="javascript:;" class="cur">选择比赛</a> <a href="/hemai.html">合买跟单</a>
                            </div>
                        </div>
                    </div>
                </div>
            </header>
            <section>
                <nav class="jczqNav">
                    <ul class="zkTitle fixed2 clearfix">
                        <li>赛事</li>
                        <li>主队</li>
                        <li class="zkVS">VS</li>
                        <li>客队</li>
                    </ul>
                </nav>
                <article id="content_">

                </article>
            </section>
            <footer class="buyFooter">
                <div class="fixed buyFloat">
                    <em class="deleted"></em>
                    <span>已选择<cite id="c_" class="yellow">0</cite>场比赛</span>
                    <a id="isOk_" class="ture" href="javascript:;">确 认</a>
                </div>
            </footer>
        </div>

    </div>
</div>
<div class="hide">

</div>
</body>
</html>
