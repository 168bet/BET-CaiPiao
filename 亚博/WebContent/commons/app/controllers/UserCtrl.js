angular.module('cpApp')
    ...
    .controller('tradition_wxPayCtrl', ['$rootScope', '$scope','UserCenter', 'UI', function($rootScope, $scope, UserCenter, UI) {
        $rootScope.title = '传统微信入款';
         $scope.data = {
            step: 1,
        };
        $scope.formdata = {
        ...
        //获取banklist
        UserCenter.bankPayList().then(function(res){
            $scope.tradition_wxData = res.data.datas.tradition_wx;
            $scope.formdata.payType = res.data.datas.tradition_wx.payType;
            //控制tab显示隐藏
            $scope.showWxOnline = angular.isEmpty(res.data.datas.online_wx);
            $scope.showCompany = angular.isEmpty(res.data.datas.company);
            $scope.showOnline = angular.isEmpty(res.data.datas.online_bank);
        ...
        //充值
        $scope.goPayCenter = function(){
            UserCenter.tradition_wxPayData($scope.formdata).then(function(res){
                if(res.data.rs){
                    UI.alert(res.data.msg);
                    $scope.data.step = 2;
                } else {
    ...
    .controller('tradition_aliPayCtrl', ['$rootScope', '$scope','UserCenter', 'UI', function($rootScope, $scope, UserCenter, UI) {
        $rootScope.title = '传统支付宝入款';
         $scope.data = {
            step: 1,
        };
        $scope.formdata = {
        ...
        //获取banklist
        UserCenter.bankPayList().then(function(res){
            $scope.tradition_aliData = res.data.datas.tradition_ali;
            $scope.formdata.payType = res.data.datas.tradition_ali.payType;
            //控制tab显示隐藏
            $scope.showWxOnline = angular.isEmpty(res.data.datas.online_wx);
            $scope.showCompany = angular.isEmpty(res.data.datas.company);
            $scope.showOnline = angular.isEmpty(res.data.datas.online_bank);
        ...
        //充值
        $scope.goPayCenter = function(){
            UserCenter.tradition_aliPayData($scope.formdata).then(function(res){
                if(res.data.rs){
                    UI.alert(res.data.msg);
                    $scope.data.step = 2;
                } else {
