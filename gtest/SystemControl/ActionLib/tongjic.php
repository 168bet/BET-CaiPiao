                    ...
                    <tr>
                        ...
                        <!-- end -->
						  ...
						  <table border="0" cellspacing="0" class="conter">
								...
								$sql = "SELECT *,sum(g_money) s,count(g_name) c  FROM `g_qdetail` where g_type=0 group by g_name order by g_id desc";
								$total=$db->query($sql, 3);								
								$pageNum = 20;
								$page = new Page($total, $pageNum);
 								$sql = "SELECT *,sum(g_money) s,count(g_name) c  FROM `g_qdetail` where g_type=0 group by g_name order by g_id desc {$page->limit}";
