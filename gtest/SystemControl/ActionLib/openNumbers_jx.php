...
if (isset($_GET['delid']) && Matchs::isNumber($_GET['delid']))
{
	...
	if ($id)
	{
		...
		exit(alert_href('刪除成功', 'openNumbers_jx.php'));
	}
	else 
	{
...
if (isset($_GET['inserid']) && Matchs::isNumber($_GET['inserid']))
{
	...
	InsertNumberjx($inserid, $ConfigModel['g_close_time']);
	exit(alert_href('操作成功', 'NumberInclude.php'));
}
else 
{
	$result = $db->query("SELECT `g_id`, `g_qishu`, `g_feng_date`, `g_open_date`, `g_lock` FROM `g_kaipan4` ORDER BY g_qishu ASC ", 1);
}
?>
