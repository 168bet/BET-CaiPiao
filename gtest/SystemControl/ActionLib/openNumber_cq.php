...
else 
{
	...
	//取出最近20期未結算注單
	$sql = "SELECT g_qishu FROM g_history2 WHERE g_game_id = 2 ORDER BY g_qishu DESC LIMIT 120";
...
<!--
	...
	function numberUpMen(){
		...
		if (num.length != 11) {alert("請輸期數格式錯誤！"); return;}
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUpMen.php", {mid : 1,men : men, num : num, type : 2}, function(data){
				if(data != 1){
					alert("第 "+num+" 期不存在列表中或未結算，無法恢復。");
					return;
				} else {
