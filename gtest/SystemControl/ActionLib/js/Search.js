...
function GoSearch (str, $this){
	...
	var href = "/SystemControl/ActionLib/Actfor.php?cid="+url;
...
function closeUser(name, $this, lid){
	if (confirm("確定將 "+name+" 用戶踢出系統嗎？")){
		$.post("/SystemControl/ActionLib/ajax/json.php", {typeid : 10, sUid : name, lid : lid}, function(data){
			if (data == 1){
				$this.src ="/SystemControl/ActionLib/images/USER_0.gif";
				$this.classNmae="";
				$this.title="";
				$this.onclick=""
			}
