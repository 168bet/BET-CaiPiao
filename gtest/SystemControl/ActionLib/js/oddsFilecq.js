var url = "/SystemControl/ActionLib/ajax/oddsJsoncq.php";
var interval, Phases, _endtime, p=0, z=true;
$(function(){
	actions();
	loadNumberInfo(false);
});
...
function loadNumberInfo(bool){
	var number = $("#numb"); //開獎期數
	$.post(url, {mid : 6}, function(data){
		_Number (data.number, data.ballArr); //開獎號碼
	}, "json");
	setTimeout(loadNumberInfo, 3000);
}
...
function PostForm(args){
	$.post("/SystemControl/ActionLib/ajax/json.php", {
			typeid : 6, 
