...
var Urls = "/SystemControl/ActionLib/ajax/jsonk3.php";
var EstateTime = 90;
var EndTime = 90;
var StartTime = 90;
...
function loadInfo(bool){
	var number = $("#n"); //開獎期數
	$.post(Urls, {typeid : 11}, function(data){
...
function setOdds (oddsList){
	...
	for (var i in oddsList[k]){
		...
		switch(k){
		case 0 :$("#t1_"+i).html(_odds);break;
		case 1 :$("#t2_"+i).html(_odds);break;
		case 2 :$("#t3_"+i).html(_odds);break;
		case 3 :$("#t4_"+i).html(_odds);break;
		case 4 :$("#t5_"+i).html(_odds);break;
		}
		var w = i.substr(1);
...
function GoPos($this, sInt){
	if (EndTime > 1) {
		...
		if (sInt == 1) {
			...
			s1=s.split('_');
			switch(s1[0]){
				case 't1' : $("#s_type1").html('三軍');break;
				case 't2' : $("#s_type1").html('圍骰');break;
				case 't3' : $("#s_type1").html('點數');break;
				case 't4' : $("#s_type1").html('長牌');break;
				case 't5' : $("#s_type1").html('短牌');break;
