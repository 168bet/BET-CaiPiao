...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if ($db->query($sql, 2))
	{
		$SumAmount = new SumAmountjx($arr['s_number'], false, $arr['uid'], $sum);
...
{
	$sql = "SELECT g_id FROM g_zhudan WHERE g_t_id = '{$uid}' AND g_type = '江西時時彩' AND g_qishu = '{$result[0]['g_qishu']}}' LIMIT 1 ";
	if ($db->query($sql, 0))
