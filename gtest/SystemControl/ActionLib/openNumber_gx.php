...
else 
{
	...
	//取出最近84期未結算注單
	$sql = "SELECT g_qishu FROM g_history3 WHERE g_game_id = 3 ORDER BY g_qishu DESC LIMIT 50";
...
<!--
	...
	function numberUp(){
		...
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1, num : num, type : 3}, function(data){
				if(data != 1){
	...
	function numberUpMen(){
		...
		if (num.length != 10) {alert("請輸期數格式錯誤！"); return;}
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUp.php", {mid : 1,men : men, num : num, type : 3}, function(data){
				if(data != 1){
					alert("第 "+num+" 期不存在列表中或未結算，無法恢復。");
					return;
				} else {
