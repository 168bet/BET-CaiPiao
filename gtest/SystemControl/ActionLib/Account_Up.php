	...
	if ($userList) {
		...
		if ($cid == 1 && $LoginId == 89){
			...
			if ($userList[0]['g_lock'] != $lock){
				...
				upDateRankLock($db, $result,'g_lock', $lock,2);
				...
				$sql="SELECT g_s_name, g_lock FROM g_relation_user WHERE g_s_nid LIKE '{$userList[0]['g_nid']}%'  ";
				$result = $db->query($sql, 1);
				upDateRankLock($db, $result,'g_lock', $lock, 0);
				//alert('1');
			}
			...
			if ($s_a_lock != $userList[0]['g_Immediate_lock']){
				if ($Luser[0]['g_Immediate_lock'] != 1){
					...
					upDateRankLock($db, $Immediate, 'g_Immediate_lock', $s_a_lock,2);
					//$sql="SELECT g_s_name, g_lock FROM g_relation_user WHERE g_s_nid = '{$userList[0]['g_nid']}'  ";
					//$result = $db->query($sql, 1);
					//upDateRankLock($db, $result,'g_lock', $s_a_lock, 0);
					//alert('2');
			...
			if ($s_money != $userList[0]['g_money']) {
				...
				if ($s_money < $userList[0]['g_money']) {				
		            ...
		            $sql = "UPDATE g_rank SET `g_money`= g_money + '{$s_moneys}' WHERE g_name = '{$Luser[0]['g_name']}' LIMIT 1";
		            $db->query($sql, 2);
				} else {
					/**
					 * 當前帳號級別不是股東的情況下
					...
					if ($userList[0]['g_login_id']!=56) {
						...
						$s_moneys = $s_money - $userList[0]['g_money'];
						if ($s_moneys > $Luser[0]['g_money']) exit(back($LRank[0].'可用餘額：'.$Luser[0]['g_money']));
...
function validMoneys ($userModel, $countMoney, $nid, $param) {
	$validMoney = $userModel->SumMoney($nid,$param);
	return $validMoney;
}
                    ...
                    <tr>
                        ...
                        <!-- strat -->
                            <table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:28px">
                                	<td class="bj">可用額度</td>
                                    <td class="left_p5"><input class="text" type="text" name="s_money" value="<?php echo $userList[0]['g_money']?>"  maxlength="20" onkeyup="DX(this.value);" />&nbsp;
									 <span class="red" style="font-weight:bold" name="rmb" id="rmb"> <?php echo $userList[0]['g_money'];?></span>
                                   	『&nbsp;可‘回收’餘額&nbsp;<span id='money_ky' class="red"><?php echo $userList[0]['g_money']?></span>&nbsp;』</td>
                                </tr>
                                <tr style="height:28px">
