...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['actions']) && isset($_GET['cid']))
{
	...
	if ($_GET['actions'] == 'add')
	{
		...
		else if ($_GET['cid']==2 && ($LoginId==89 || $LoginId==56)) //新增股東帳號
		{
			...
			$sql="update g_rank set g_money=g_money - ".$_POST['s_money']." where g_name='{$g_nid[0]['g_name']}'";
			$db->query($sql, 2);
			$userList = $userModel->AddUser($userList);
			alert_href('新增成功', 'Actfor.php?cid='.$_GET['cid']);
			exit;
                        ...
                        <?php if ($aid=='add'){?>
                        	<table border="0" cellspacing="0" class="conter">
                                ...
                                <tr style="height:28px">
                                    ...
                                    <td class="left_p5"><input name="s_Name" id="s_Name" type="text" class="text" />&nbsp;&nbsp; 
