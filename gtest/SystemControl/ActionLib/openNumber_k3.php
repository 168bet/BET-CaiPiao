...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	if (!empty($_POST['s_num1'])&&!empty($_POST['s_num2'])&&!empty($_POST['s_num3'])){
			...
			if ($db->query("SELECT g_id FROM g_history7 WHERE g_id = '{$_GET['UpNumcid']}' AND g_game_id =7 LIMIT 1", 0)){
				...
				$db->query($sql, 2);
				exit(alert_href('更變成功。', 'openNumber_k3.php'));
			}
	} else {
		if (!Matchs::isNumber($_POST['number'],10,15)) exit(back('開獎期數格式錯誤！'));
		...
		if ($db->query($sql, 0)){
			...
			if ($db->query($sql, 2)){
				if ($_POST['cry'] == 1||$_POST['cry'] == 'on'){
					$SumAmount = new SumAmountk3($arr['g_qishu']);
...
else if (isset($_GET['startId']) && $_GET['startId'] == 1)
{
	//執行結算
	$numberId = $_GET['numId'];
	$sql ="SELECT g_id FROM g_history7 WHERE g_qishu = '{$numberId}' AND g_game_id =7 AND g_ball_1 is not null LIMIT 1";
	if ($db->query($sql, 0)){
		$SumAmount = new SumAmountk3($numberId);
...
else 
{
	...
	//取出最近82期未結算注單
	$sql = "SELECT g_qishu FROM g_history7 WHERE g_game_id = 7 ORDER BY g_qishu DESC LIMIT 82";
...
<!--
	...
	function numberUpMen(){
		...
		if (num.length != 11) {alert("請輸期數格式錯誤！"); return;}
		if(confirm("確定恢復 第 "+num+" 期 注單嗎？")){
			$.post("/ajax/numberUpMen.php", {mid : 1,men : men, num : num, type : 7}, function(data){
				if(data != 1){
					alert("第 "+num+" 期不存在列表中或未結算，無法恢復。");
					return;
				} else {
                    ...
                    <tr>
                        ...
                        <!-- strat -->
                            ...
                            <table border="0" cellspacing="0" class="conter2" style="display:none" id="NumberList">
                            	<tr class="tr_top">
                            		<td width="250">開獎期數</td>
                                    <td width="300">開獎時間</td>
                                    <td colspan="3" width="120">開出號碼</td>
                                    <td colspan="2">總和</td>
										...
										<table border="0" cellspacing="0" cellpadding="0">
			                                 <tr>
				                                 <td class="nones" width="14" height="18"><img src="/SystemControl/ActionLib/images/edt.gif"/></td>
				                                  <td class="nones" width="30"><a href="openNumber_k3.php?UpNumcid=<?php echo$numberList[$i][0]?>">修改</a></td>
				                                  <td class="nones" width="15"><img src="/SystemControl/ActionLib/images/edit.gif"/></td>
				                                  <td class="nones" width="30"><a onmousemove="status=''" href="javascript:if(confirm('確定刪除嗎？')){location.href= 'openNumber_k3.php?Numdelid=<?php echo$numberList[$i][0]?>'}">刪除</a></td>
			                               </tr>
                           				</table>
									</td>
                            	</tr>
                        ...
                        <td width="15%">
							<table border="0" cellspacing="0" cellpadding="0">
                                 <tr>     
                                    ...
                                    <td class="nones" width="30"><a href="javascript:void(0)" onclick="crySum('openNumber_k3.php?startId=1&numId=<?php echo$NumberArr[$i]['g_qishu']?>')">結算</a></td>
                                 </tr>
                           </table>
						</td>
                    </tr>
