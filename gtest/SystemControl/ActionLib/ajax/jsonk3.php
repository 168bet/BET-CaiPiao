...
JSON;
	...
	{
		$cid = $_POST['cid'];
		$userReportInfo = new UserReportInfok3($Users, $cid);
...
JSON;
	...
	else if ($typeID == 6)
	{
		...
		$ResultList = '""';
		$error = null;
		$p = $_POST['nid'] == 1 ? "g_kaipan7" : "g_kaipan7";
				...
				if ($Users[0]['g_login_id'] ==89 || $Users[0]['g_login_id'] ==56){
					...
					if ($dayMoney < 0 || $_POST['s_money'] > $dayMoney){
						...
						if ($_POST['s_odds'] == 'LM')
						{
							...
							$odds = $db->query("SELECT g_odds7 FROM g_zhudan WHERE g_qishu='{$arr['s_number']}' AND  
						...
						if ($error == null){
							$arr['g_typeid'] = isset($_POST['cid']) ? '重慶時時彩' : '江苏骰寶(快3)';
							$userReportInfo = new UserReportInfok3($Users, 0);
...
JSON;
	...
	else if ($typeID == 8)
	{
		...
		$H = $_POST['hid'];
		$odds = $_POST['oid'];
		$sql = "UPDATE g_odds7 SET `{$H}` = '{$odds}' WHERE g_type = '{$Ball}' ";
		$db->query($sql, 2);
		echo 1;
	}
	else if ($typeID == 9)
