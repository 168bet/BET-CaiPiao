...
if ($_POST['act']=="添加") {
	...
	$result=mysql_query("select * from g_kaipan6 where g_qishu=".$_POST['nn']."  order by g_qishu");
	$row11=mysql_fetch_array($result);
	if ($row11!=""){
		echo "<script>alert('对不起！这一“期”已被开过，请重新输入!');window.history.go(-1);</script>";
		exit;
	}
	/*if(strtotime($_POST['zfbdatend']) >= mktime('21','30','0',date('m'),date('d'),date('Y')))
	{
		echo "<script>alert('对不起！已达今日最大开盘期数，请重新输入!');window.history.go(-1);</script>";
		exit;
	}*/
	$mm=$_POST['MM'];
	$dd=$_POST['dd'];
	
	$zfbdate=date('Y-m-d H:i:s',strtotime($_POST['zfbdate1'])+(300-$mm));

	$sql="INSERT INTO  g_kaipan6 set g_qishu='".$_POST['nn']."',g_kai_date='".$_POST['zfbdate1']."',g_feng_date='".$zfbdate."',g_open_date='".$_POST['zfbdatend']."',g_lock=2 ";
	...
	//		break;
	//	}
		$sql="INSERT INTO  g_kaipan6 set g_qishu='".$nn."',g_kai_date='".$zfbdate1."',g_feng_date='".$zfbdate."',g_open_date='".$zfbdatend."',g_lock=1  ";
		$exe=mysql_query($sql) or  die("数据库修改出错".$sql);

	}
	echo "<script>alert('盘口修改成功!');window.location.href='/SystemControl/ActionLib/openNumbers_pk.php';</script>";
	exit;
}
		...
		action="">
                <tr>
                  <td valign="top"><table width="100%" border="0" >
					...
					class="STYLE2"> *</span></td>
                        ...
                        <td bordercolor="#CCCCCC"><input name="dd" class="textc" id="dd" type="text" size="5" value="179" />
                          期 <span
					class="STYLE2"> *</span></td>
                      </tr>
                      <tr>
					...
					bgcolor="#D4E5F4">提前封盘时间：</td>
                        <td bordercolor="#CCCCCC"><INPUT id="MM" class="textc" value=<?php echo ($ConfigModel_c['g_close_time']*60)?> size=5 name="MM">
					...
					bgcolor="#D4E5F4">&nbsp;</td>
                      ...
                      <tr>
                        <td colspan="4" align="center"><span class="red">
                          <P>比如想添加北京赛车(PK10)的<SPAN class=style22>2012-07-07</SPAN>这一天的期数;期数：306972 ；添加：<SPAN class=style21>178</SPAN>；自动开盘时间：<SPAN class=style21>2012-03-27 09:00:00</SPAN>;开奖时间：<SPAN class=style21>2012-03-27 09:05:00</SPAN>；提前封盘时间：<SPAN class=style21>90</SPAN>；这样178期就添加完成了。</P>
                          </span></td>
                      </tr>
                    </table></td>
                </tr>
              ...
              <tr>
                <td><table id="tb"  border="0" align="center"  width="99%">
                    <tr class="tr_top">
                      <td height="28" colspan="7" align="left" nowrap="nowrap" bordercolor="cccccc" bgcolor="#D4E5F4"><input class="button_a" type="button" name="Submit2" value="一键删除" onclick="javascript:location.href='/SystemControl/ActionLib/openNumbers_pk.php?actcc=删除'" /></td>
                    </tr>
                    <tr class="tr_top">
