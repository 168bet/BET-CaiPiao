...
if ($typeId == "sumball_s")
{
	...
	$row_2 = $href == 8 ? sum_str_s_nc ($result, $href, 25, TRUE) : sum_str_s_nc ($result, $href); //1-8號球-號碼
	$row_3 = sum_str_s_nc ($result, $href, 25, FALSE, 3);	//1-8號球-大小
	$row_4 = sum_str_s_nc ($result, $href, 25, FALSE, 1);	//1-8號球-單雙
	$row_5 = sum_str_s_nc ($result, $href, 25, FALSE, 5, NULL, 0);	//1-8號球-尾數大小
	$row_6 = sum_str_s_nc ($result, $href, 25, FALSE, 7, NULL, 0);	//1-8號球-合數單雙
	$row_7 = sum_str_s_nc ($result, $href, 25, FALSE, 8);	//1-8號球-方位
	$row_8 = sum_str_s_nc ($result, $href, 25, FALSE, 9);	//1-8號球-中發白
	$row_9 = sum_str_s_nc ($result, $href, 25, FALSE, FALSE, 2, 0); 	//總和大小
	$row_10 = sum_str_s_nc ($result, $href, 25, FALSE, FALSE, 4, 0);	//總和單雙
	$row_11 = sum_str_s_nc ($result, $href, 25, FALSE, FALSE, 6, 0);	//總和尾數大小
	$row_12 = sum_str_s_nc ($result, $href, 25, TRUE);	//龍虎
	
	//雙面計算
	$num_arr = sum_ball_count_1_nc ($BallString, $BallString_nc, $result, 3); 
	arsort($num_arr);
...
if ($typeId == "sumball")
{
	...
	{
		$result = history_resultnc(0);
		$BallArr = sum_ball_count_nc ($result, $gid);
