...
else if ($tid == 2)
{
	...
	if ($result && Copyright)
	{
		...
		//取出1-8球和總和龍虎雙面賠率
		$db=new DB();
		$sql = "SELECT   h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11,h12,h13,h14,h15,h16,`g_type`  FROM `g_odds6` WHERE `g_type` = 'Ball_1' OR `g_type` = 'Ball_2'  OR `g_type` = 'Ball_3'  OR `g_type` = 'Ball_4'   OR `g_type` = 'Ball_5' ORDER BY g_id ASC ";
