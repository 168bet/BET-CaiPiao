...
else if ($tid == 2)
{
	...
	if ($result && Copyright)
	{
		...
		//取出1-8球和總和龍虎雙面賠率
		$db=new DB();
		$sql = "SELECT  `h1`, `h2`, `h3`, `h4`, `h5`, `h6`, `h7`, `h8`, `h9`, `h10`, `h11`, `h12`, `h13`, `h14`, `h15`, `h16`, `h17`, `h18`, `h19`, `h20`,`g_type` FROM `g_odds5` WHERE `g_type` = 'Ball_11' ORDER BY g_id ASC ";
		$sresult = $db->query($sql, 1);
		$sql = "SELECT `h1`, `h2`, `h3`, `h4`, `h5`, `h6`, `h7`, `h8`,`g_type` FROM `g_odds5` WHERE g_type = 'Ball_9' ORDER BY g_id ASC ";
		$eresult = $db->query($sql, 1);
		$list = array_merge( $eresult,$sresult);
		$oddsMax = 0;
		...
		for ($i=0; $i<count($list); $i++){
					switch($list[$i]['g_type']){
						case 'Ball_11':$str='g1';break;
						case 'Ball_9':$str='g9';break;
				}
			foreach ($list[$i] as $key=>$value){
				if($str!='g9')
				$arrList[$i][$key] = setoddsnc($key, $value, $ConfigModel, $user, 0,$str);
				else
				$arrList[$i][$key] = setoddsnc($key, $value, $ConfigModel, $user, 2,$str);
			}
		}
