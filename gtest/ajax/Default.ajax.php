...
if ($typeId == "action" && Copyright)
{
	...
	//$db = new DB();
	//$lh = $db->query("SELECT h7 as h36,h8 as h37 FROM `g_odds` WHERE g_type =  'Ball_9' LIMIT 1 ", 1);
	//$oArr = array_merge($oArr,$lh); 
	//判斷當前登錄帳號的盤口
	$oddsMax = 0;
...
if ($typeId == "sumball_s")
{
	...
	//雙面計算
	$num_arr = sum_ball_count_1 ($BallString, $BallString_a, $result, 3); 
	arsort($num_arr);
