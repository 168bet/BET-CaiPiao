...
$html_data = $curl->fetch_url("http://kj.13322.com/kl10_dkl10_history_dtoday.html");
//echo "ok1";exit;
...
$pattern2="#<tr><tdclass=\"tdbbstdbrs\">([\d \-\:\ ]*?)<\/td><tdclass=\"tdbbstdbrs\">([\d]*?)<\/td><tdclass=\"tdbbtdbrs\"align=\"center\"><tablewidth=\"250\"border=\"0\"cellpadding=\"0\"cellspacing=\"0\"class=\"m0a\"><tr><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><td><spanclass=\"BwibB\">([\d]*?)<\/span><\/td><\/tr><\/table>#";

preg_match_all($pattern2,$html_data,$matches1);

	//print_r($matches1);exit;
	$html_data=$matches1[0][0];
	//echo $html_data; exit;//输出代码，然后去正则软件去匹配！
...
echo "当前期号"	.$qishu."</br>";
echo "当前期号"	.$qishunum."</br>";

//print_r($num);
echo  $ball_1		=$matches2[3][0];
echo	$ball_2		=$matches2[4][0];
echo	$ball_3		=$matches2[5][0];
echo	$ball_4		=$matches2[6][0];
echo	$ball_5		= $matches2[7][0];
echo	$ball_6		=$matches2[8][0];
echo	$ball_7		=$matches2[9][0];
echo	$ball_8		=$matches2[10][0];
echo "</br>";
