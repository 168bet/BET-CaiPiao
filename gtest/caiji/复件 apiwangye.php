...
function autokj_k3($List, $_number)
{
	...
	$date = date('Y-m-d H:i');
	$db = new DB();
	if (!$db->query("SELECT `g_id` FROM `g_history3` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 9 and `g_ball_1` is  null LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history3` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 9) ", 2);
	}
	$list = $List['openResult'];
...
function getNumberList_k3 ()
{
	...
	//{
		...
		$endTime =null;
	if($_REQUEST['qs']!=""){
			$result = $db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan3` WHERE  g_qishu='".$_REQUEST['qs']."'  LIMIT 1", 1);
			if ($result)
			{
				$number = $result[0]['g_qishu'];
				$endTime = strtotime($result[0]['g_open_date']) - time();
				...
				$number = $_REQUEST['qs'];
				$endTime = 0;
				$db->query("UPDATE `g_kaipan3` SET `g_lock` = 2 WHERE g_open_date>=now() order by g_qishu LIMIT 1 ", 2);
			}
	}else{
			$db->query("delete from `g_kaipan3` WHERE g_open_date<now() ", 2);
		$result = $db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan3` WHERE `g_lock` = 2 and g_open_date>=now() ORDER BY g_qishu LIMIT 1", 1);
		if ($result)
		{
			$number = $result[0]['g_qishu'];
			$endTime = strtotime($result[0]['g_open_date']) - time();
		...
		else
		{
			...
			if(!$db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan3` WHERE `g_lock` = 2 and g_open_date>=now() ORDER BY g_qishu LIMIT 1", 1)){
...
function setNumberAndPlanning_k3 ()
{
	...
	if ($result)
	{
		...
		$lastNum = mb_substr($nums, -2);
		//將開獎時間已經結束的期數添加到歷史記錄
		if (!$db->query("SELECT `g_id` FROM `g_history3` WHERE `g_qishu` = '{$nums}' AND `g_game_id` = 9 LIMIT 1", 0))
			//$db->query("INSERT INTO `g_history3` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$nums}', '{$date}', 9) ", 2);//3如果找不到下注期数，那么把下注期数的期数跟开奖时间写入到history表里！这句就会导致出错，注释之！
		if ($lastNum < 82)
		{
			/**
			 * 開啟下一期期數
			 ...
			 */
			/* $number = $nums+1;
			if ($db->query("UPDATE `g_kaipan3` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}'  ", 2))
				$db->query("DELETE FROM `g_kaipan3` WHERE g_qishu = '{$nums}' AND `g_lock` = 2  ", 2);
		}
		exit("success123"); */
		$number=$_REQUEST['qs']+1;
			    $nums=$number-1;
				$db->query("UPDATE `g_kaipan3` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}' ", 2);
				$db->query("DELETE FROM `g_kaipan3` WHERE g_qishu <= '{$nums}'  ", 2);
		}
		$number=$_REQUEST['qs']+1;
			    $nums=$number-1;
				$db->query("UPDATE `g_kaipan3` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}' ", 2);
...
function setBall_k3($List, $_number)
{
	...
	if (!$db->query("SELECT `g_id` FROM `g_history3` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 9  LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history3` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 9) ", 2);
	}
	if ($db->query("SELECT `g_id` FROM `g_history3` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 9 and `g_ball_1` is  null LIMIT 1", 0)){
	$list = $List['openResult'];
	...
	//降賠率
	$result = $db->query("SELECT `g_open_date` FROM `g_kaipan3` WHERE DATEDIFF(`g_open_date`,CURDATE())=0 LIMIT 1 ", 1);
