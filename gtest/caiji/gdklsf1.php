...
$html_data = $curl->fetch_url("http://www.1392c.com/gdkl10/");
$html_data=trimall($html_data);
//echo $html_data; exit;//输出代码，然后去正则软件去匹配！

$pattern2="#<trclass='even'><td><pclass=\"p\">(.*?)</p><pclass=\"t\">.*?</p></td><tdclass=\"nums\"><spanclass='no1'>(.*?)</span><spanclass='no2'>(.*?)</span><spanclass='no3'>(.*?)</span><spanclass='no4'>(.*?)</span><spanclass='no5'>(.*?)</span><spanclass='no6'>(.*?)</span><spanclass='no7'>(.*?)</span><spanclass='no8'>(.*?)</span></td>#";
preg_match_all($pattern2,$html_data,$matches2);
$qishu		= $matches2[1][0];
echo "当前期号"	.$qishu."</br>";
echo "当前号码";
echo  $ball_1		= $matches2[2][0];
echo	$ball_2		= $matches2[3][0];
echo	$ball_3		= $matches2[4][0];
echo	$ball_4		= $matches2[5][0];
echo	$ball_5		= $matches2[6][0];
echo	$ball_6		= $matches2[7][0];
echo	$ball_7		= $matches2[8][0];
echo	$ball_8		= $matches2[9][0];
echo "</br>";
