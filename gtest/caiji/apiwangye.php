...
function autokj_k3($List, $_number)
{
	...
	$date = date('Y-m-d H:i');
	$db = new DB();
	if (!$db->query("SELECT `g_id` FROM `g_history7` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 7 and `g_ball_1` is  null LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history7` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 7) ", 2);
	}
	$list = $List['openResult'];
...
function autokj_k8($List, $_number)
{
	...
	$date = date('Y-m-d H:i');
	$db = new DB();
	if (!$db->query("SELECT `g_id` FROM `g_history5` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 5 and `g_ball_1` is  null LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history5` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 5) ", 2);
	}
	$list = $List['openResult'];
	$set = "`g_ball_1` = '{$list[0]}', `g_ball_2` = '{$list[1]}', `g_ball_3` = '{$list[2]}', `g_ball_4` = '{$list[3]}', `g_ball_5` = '{$list[4]}', `g_ball_6` = '{$list[5]}', `g_ball_7` = '{$list[6]}', `g_ball_8` = '{$list[7]}' ";
	$db->query("UPDATE `g_history5` SET {$set} WHERE g_qishu = '{$_number}' LIMIT 1 ", 2);
...
function getNumberList_k8 ()
{
	...
	//{
		...
		$endTime =null;
	if($_REQUEST['qs']!=""){
			$result = $db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan5` WHERE  g_qishu='".$_REQUEST['qs']."'  LIMIT 1", 1);
			if ($result)
			{
				$number = $result[0]['g_qishu'];
				$endTime = strtotime($result[0]['g_open_date']) - time();
				...
				$number = $_REQUEST['qs'];
				$endTime = 0;
				$db->query("UPDATE `g_kaipan5` SET `g_lock` = 2 WHERE g_open_date>=now() order by g_open_date LIMIT 1 ", 2);
			}
	}else{
			$db->query("delete from `g_kaipan5` WHERE g_open_date<now() ", 2);
		$result = $db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan5` WHERE `g_lock` = 2 and g_open_date>=now() ORDER BY g_open_date LIMIT 1", 1);
		if ($result)
		{
			$number = $result[0]['g_qishu'];
			$endTime = strtotime($result[0]['g_open_date']) - time();
		...
		else
		{
			...
			if(!$db->query("SELECT `g_qishu`,  `g_open_date` FROM `g_kaipan5` WHERE `g_lock` = 2 and g_open_date>=now() ORDER BY g_open_date LIMIT 1", 1)){
...
function setNumberAndPlanning_k3 ()
{
	...
	if ($result)
	{
		...
		$lastNum = mb_substr($nums, -2);
		//將開獎時間已經結束的期數添加到歷史記錄
		if (!$db->query("SELECT `g_id` FROM `g_history7` WHERE `g_qishu` = '{$nums}' AND `g_game_id` = 7 LIMIT 1", 0))
			//$db->query("INSERT INTO `g_history7` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$nums}', '{$date}', 7) ", 2);//3如果找不到下注期数，那么把下注期数的期数跟开奖时间写入到history表里！这句就会导致出错，注释之！
		if ($lastNum < 82)
		{
			/**
			 * 開啟下一期期數
...
{
	$db = new DB();
	$result = $db->query("SELECT `g_qishu`,`g_open_date` FROM `g_kaipan5` WHERE `g_lock` = 2 ORDER BY g_open_date DESC LIMIT 1 ", 1);
	...
	if ($result)
	{
		...
		$nums = $result[0]['g_qishu'];
		//將開獎時間已經結束的期數添加到歷史記錄
		if (!$db->query("SELECT `g_id` FROM `g_history5` WHERE `g_qishu` = '{$nums}' AND `g_game_id` = 5 LIMIT 1", 0))
			//$db->query("INSERT INTO `g_history5` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$nums}', '{$date}', 5) ", 2);//4如果找不到下注期数，那么把下注期数的期数跟开奖时间写入到history表里！这句就会导致出错，注释之！
		...
		if (mb_substr($result[0]['g_open_date'],-8) != '23:55:00')
		{
			 ...
			 */
				/* $number=$nums+1;
				$db->query("UPDATE `g_kaipan5` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}' ", 2);
				$db->query("DELETE FROM `g_kaipan5` WHERE g_qishu = '{$nums}' AND `g_lock` = 2  ", 2);
		}
		exit("success"); */
		$number=$_REQUEST['qs']+1;
			    $nums=$number-1;
				$db->query("UPDATE `g_kaipan5` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}' ", 2);
				$db->query("DELETE FROM `g_kaipan5` WHERE g_qishu <= '{$nums}'  ", 2);
		}
		$number=$_REQUEST['qs']+1;
			    $nums=$number-1;
				$db->query("UPDATE `g_kaipan5` SET `g_lock` = 2 WHERE `g_qishu` = '{$number}' ", 2);
				$db->query("DELETE FROM `g_kaipan5` WHERE g_qishu <= '{$nums}'  ", 2);
		echo $number."success幸运农场開啟下一期期數/刪除開獎時間已經結束的期數".$nums;
	}
}
...
function setBall_k3($List, $_number)
{
	...
	if (!$db->query("SELECT `g_id` FROM `g_history7` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 7  LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history7` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 7) ", 2);
	}
	if ($db->query("SELECT `g_id` FROM `g_history7` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 7 and `g_ball_1` is  null LIMIT 1", 0)){
	$list = $List['openResult'];
...
function setBall_k8($List, $_number)
{
	...
	$db = new DB();
	$date = date('Y-m-d H:i');
	if (!$db->query("SELECT `g_id` FROM `g_history5` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 5  LIMIT 1", 0)){
		$db->query("INSERT INTO `g_history5` (`g_qishu`, `g_date`, `g_game_id`) VALUES ('{$_number}', '{$date}', 5) ", 2);
	}
	if ($db->query("SELECT `g_id` FROM `g_history5` WHERE `g_qishu` = '{$_number}' AND `g_game_id` = 5 and `g_ball_1` is  null LIMIT 1", 0)){
	$list = $List['openResult'];
	...
	//降賠率
	$result = $db->query("SELECT `g_open_date` FROM `g_kaipan5` WHERE DATEDIFF(`g_open_date`,CURDATE())=0 LIMIT 1 ", 1);
	...
	exit("success幸运农场当前期数".$_number."跟号码写入成功");
}
/**
 * 結算報表
 * @param int 已經開獎的期數
...
function inventory_k8 ($number, $ConfigModel)
{
	...
	$db = new DB();
	$result = $db->query("SELECT `g_open_date` FROM `g_kaipan5` WHERE g_open_date>=now() LIMIT 1 ", 1);
	if (!$result)
	{
		$insertday=time()<strtotime(date("Y-m-d 23:55:00"))?0:1;
			$did = 612998;
