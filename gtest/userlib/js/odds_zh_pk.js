...
function loadResult() {
    $.post("../ajax/loadResult.php", {
        gameType: "pk"
    },
    function(data) {
        $('#history', win).html(data);
    })
}
...
function endtimes(endtime) {
    ...
    _endtime = endtime;
    if (_endtime > 1) endTime.html(settime(_endtime));
...
function opentimes(opentime) {
    ...
    _opentime = opentime;
    if (_opentime > 1) openTime.html(settime(_opentime));
    var interval = setInterval(function() {
...
function doOK(sArray){ 
	...
	$('form[name=postform]',win).attr('action','./inc/DataProcessingpk3.php');
