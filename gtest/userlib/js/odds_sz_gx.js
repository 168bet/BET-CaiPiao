var _url = "../ajax/oddsJsonsz_gx.php";
var _endtime, _opentime, _refreshtime, _openNumber, _lock=false;
var setResults = new Array();

$(function (){
	$("#dp").attr("action","./inc/DataProcessinggxsz.php?t="+encodeURI($("#tys").html()));
	loadInfo(false);
	loadTime();
	setOpnumberTirem();
...
function loadodds(oddslist, endtime, number){
	...
	for (var n=0; n<oddslist.length; n++){
		for (var i in oddslist[n]){
			odds = oddslist[n][i];
			urls = "fn5.php?v="+number+"&n="+i+"&t=t"+(n+1);
...
function loadinput(endtime){
	...
	loads.each(function(){
		count++;
		if (count>=1 && count <= 105){
			if(lock2==21) s = "t"+lock1+"_h2"+lock2;
			else
			s = "t"+lock1+"_h"+lock2;
			lock1++;
...
function nameformat(array){
	...
	switch (array[1]) {
		...
		case "h221": arr[1] = '21'; arr[2]=h+"h221"; break;
	}
	return arr;
}
