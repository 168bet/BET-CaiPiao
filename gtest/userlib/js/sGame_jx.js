var setResultjx = new Array();
(function(){
	var _hiden, _endtime, _opentime, _refreshtime, _openNumber, _lock=false;			
	$(function(){
		$("#dp").attr("action","./inc/DataProcessingjx.php?idt="+encodeURI($("#tys").html()));
		_hiden = $("#hiden").val();
	...
	function loadGameInfo(bool){
		var number = $("#number");
		var sy = $("#sy");
		$.post("/ajax/jxJson.php", { typeid : 1, mid : _hiden}, function(data){
	...
	function loadDayInfo(){
		$.post("/ajax/jxJson.php", { typeid : 2, mid : _hiden}, function(data){
	...
	function loadodds(oddslist, endtime, number){
		...
		for (var n=0; n<oddslist.length; n++){
			for (var i in oddslist[n]){
				odds = oddslist[n][i];
				urls = "fn6.php?tid="+bc(a[n])+"&numberid="+number+"&hid="+a[n]+i;
	...
	function setOpnumberTirem(){
		...
		if (opnumber != ""){
			...
			if (sum == 2 || sum == 882) {
				var interval = setInterval(function(){
					$.post("/ajax/jxJson.php", {typeid : 3}, function(data){
	...
	function openNumberCount(row, bool){
		...
		setResultjx[0] = row.row2;
		setResultjx[1] = row.row3;
		setResultjx[2] = row.row4;
		setResultjx[3] = row.row5;
		setResultjx[4] = row.row6;
		setResultjx[5] = row.row7;
	}
...
function stringByInt (str){
	if (str == "第1球" || str == "第2球" || str == "第3球" || str == "第4球" || str == "第5球")
		return setResultjx[0];
	switch (str){
		case "大小" : return setResultjx[1];
		case "單雙" : return setResultjx[2];
		case "總和大小" : return setResultjx[3];
		case "總和單雙" : return setResultjx[4];
		case "龍虎和" : return setResultjx[5];
	}
}
...
function submitforms(){
	...
	if (confirm(confrims)){
		input.val("");
		var number = $("#o").html();
		var s_type = '<input type="hidden" name="s_jx" value="'+ss+'"><input type="hidden" name="s_number" value="'+number+'">';
