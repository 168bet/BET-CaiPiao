var _url = "../ajax/jxoddsJsonsz.php";
var _endtime, _opentime, _refreshtime, _openNumber, _lock=false;
var setResultjx = new Array();
$(function (){
	$("#dp").attr("action","./inc/DataProcessingjxsz.php?t="+encodeURI($("#tys").html()));
	loadInfo(false);
	loadTime();
	setOpnumberTirem();
...
function openNumberCount(row, bool){
		...
		setResultjx[0] = row.row1;
		setResultjx[1] = row.row2;
		setResultjx[2] = row.row3;
		setResultjx[3] = row.row4;
		setResultjx[4] = row.row5;
		setResultjx[5] = row.row6;
	}
	...
	var a = ["a","b","c","d","e","f"];
		...
		for (var n=0; n<oddslist.length; n++){
			for (var i in oddslist[n]){
				odds = oddslist[n][i];
				urls = "fnszjx.php?tid="+bc(a[n])+"&numberid="+number+"&hid="+a[n]+i;
...
function submitforms(){
	...
	if (confirm(confrims)){
		input.val("");
		var number = $("#o").html();
		var s_type = '<input type="hidden" name="s_jx" value="'+sArray+'"><input type="hidden" name="s_number" value="'+number+'">';
...
function stringByInt (str){
	switch (str){
		case "第1球" : return setResultjx[1];
		case "第2球" : return setResultjx[2];
		case "第3球" : return setResultjx[3];
		case "第4球" : return setResultjx[4];
		case "第5球" : return setResultjx[5];
	}
}
