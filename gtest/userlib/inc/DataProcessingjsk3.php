...
$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_k3_game_lock`");
...
if ($ConfigModel['g_k3_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
...
if (true)
{
	...
	if ($action == 'fn')
	{
		...
		switch($types){
			case '三軍' : $ty="Ball_th_j";break;
			case '圍骰' : $ty="Ball_w_s";break;
			case '點數' : $ty="Ball_d_s";break;
			case '長牌' : $ty="Ball_l_p";break;
			case '短牌' : $ty="Ball_d_p";break;			
			}										//下注遊戲玩法
		...
		$result = GetUserXianErk3 ($types, $number_2, $user[0]['g_name']); 	//當前用戶退水列表
		$max = GetUser_s_k3 ($result, $user,$types,$number_2);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		$is_Number = isNumberk3($types, $number_2, $number_1); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
		...
		$h = $_POST['hid'];
		$odds = GetOddsk3 ($types, $h); 														//賠率
		
		$odds = setoddsk3($h, $odds, $user, 0,$ty);
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistributionk3 ($user, $money, $number_2, $types);
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumberk3($sm_arr[$i][0], $sm_arr[$i][1], $number_1);
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianErk3 ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']); 	//當前用戶退水列表
			$max = GetUser_s_k3 ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1]);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
				$winm=$user[0]['g_money_yes']-$user[0]['g_money'];
				if($max['HuiYuan_XianH']!=0){		
		if($max['HuiYuan_XianH']<$winm)
		exit(alert_href("抱歉！您的限红額度為".$max['HuiYuan_XianH'], '../left.php'));	
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddsk3 ($sm_arr[$i][0], $sm_arr[$i][1]); 														//賠率
			
			$hv = sresultk3 ($sm_arr[$i][1]);
			...
			case '長牌' : $ty="Ball_l_p";break;
			case '短牌' : $ty="Ball_d_p";break;
			case '圍骰、全骰' : $ty="Ball_w_s";break;
			}	

			
			$odds = setoddsk3($hv, $odds, $user, 0,$ty);
			...
			$DtnArray = SumRankDistributionk3 ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0]);
			/* 退水修正*/
			$result = GetUserXianErk3 ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
