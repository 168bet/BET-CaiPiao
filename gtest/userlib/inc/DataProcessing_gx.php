...
$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_gx_game_lock`, `g_odds_ratio_gx_b1`,`g_odds_ratio_gx_b2`,`g_odds_ratio_gx_b3`,`g_odds_ratio_gx_b4`,`g_odds_ratio_gx_b5`,`g_odds_ratio_gx_c1`,`g_odds_ratio_gx_c2`,`g_odds_ratio_gx_c3`,`g_odds_ratio_gx_c4`,`g_odds_ratio_gx_c5`");
...
if ($ConfigModel['g_gx_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
...
if (true)
{
	...
	if ($action == 'fn')
	{
		...
		$result = GetUserXianErgx ($types, $number_2, $user[0]['g_name']); 	//當前用戶退水列表
		$max = GetUser_s_gx ($result, $user,$types, $number_2);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		$is_Number = isNumbergx($types, $number_2, $number_1); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
			exit(alert_href('抱歉！第 '.$number_1.' 期已經封盤', '../left.php'));
		if (!$is_Number) 
			exit("NumberError4"); 																	//驗證號碼是否非法提交
		isUserMoney ($money, $max,$money); 															//驗證下注金額是否大於可用金額
		$countBiShu = 1;																				//總筆數
		$countZhuEr = $money;																	//總注額
		$h = $_POST['hid'];
		$odds = GetOddsgx ($types, $h); 														//賠率
		if ($types=='總和、龍虎'){
			$odds = setoddsgx($h, $odds, $ConfigModel, $user, 1);
		} else {
			$odds = setoddsgx($h, $odds, $ConfigModel, $user, 0);
		}
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistributiongx ($user, $money, $number_2, $types);
	...
	else if ($action == 'fn1') //alert提交
	{
		...
		for ($i=0; $i<count($s_money_arr); $i++)
		{
			...
			$is_Number = isNumbergx($s_type, $s_ball_arr[$i], $s_number);
			if ($is_Number === 2)
				exit(alert_href('抱歉！第 '.$s_number.' 期已經封盤', '../left.php'));
			if (!$is_Number)
				exit("NumberError1".$s_type.'----'.$s_ball_arr[$i].'----'.$s_number);
		...
		for ($i=0; $i<count($s_ball_arr); $i++)
		{
			switch ($s_ball_arr[$i])
			{
			case '總和大':$s_type='總和、龍虎'; break;
			case '總和單':$s_type='總和、龍虎'; break;
			case '總和小':$s_type='總和、龍虎'; break;
			case '總和雙':$s_type='總和、龍虎'; break;
			case '總和尾大':$s_type='總和、龍虎'; break;
			case '龍':$s_type='總和、龍虎'; break;
			case '總和尾小':$s_type='總和、龍虎'; break;
			case '虎':$s_type='總和、龍虎'; break;
			}
			//exit("NumberError1".$s_type.'----'.$s_ball_arr[$i].'----'.$s_number);
			$result = GetUserXianErgx ($s_type, $s_ball_arr[$i], $user[0]['g_name']); 				//當前用戶退水列表
			$max = GetUser_s_gx ($result, $user,$s_type, $s_ball_arr[$i]);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		exit(alert_href("抱歉！您的限红額度為".$max['HuiYuan_XianH'], '../left.php'));	
			...
			$odds = GetOddsgx ($s_type, $s_hid_arr[$i]); 														//賠率
			if ($s_type=='總和、龍虎'){
				$odds = setoddsgx($s_hid_arr[$i], $odds, $ConfigModel, $user, 1);
			} else {
				$odds = setoddsgx($s_hid_arr[$i], $odds, $ConfigModel, $user, 0);
			}
			...
			$DtnArray = SumRankDistributiongx ($user, $s_money_arr[$i], $s_ball_arr[$i], $s_type);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
	...
	else if ($action == 'fn2') //連碼提交
	{
		...
		for ($i=0; $i<count($s_ball_arr); $i++)
		{
			...
			if (!$is_Number) exit("NumberError2");
		}
		$hi = 'h'.trim(strtr($s_type, "t"," "));
		$stringList = GetGameType_gx($s_type);
		$results = subArr_gx ($s_ball_arr, $stringList['count']); //復式計算、返回值、【總組數】、【總個數】、【號碼個數】
		$number_1 = $s_number; //期數
		$countBiShu = 1; //總筆數
		$countZhuEr = $results[0] * $s_money; //總下注金額
		$odds = GetOddsgx ('連碼', $hi); //賠率
		$odds = setoddsgx($hi, $odds, $ConfigModel, $user, 2);
		
		$result = GetUserXianErgx($stringList['type'], null, $user[0]['g_name']); 				//當前用戶退水列表
		$max = GetUser_s_gx ($result, $user,$stringList['type'],null);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		$DtnArray = SumRankDistributiongx ($user, $s_money, null, $stringList['type']);
		$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[0]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumbergx($sm_arr[$i][0], $sm_arr[$i][1], $number_1);
			if ($is_Number === 2) exit(alert_href('抱歉！第 '.$number_1.' 期已經封盤', '../left.php'));
			if (!$is_Number) exit("NumberError3");
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianErgx ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']); 	//當前用戶退水列表
			$max = GetUser_s_gx ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1]);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddsgx ($sm_arr[$i][0], $sm_arr[$i][1]); 														//賠率
			$hv = sresult ($sm_arr[$i][1]);
			if ($sm_arr[$i][0]=='總和、龍虎'){
				$odds = setoddsgx($hv, $odds, $ConfigModel, $user, 1);
			} else {
				$odds = setoddsgx($hv, $odds, $ConfigModel, $user, 0);
			}
			$ListArr[$i]['g_s_nid'] = $user[0]['g_nid'];															//外鍵
			$ListArr[$i]['g_mumber_type'] = $user[0]['g_mumber_type'];					//會員類型
			$ListArr[$i]['g_nid'] = $user[0]['g_name'];															//帳號
			...
			$DtnArray = SumRankDistributiongx ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0]);
			/* 退水修正*/
			$result = GetUserXianErgx ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
...
new AutoLet($number_1, $ListArr,3);
}
else
{
