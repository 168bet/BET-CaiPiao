...
$ConfigModel= configModel("`g_mix_money`, `g_web_lock`,`g_nc_game_lock`, `g_odds_ratio_nc_b1`,`g_odds_ratio_nc_b2`,`g_odds_ratio_nc_b3`,`g_odds_ratio_nc_b4`,`g_odds_ratio_nc_b5`,`g_odds_ratio_nc_c1`,`g_odds_ratio_nc_c2`,`g_odds_ratio_nc_c3`,`g_odds_ratio_nc_c4`,`g_odds_ratio_nc_c5`");
...
if (isset($_SESSION['guid_code']))
{
	...
	if ($action == 'fn')
	{
		...
		$result = GetUserXianErnc ($types, $number_2, $user[0]['g_name']); 	//當前用戶退水列表
		$max = GetUser_s_nc ($result, $user,$types,$number_2);													//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
		...
		$is_Number = isNumbernc($types, $number_2, $number_1); 				//驗證提交的期數是否已經封盤
		if ($is_Number === 2) 
		 ...
		 * 重寫個級別退水及占成
		 */
		$DtnArray = SumRankDistributionnc ($user, $money, $number_2, $types);
	...
	else if ($action == 'fn1') //alert提交
	{
		...
		{
			$result = GetUserXianErnc ($s_type, $s_ball_arr[$i], $user[0]['g_name']); 				//當前用戶退水列表
			$max = GetUser_s_nc ($result, $user,$s_type,$s_ball_arr[$i]);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
				$winm=$user[0]['g_money_yes']-$user[0]['g_money'];
				if($max['HuiYuan_XianH']!=0){		
		if($max['HuiYuan_XianH']<$winm)
		exit(alert_href("抱歉！您的限红額度為".$max['HuiYuan_XianH'], '../left.php'));	
			...
			$odds = GetOddsnc ($s_type, $s_hid_arr[$i]); 														//賠率
			if ($s_type=='總和、龍虎'){
			...
			$DtnArray = SumRankDistributionnc ($user, $s_money_arr[$i], $s_ball_arr[$i], $s_type);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
	...
	else if ($action == 'fn2') //連碼提交
	{
		...
		$odds = GetOddsnc ('連碼', $hi); //賠率
		$odds = setoddsnc($hi, $odds, $ConfigModel, $user, 2);
		
		$result = GetUserXianErnc ($stringList['type'], null, $user[0]['g_name']); 				//當前用戶退水列表
		$max = GetUser_s_nc ($result, $user,$stringList['type'],null);																	//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
			$winm=$user[0]['g_money_yes']-$user[0]['g_money'];
			if($max['HuiYuan_XianH']!=0){		
		if($max['HuiYuan_XianH']<$winm)
		exit(alert_href("抱歉！您的限红額度為".$max['HuiYuan_XianH'], '../left.php'));	
		...
		$DtnArray = SumRankDistributionnc ($user, $s_money, null, $stringList['type']);
		$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[0]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
	...
	else if ($action == 'fn3') //雙面提交
	{
		...
		for ($i=0; $i<count($sm_arr); $i++){
			if (!Matchs::isNumber($sm_arr[$i][3]))
				exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
			$is_Number = isNumbernc($sm_arr[$i][0], $sm_arr[$i][1], $number_1);
		...
		$max = null;
		for ($i=0; $i<count($sm_arr); $i++){
			$result = GetUserXianErnc ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']); 	//當前用戶退水列表
			$max = GetUser_s_nc ($result, $user,$sm_arr[$i][0], $sm_arr[$i][1]);																			//當前用戶、單注限額、單號限額、單號已下、 單期限額、單期已下
				$winm=$user[0]['g_money_yes']-$user[0]['g_money'];	
				if($max['HuiYuan_XianH']!=0){	
		if($max['HuiYuan_XianH']<$winm)
		exit(alert_href("抱歉！您的限红額度為".$max['HuiYuan_XianH'], '../left.php'));	
		...
		for ($i=0; $i<count($sm_arr); $i++){
			$odds = GetOddsnc ($sm_arr[$i][0], $sm_arr[$i][1]); 														//賠率
			$hv = sresult ($sm_arr[$i][1]);
			if ($sm_arr[$i][0]=='總和、家禽野兽'){
			...
			$DtnArray = SumRankDistributionnc ($user, $sm_arr[$i][3], $sm_arr[$i][1], $sm_arr[$i][0]);
			/* 退水修正*/
			$result = GetUserXianErnc ($sm_arr[$i][0], $sm_arr[$i][1], $user[0]['g_name']);
			$g_pan=$user[0]['g_panlu'];
		if($g_pan=="A"){
		$ListArr[$i]['g_tueishui'] = $result[0]['g_panlu_a']; 		//會員退水
		}
...
<form id="dp" action="" method="post">
  <table border="0" cellpadding="0" cellspacing="1" class="t_list" width="230">
    ...
    <tr>
      <td class="t_td_but" colspan="2" align="center"><input type="button" value="打印" onclick="window.print()" class="inputq" />
        <input type="button" value="返回" onclick="location.href='../left.php'" class="inputq"  />
      </td>
    </tr>
    <tr>
      <td class="t_td_text" colspan="2"><span class="captions"><?php echo $number_1?>期</span>
        <?php 
