...
global $user, $UserOut, $stratGamejx, $endGamejx;
$dateTime = date('Y-m-d H:i:s');
$a = date('Y-m-d ').'01:55:00';
if ( ($dateTime < $stratGamejx && $dateTime > $a) || $dateTime > $endGamejx)
{
	back('開盤時間為：'.$stratGamejx.'--'.$endGamejx);exit;
}

$ConfigModel =configModel("`g_web_lock`, `g_jx_game_lock`,`g_odds_ratio_jx_b1`,`g_odds_ratio_jx_b2`,`g_odds_ratio_jx_b3`,`g_odds_ratio_jx_c1`,`g_odds_ratio_jx_c2`,`g_odds_ratio_jx_c3`");

if ($ConfigModel['g_jx_game_lock'] !=1 || $ConfigModel['g_web_lock'] !=1)
...
$s_jx = explode('|', $_POST['s_jx'], -1);

for ($i=0; $i<count($s_jx); $i++)
{
	$s_jx[$i] = explode(',', $s_jx[$i]);
	
}

for ($i=0; $i<count($s_jx); $i++)
{
	if (!Matchs::isNumber($s_jx[$i][2]))
		exit(alert_href('抱歉！您的下注金額錯誤。', '../left.php'));
	isNumberjx($s_jx[$i][0], $s_number);
	$countZhuEr += $s_jx[$i][2];
	$countBiShu ++;
}
...
{	
	$c =tsz($s_jx[$i]);
	$result = GetUserXianErjx($c, $user[0]['g_name']);
	$ccd=isBallTypejxsz($s_jx[$i][0],$s_jx[$i][1]);
	$max = GetUser_s_jx($result, $user,$ccd[0], $ccd[1], true);
	...
	for ($k=0; $k<count($s_jx); $k++)
	{
	isUserMoney($s_jx[$k][2], $max,$countZhuEr);
	}
}
...
for ($i=0; $i<count($s_jx); $i++)
{
	...
	$a = mb_substr($s_jx[$i][1], 1, mb_strlen($s_jx[$i][1]));
	$odds = GetOddsjx($s_jx[$i][0], $a);
	if ($s_jx[$i][0] == 'Ball_1' || $s_jx[$i][0] == 'Ball_2' || $s_jx[$i][0] == 'Ball_3' || $s_jx[$i][0] == 'Ball_4' || $s_jx[$i][0] == 'Ball_5')
	{
		if ($a != 'h11' &&$a != 'h12' &&$a != 'h13' &&$a != 'h14' && Copyright)
			$odds = setoddsjx($a, $odds, $ConfigModel, $user, 0,$s_jx[$i][0]);
		else 
			$odds = setoddsjx($a, $odds, $ConfigModel, $user, 1);
	}
	else if ($s_jx[$i][0] == 'Ball_6')
	{
		$odds = setoddsjx($a, $odds, $ConfigModel, $user, 1,$s_jx[$i][0]);
	}
	else if ($s_jx[$i][0] == 'Ball_7' || $s_jx[$i][0] == 'Ball_8' || $s_jx[$i][0] == 'Ball_9')
	{
		$odds = setoddsjx($a, $odds, $ConfigModel, $user, 2,$s_jx[$i][0]);
	}
	...
	$s = isBallTypejxsz($s_jx[$i][0], $s_jx[$i][1]);
	...
	$DtnArray = SumRankDistribution ($user, $s_jx[$i][2], null, $c, true);
...
new AutoLet($s_number, $ListArr, 4);
