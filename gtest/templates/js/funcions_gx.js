...
var URL = "../ajax/Default.ajax_gx.php";
...
setAction[3] = function(lock, odds) { //賠率
	...
	if (setTime[0] > 0){
		...
		Oclass.each( function () {
			...
			switch ($thisId)
			{
			case 'ah1':tysId='t9';$thisId='h1'; break;
			case 'ah2':tysId='t9';$thisId='h2';  break;
			case 'ah3':tysId='t9';$thisId='h3';  break;
			case 'ah4':tysId='t9';$thisId='h4';  break;
			case 'ah5':tysId='t9';$thisId='h5';  break;
			case 'ah6':tysId='t9';$thisId='h6';  break;
			case 'ah7':tysId='t9';$thisId='h7';  break;
			case 'ah8':tysId='t9';$thisId='h8';  break;
			}
			if (_href == "k2"){
				...
				$(this).html("<a id=\""+clas+"\" href=\"fn5.php?v="+nunId+"&n="+$thisId+"&t="+tysId+"\" target=\"leftFrame\" class=\"bgh\">"+$odds+"</a>");
		});
	}
};
...
setAction[7] = function () { //兩面長龍
	...
	$.post (URL, {typeid : "sumball_s", href : _href}, function (data) {
		...
		setResult[5] = data.row_7; //神奇快乐
		setResult[6] = data.row_8; //红蓝绿
	...
	if ($t == "第一球" || $t == "第二球" || $t == "第三球" || $t == "第四球" || $t == "特码" || $t == "第六球" || $t == "第七球" || $t == "第八球"){
		if ($n.length <=1) {
			...
			switch ($n) {
				...
				case "29": result = '神'; break;
				case "30": result = '奇'; break;
				case "31": result = '快'; break;
				case "32": result = '乐'; break;
				case "33": result = '红'; break;
				case "34": result = '蓝'; break;
				case "35": result = '绿'; break;
				case "a1": result = '總和大'; break;
				case "a2": result = '總和單'; break;
				case "a3": result = '總和小'; break;
				case "a4": result = '總和雙'; break;
				case "a5": result = '總和尾大'; break;
				case "a6": result = '龍'; break;
				case "a7": result = '總和尾小'; break;
				case "a8": result = '虎'; break;
				default : result = $n;
			}
		}
	}
...
{
	if (str == "第1球" || str == "第2球" || str == "第3球" || str == "第4球" || str == "特码" || str == "第6球" || str == "第7球" || str == "第8球")
		return setResult[0];
	switch (str){
		...
		case "神奇快乐" : return setResult[5];
		case "红蓝绿" : return setResult[6];
...
function sodds (id, odds)
{
	...
	switch (id)
	{
		...
		case "ah1" : r = odds[1].h1; break;
		case "ah2" : r = odds[1].h2; break;
		case "ah3" : r = odds[1].h3; break;
		case "ah4" : r = odds[1].h4; break;
		case "ah5" : r = odds[1].h5; break;
		case "ah6" : r = odds[1].h6; break;
		case "ah7" : r = odds[1].h7; break;
		case "ah8" : r = odds[1].h8; break;
	}
	return r;
}
...
function postodds (data) 
{
	...
	if (cou <= 0) {
		...
		for (var i = 0; i < tArr.length; i++) {
			...
			switch (result)
			{
			case '總和大':tys='總和、龍虎'; break;
			case '總和單':tys='總和、龍虎'; break;
			case '總和小':tys='總和、龍虎'; break;
			case '總和雙':tys='總和、龍虎'; break;
			case '總和尾大':tys='總和、龍虎'; break;
			case '龍':tys='總和、龍虎'; break;
			case '總和尾小':tys='總和、龍虎'; break;
			case '虎':tys='總和、龍虎'; break;
			}
			if (tys == "總和、龍虎"){
		...
		if (confirm(p)) {
			var oid = $("#o").html();
			var s_type = '<input type="hidden" name="s_type" value="'+tys1+'"><input type="hidden" name="s_number" value="'+oid+'">';
