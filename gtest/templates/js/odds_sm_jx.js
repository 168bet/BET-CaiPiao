var _url = "../ajax/jxoddsJson.php";
var _endtime, _opentime, _refreshtime, _openNumber, _lock=false;
var setResultjx = new Array();
$(function (){
	$("#dp").attr("action","./inc/DataProcessingjxsm.php?t="+encodeURI($("#tys").html()));
	loadInfo(false);
	loadTime();
	setOpnumberTirem();
	...
	var a = ["a","b","c","d","e","f"];
		...
		for (var n=0; n<oddslist.length; n++){
			for (var i in oddslist[n]){
				odds = oddslist[n][i];
				urls = "fn7.php?tid="+bc(a[n])+"&numberid="+number+"&hid="+a[n]+i;
...
function stringByInt (str){
	switch (str){
		...
		case "總和尾數大小" : return setResultjx[3];
		case "龍虎" : return setResultjx[5];
	}
}
