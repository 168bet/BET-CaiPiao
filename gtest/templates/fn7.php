...
$a = mb_substr($s_jx[0][1], 1, mb_strlen($s_jx[0][1]));
$odds = GetOddsjx($s_jx[0][0], $a);
$ConfigModel =configModel("`g_max_money`,`g_mix_money`,`g_odds_ratio_jx_b1`,`g_odds_ratio_jx_b2`,`g_odds_ratio_jx_b3`,`g_odds_ratio_jx_c1`,`g_odds_ratio_jx_c2`,`g_odds_ratio_jx_c3`");
if ($s_jx[0][0] == 'Ball_1' || $s_jx[0][0] == 'Ball_2' || $s_jx[0][0] == 'Ball_3' || $s_jx[0][0] == 'Ball_4' || $s_jx[0][0] == 'Ball_5')
{
	if ($a != 'h11' &&$a != 'h12' &&$a != 'h13' &&$a != 'h14')
		$odds = setoddsjx($a, $odds, $ConfigModel, $user, 0,$s_jx[0][0]);
	else 
		$odds = setoddsjx($a, $odds, $ConfigModel, $user, 1);
}
else if ($s_jx[0][0] == 'Ball_6')
{
	$odds = setoddsjx($a, $odds, $ConfigModel, $user, 1,$s_jx[0][0]);
}
else if ($s_jx[0][0] == 'Ball_7' || $s_jx[0][0] == 'Ball_8' || $s_jx[0][0] == 'Ball_9')
{
	$odds = setoddsjx($a, $odds, $ConfigModel, $user, 2,$s_jx[0][0]);
}

$c =tjxsm($s_jx[0]);
$result = GetUserXianErjx($c, $user[0]['g_name']);
$ccd=isBallType($s_jx[0][0],$s_jx[0][1]);
...
$gMoney = $max['KeYongEr'];
$s = isBallTypejxsm($s_jx[0][0], $s_jx[0][1]);
?>
...
<script type="text/javascript" src="./js/sGames_jx.js"></script>
<style type="text/css">
body {background-color:#FFEFE2}
</style>
</head>
<body>
<form id="dp" action="inc/DataProcessingjxsm.php" method="post" onsubmit = "return submitformsjx()">
	...
	<input type="hidden" name="s_number" value="<?php echo $number?>" />
                    ...
                    <tr>
                    	<td class="t_td_but" colspan="2" align="center">
                            ...
                            <input type="hidden" name="s_jx" class="actiionn" value="<?php echo$s_jx[0][0].','.$s_jx[0][1]?>" />
                        </td>
                    </tr>
                </table>
                </form>
