...
{ //子帳號
	...
	if ($Users[0]['g_s_out'] == 0) {
			...
			$sql = "UPDATE `g_relation_user` SET `g_url` ='{$url}' WHERE `g_s_name`='{$_SESSION['sName']}' LIMIT 1 ";
			$db->query($sql, 2);
		}
} 
else 
{
	...
	if ($Users[0]['g_login_id'] != 89) {
		...
		if ($Users[0]['g_out'] == 0) {
			...
			$db->query($sql, 2);
			$url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
			$sql = "UPDATE `g_rank` SET `g_url` ='{$url}' WHERE `g_name`='{$_SESSION['sName']}' LIMIT 1 ";
			$db->query($sql, 2);
		}
	} else {
		if (!$db->query("SELECT g_name FROM g_manage WHERE g_uid = '{$uid}' LIMIT 1 ", 0)){
			...
			$sql = "UPDATE `g_manage` SET `g_out` =1, `g_count_time`=now() WHERE `g_name`='{$_SESSION['sName']}' LIMIT 1 ";
			$db->query($sql, 2);
			$url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
			$sql = "UPDATE `g_manage` SET `g_url` ='{$url}' WHERE `g_name`='{$_SESSION['sName']}' LIMIT 1 ";
			$db->query($sql, 2);
		}
	}
}
