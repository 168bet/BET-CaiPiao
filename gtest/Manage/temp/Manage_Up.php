...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_GET['cid']))
{
	...
	if ($memberModel)
	{
		...
		if( ($_s_money-$s_money) > $h_money){
		   exit(back('當前會員可回收餘額为：'.$h_money));
		}else{
          ...
          <tr>
            ...
            <td class="c"><!-- strat -->
              <table border="0" cellspacing="0" class="conter">
                ...
                <tr style="height:28px">
                  <td class="bj">信用額度</td>
                  <td class="left_p5"><input class="textb" name="s_money" id="s_money" value="<?php echo $memberModel[0]['g_money']?>"  maxlength="7" value="0" onkeyup="DX(this.value);" />
				 <span class="red" style="font-weight:bold" name="rmb" id="rmb"> <?php echo toCNcap($memberModel[0]['g_money']);?></span>
                    『<span class="red">&nbsp;餘額&nbsp;<span id='money_ky'><?php echo$validMoneys?></span>&nbsp;</span>』
				    『<span class="red">&nbsp;可"回收"餘額&nbsp;<span id='money_hsje' name='money_hsje'><?php echo $memberModel[0]['g_money']>$validMoneys? $validMoneys:$memberModel[0]['g_money'];?></span>&nbsp;</span>』
					<input type="hidden"  id='money_hs' name='money_hs' value="<?php echo $memberModel[0]['g_money']>$validMoneys? $validMoneys:$memberModel[0]['g_money'];?>" />
					 </td>
                </tr>
				 <tr>
                  <td class="bj">限红額度</td>
