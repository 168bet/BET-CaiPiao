...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	for ($i=0; $i<count($List); $i++){
		if ($List[$i]['g_money'] != $result[$i]['g_money'] || $List[$i]['g_lock'] != $result[$i]['g_lock']){
			...
			}else if($result[$i]['g_game_id'] == 4){
			$valueList['g_up_type'] ='江西時時彩【'.$result[$i]['g_type'].'】';
			}else if($result[$i]['g_game_id'] == 5){
				$valueList['g_up_type'] = '幸运农场【'.$result[$i]['g_type'].'】';
			}else if($result[$i]['g_game_id'] ==6){
				$valueList['g_up_type'] = '北京赛车【'.$result[$i]['g_type'].'】';
			}else if($result[$i]['g_game_id'] ==7){
				$valueList['g_up_type'] = '江苏骰寶(快3)【'.$result[$i]['g_type'].'】';
			}else {
                            ...
                            <table border="0" cellspacing="0" class="t_odds" width="49%">
							 ...
							 <table border="0" cellspacing="0" class="t_odds" width="49%" style="display:none">
                                ...
                                <tr id="p<?php echo$n?>" align="center" style="height:25px">
                                    ...
                                    <td>&nbsp;</td>
                                    <td><?php echo$mix?></td>
                                    <td><?php echo$ConfigModel['g_mix_money']?>
                                    <input class="inp1" name="money<?php echo$n?>2" id="money<?php echo$n?>2" maxlength="9" type="text" value="<?php echo$result[$i]['g_money']?>"  disabled="disabled" /></td>
