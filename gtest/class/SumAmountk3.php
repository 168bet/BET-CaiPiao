...
class SumAmountk3 
{
		...
		for ($i=0; $i<count($result); $i++)
		{
			...
			if ($result[$i]['g_result'] == 'LM'&& Copyright)
			{
				...
				if ($result[$i]['g_mingxi_2_str']&& Copyright){
					//寫入MONEY是不用在-本金
					if($result[$i]['g_mingxi_1']=='三軍'){
					$money = $result[$i]['g_mingxi_2_str'] * $result[$i]['g_jiner'] * ($result[$i]['g_odds']-1) + $_tuiShui;
					$result[$i]['g_win'] = $money ;
					$money = $money + $a;
					}else{
	...
	private function Formula ()
	{
		...
		if ($numberList&& Copyright)
		{
			...
			{
				if ((isString($resultList[$i]['g_mingxi_2']) && $resultList[$i]['g_result'] <=20&& Copyright && $resultList[$i]['g_mingxi_1']=='點數')||$resultList[$i]['g_mingxi_1']=='點數')
				{
					...
					if(strlen($resultList[$i]['g_mingxi_2'])>1)
					$resultList[$i]['g_result'] = $resultList[$i]['g_result'].'點' == $resultList[$i]['g_mingxi_2'] ? '贏' : '輸';
					else
	...
	private function ResultCorrespond ($numberList, $resultList, $param=0)
	{
		if ($param == 0&& Copyright)
		{
			for ($i=0; $i<count($resultList); $i++)
			{
				...
				{
					case '點數' :$resultList[$i]['g_result'] = $numberList[0]['g_ball_1']+$numberList[0]['g_ball_2']+$numberList[0]['g_ball_3'];break;
		...
		else if ($param == 1&& Copyright)
		{
			...
			{
				if($_numberList[0]==$_numberList[1] && $_numberList[0]==$_numberList[2]){
				$resultList ='通吃';
				}else
				$resultList = sum_ball_string_k3($_numberList[0]+$_numberList[1]+$_numberList[2], 3);
			}
			else if ($resultList['g_mingxi_2'] == '全骰')
			{
				$param = $this-> quan($numberList);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
			
			//連碼 g_result 返回值不變，直接在 $resultList['g_mingxi_2_str'] 中返回 【中】的注數、交予其他函數處理
			else if ($resultList['g_mingxi_1'] == '三軍'&& Copyright)
			{
				$result = $this->SumLM3J ($numberList, $resultList, 1);
				$resultList[0] =$result[0];
				$resultList[1] = $result[1];
			}
			else if ($resultList['g_mingxi_1'] == '長牌')
			{
				$result = $this->SumLM ($numberList, $resultList, 2);
				$resultList[0] =$result[0];
				$resultList[1] = $result[1];
			}
			else if ($resultList['g_mingxi_1'] == '圍骰'&& Copyright)
			{
				$param = $this->allsame($numberList, $resultList);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
			else if ($resultList['g_mingxi_1'] == '圍骰、全骰'&& Copyright)
			{
				$param = $this->allsame($numberList, $resultList);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
			else if ($resultList['g_mingxi_1'] == '短牌'&& Copyright)
			{
				$param = $this->DuanP ($numberList, $resultList, 2);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
			else 
		...
		//$this->allsame($numberList, $resultList);
		return $resultList;
	}
	...
	private function SumLM ($numberList, $resultList, $count, $bool=true)
	{
		$nArray = array();
		$result = array();
		if(stristr($resultList['g_mingxi_2'], ','))
	...
	private function SumLM3J ($numberList, $resultList, $count, $bool=true)
	{
		$nArray = array();
		$result = array();
		$eArray = explode(',', $resultList['g_mingxi_2']);
	...
	private function SumLM13 ($_numberList, $resultList, $index)
	{
		$userNum = explode(',', $resultList['g_mingxi_2']);
		$userNum = $this->subNumber($userNum, $index[3]);
		$param = 0;
		...
		for ($i=0; $i<count($stringArr); $i++) //開獎號碼循環
		{
			for ($n=0; $n<count($userNum); $n++) //會員號碼循環體
			{
				...
				for ($l=0; $l<count($userNum[$n]); $l++)
				{
					...
					$count += $this->MaxFor ($stringArr[0], $userNum[$n][$l]);
					//print_r($count.'=='. $index[2].'--');
					if ($count == 0&& Copyright)
						break;
					else if ($count == $index[2])
						$param += 1;
	...
	private function DuanP ($_numberList, $resultList, $count){
		
		if(stristr($resultList['g_mingxi_2'], '+')){
			$temp = explode('+', $resultList['g_mingxi_2']);
			$resultList['g_mingxi_2'] = $temp[0];	
		}
		$param = 0;
		$ball_1=$_numberList['g_ball_1'];
		$ball_2=$_numberList['g_ball_2'];
		$ball_3=$_numberList['g_ball_3'];
		if ($ball_1==$ball_2&&$ball_2!=$ball_3){
			if($ball_1==$resultList['g_mingxi_2'])  $param+=1;		
		}  
		if ($ball_1!=$ball_2&&$ball_2==$ball_3) {
			if($ball_2==$resultList['g_mingxi_2'])  $param+=1;
		}
		return $param;
	}
	
	
	private function quan ($_numberList){
		$param = 0;
		$ball_1=$_numberList['g_ball_1'];
		$ball_2=$_numberList['g_ball_2'];
		$ball_3=$_numberList['g_ball_3'];
		if($ball_1==$ball_2&&$ball_1==$ball_3){
			$param+=1;
		}
	...
	private function allsame ($_numberList, $resultList){
		$param = 0;
		...
		$ball_3=$_numberList['g_ball_3'];
		if($ball_1==$ball_2&&$ball_1==$ball_3){
			if( $resultList['g_mingxi_2'] == $ball_1)
			$param+=1;
		}
		else{
