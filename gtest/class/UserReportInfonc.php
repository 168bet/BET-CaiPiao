...
class UserReportInfonc 
{
	...
	private function UserInfo ($number)
	{
		...
		if ($result && Copyright)
		{
			...
			switch ($this->cid)
			{
				...
				case 9: $typeString = '總和、家禽野兽'; break;
	...
	private function GroupCount ($RepList, $typeString, $number=null)
	{
		...
		for ($i=0; $i<count($RepList); $i++)
		{
			...
			if ($RepList[$i]['g_mingxi_1'] == '第一球' && Copyright){
				...
				$arr['count_d'][7] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '總和、家禽野兽' ||$RepList[$i]['g_mingxi_1'] == '總和' ||$RepList[$i]['g_mingxi_1'] == '家禽野兽' && Copyright){
			...
			} else if ($RepList[$i]['g_mingxi_1'] == '蔬菜单选' && Copyright){
				$arr['count_c'][13] += $RepList[$i]['g_jiner'];
				$arr['count_d'][12] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '动物单选' && Copyright){
				$arr['count_c'][14] += $RepList[$i]['g_jiner'];
				$arr['count_d'][13] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '幸运二' && Copyright){
				$arr['count_c'][15] += $RepList[$i]['g_jiner'];
				$arr['count_d'][14] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '连连中' && Copyright){
				$arr['count_c'][16] += $RepList[$i]['g_jiner'];
				$arr['count_d'][15] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '背靠背' && Copyright){
				$arr['count_c'][17] += $RepList[$i]['g_jiner'];
				$arr['count_d'][16] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '幸运三' && Copyright){
				$arr['count_c'][18] += $RepList[$i]['g_jiner'];
				$arr['count_d'][17] ++;
			} else if ($RepList[$i]['g_mingxi_1'] == '幸运四' && Copyright){
			...
			} else if ($RepList[$i]['g_mingxi_1'] == $typeString && ($RepList[$i]['g_mingxi_2'] =='梅' || $RepList[$i]['g_mingxi_2'] == '兰' || $RepList[$i]['g_mingxi_2'] == '菊' || $RepList[$i]['g_mingxi_2'] == '竹')) {
				//梅兰菊竹組總投注額
			...
			} else if ($RepList[$i]['g_mingxi_1'] == $typeString ||$RepList[$i]['g_mingxi_1'] == "家禽野兽" ||$RepList[$i]['g_mingxi_1'] == "總和"&& ($RepList[$i]['g_mingxi_2'] =='家禽' || $RepList[$i]['g_mingxi_2'] == '野兽')) {
				//家禽野兽總投注額
			
				$countList[18] += $RepList[$i]['g_tueishui'];	
			}  else if ($typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "蔬菜单选") {
				$countList[2] += $RepList[$i]['g_tueishui'];
			}  else if ( $typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "幸运二") {
				$countList[3] += $RepList[$i]['g_tueishui'];
			}  else if ( $typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "连连中") {
				$countList[4] += $RepList[$i]['g_tueishui'];
			}  else if ( $typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "選三前組") {
				$countList[5] += $RepList[$i]['g_tueishui'];
			}  else if ( $typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "幸运四") {
				$countList[6] += $RepList[$i]['g_tueishui'];
			}  else if ( $typeString =='連碼' && $RepList[$i]['g_mingxi_1'] == "幸运五") {
				$countList[7] += $RepList[$i]['g_tueishui'];
			}
			
			if($this->cid==9){
			if ($RepList[$i]['g_mingxi_1'] == $typeString  || $RepList[$i]['g_mingxi_1'] == "家禽野兽"  || $RepList[$i]['g_mingxi_1'] == "總和" && Copyright) 
			{				
				switch ($RepList[$i]['g_mingxi_2']) {
					...
					case '梅' : $sArr['29'] += $RepList[$i]['g_jiner']; break;
					case '兰' : $sArr['30'] += $RepList[$i]['g_jiner']; break;
					case '菊' : $sArr['31'] += $RepList[$i]['g_jiner']; break;
					case '竹' : $sArr['32'] += $RepList[$i]['g_jiner']; break;
					...
					case '家禽' : $sArr['6'] += $RepList[$i]['g_jiner']; break;
					case '總和尾小' : $sArr['7'] += $RepList[$i]['g_jiner']; break;
					case '野兽' : $sArr['8'] += $RepList[$i]['g_jiner']; break;
				}
			}
			else 
			{
				switch ($RepList[$i]['g_mingxi_1']) {
					case '蔬菜单选' : $sArr['101'] += $RepList[$i]['g_jiner']; break;
					case '幸运二' : $sArr['102'] += $RepList[$i]['g_jiner']; break;
					case '连连中' : $sArr['103'] += $RepList[$i]['g_jiner']; break;
					case '選三前組' : $sArr['104'] += $RepList[$i]['g_jiner']; break;
					case '幸运四' : $sArr['105'] += $RepList[$i]['g_jiner']; break;
					case '幸运五' : $sArr['106'] += $RepList[$i]['g_jiner']; break;
				}
			}
			}else{
	...
	private function GetUserCrystals($UserList, $sArr, $arr, $typeString)
	{
		...
		for ($i=0; $i<count($UserList); $i++){
			...
			if ($UserList[$i]['g_mingxi_1'] == '第一球'){
				...
				//$arr['count_d'][7] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '總和、家禽野兽' || $UserList[$i]['g_mingxi_1'] == '家禽野兽' ||$UserList[$i]['g_mingxi_1'] == '總和'){
			...
			} else if ($UserList[$i]['g_mingxi_1'] == '蔬菜单选' && Copyright){
				$arr['count_c'][13] = $arr['count_c'][13]-$n*$h;
				//$arr['count_d'][12] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '动物单选'){
				$arr['count_c'][14] = $arr['count_c'][14]-$n*$h;
				//$arr['count_d'][13] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '幸运二' && Copyright){
				$arr['count_c'][15] = $arr['count_c'][15]-$n*$h;
				//$arr['count_d'][14] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '连连中'){
				$arr['count_c'][16] = $arr['count_c'][16]-$n*$h;
				//$arr['count_d'][15] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '背靠背'){
				$arr['count_c'][17] = $arr['count_c'][17]-$n*$h;
				//$arr['count_d'][16] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '幸运三'){
				$arr['count_c'][18] = $arr['count_c'][18]-$n*$h;
				//$arr['count_d'][17] ++;
			} else if ($UserList[$i]['g_mingxi_1'] == '幸运四'){
			...
			if($this->cid==9){
			if ($UserList[$i]['g_mingxi_1'] == $typeString || $UserList[$i]['g_mingxi_1']=="家禽野兽" || $UserList[$i]['g_mingxi_1']=="總和、家禽野兽" || $UserList[$i]['g_mingxi_1']=="總和" && Copyright) 
			{
				switch ($UserList[$i]['g_mingxi_2']) {
					...
					case '梅' : $arr['list']['29']=$arr['list']['29'] - $n; break;
					case '兰' : $arr['list']['30']=$arr['list']['30'] - $n; break;
					case '菊' : $arr['list']['31']=$arr['list']['31'] - $n; break;
					case '竹' : $arr['list']['32']=$arr['list']['32'] - $n; break;
					...
					case '家禽' : $arr['list']['6']=$arr['list']['6'] - $n; break;
					case '總和尾小' : $arr['list']['7']=$arr['list']['7'] - $n; break;
					case '野兽' : $arr['list']['8']=$arr['list']['8'] - $n; break;
				}
			}
			else 
			{
				switch ($UserList[$i]['g_mingxi_1']) {
					case '蔬菜单选' : $arr['list']['101']=$arr['list']['101'] - $n*$h; break;
					case '幸运二' : $arr['list']['102'] =$arr['list']['102']- $n*$h; break;
					case '连连中' : $arr['list']['103']=$arr['list']['103'] - $n*$h; break;
					case '選三前組' : $arr['list']['104']=$arr['list']['104'] - $n*$h; break;
					case '幸运四' : $arr['list']['105']=$arr['list']['105'] - $n*$h; break;
					case '幸运五' : $arr['list']['106']=$arr['list']['106'] - $n*$h; break;
				}
			}
	...
	private function GroupCounts ($RepList, $typeString, $list)
	{
		...
		{
		if($this->cid==9){
		if ($RepList[$i]['g_mingxi_1'] == $typeString ||$RepList[$i]['g_mingxi_1'] == "總和、家禽野兽"||$RepList[$i]['g_mingxi_1'] == "家禽野兽" ||$RepList[$i]['g_mingxi_1'] == "總和" && Copyright)
			{
...
/**
	...
	{
		$result = $this->db->query("SELECT * FROM g_kaipan5 WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1  ", 1);
		if ($result)
		{
			switch ($this->cid)
			{
	...
	public function PostCrystls($List, $param='a')
	{
		...
		switch ($this->User[0]['g_login_id'])
		{
			case 56 :
				...
				{
					$floorMoneync = floorMoneync2 ($this->User, $p, $List['s_num'][$i], $RankUser, $s);
					...
					$arr[$i]['g_tueishui'] = $floorMoneync;
					$arr[$i]['g_tueishui_1'] = 0;
					$arr[$i]['g_tueishui_2'] = 0;
					$arr[$i]['g_tueishui_3'] = 0;
					$arr[$i]['g_tueishui_4'] = $floorMoneync;
			...
			case 22 :
				...
				for ($i=0; $i<count($List['s_num']); $i++)
				{
					$floorMoneync = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser, $s);
					$arr[$i]['g_t_id'] = $List['s_id'];
					$arr[$i]['g_s_nid'] = $this->User[0]['g_nid'];
					$arr[$i]['g_nid'] = '股東走飛';
					$arr[$i]['g_mumber_type'] = 5;
					...
					$arr[$i]['g_tueishui_1'] = 0;
					$arr[$i]['g_tueishui_2'] = 0;
					$arr[$i]['g_tueishui_3'] = $floorMoneync;
					$arr[$i]['g_tueishui_4'] = $floorMoneync;
					$arr[$i]['g_distribution'] = 0;
					$arr[$i]['g_distribution_1'] = 0;
					$arr[$i]['g_distribution_2'] = 0;
			...
			case 78 :
				...
				for ($i=0; $i<count($List['s_num']); $i++)
				{
					$floorMoneync1 = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser1, $s);
					$floorMoneync2 = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser2, $s);
					$arr[$i]['g_t_id'] = $List['s_id'];
					$arr[$i]['g_s_nid'] = $this->User[0]['g_nid'];
					$arr[$i]['g_nid'] = '總代理走飛';
					$arr[$i]['g_mumber_type'] = 5;
					...
					$arr[$i]['g_odds'] = $List['s_odds'];
					$arr[$i]['g_jiner'] = $List['s_money'][$i];
					$arr[$i]['g_tueishui'] = $floorMoneync1;
					$arr[$i]['g_tueishui_1'] = 0;
					$arr[$i]['g_tueishui_2'] = $floorMoneync1;
					$arr[$i]['g_tueishui_3'] = $floorMoneync2;
					$arr[$i]['g_tueishui_4'] = $floorMoneync2;
					$arr[$i]['g_distribution'] = 0;
					$arr[$i]['g_distribution_1'] = 0;
			...
			case 48 :
				...
				for ($i=0; $i<count($List['s_num']); $i++)
				{
					$floorMoneync1 = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser1, $s);
					$floorMoneync2 = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser2, $s);
					$floorMoneync3 = floorMoneync ($this->User, $p, $List['s_num'][$i], $RankUser3, $s);
					$arr[$i]['g_t_id'] = $List['s_id'];
					$arr[$i]['g_s_nid'] = $this->User[0]['g_nid'];
					$arr[$i]['g_nid'] = '代理走飛';
					$arr[$i]['g_mumber_type'] = 5;
					...
					$arr[$i]['g_jiner'] = $List['s_money'][$i];
					$arr[$i]['g_tueishui'] = $floorMoneync1;
					$arr[$i]['g_tueishui_1'] = $floorMoneync1;
					$arr[$i]['g_tueishui_2'] = $floorMoneync2;
					$arr[$i]['g_tueishui_3'] = $floorMoneync3;
					$arr[$i]['g_tueishui_4'] = $floorMoneync3;
					$arr[$i]['g_distribution'] = 0;
