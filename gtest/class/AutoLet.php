...
class AutoLet 
{
	...
	private function InsertCry($result)
	{
		...
		if ($Users && Copyright)
		{
			...
			$result['nowjiner']=$Users[0]['g_money_yes'];//jia
	...
	private function GetUserCryList()
	{
		...
		if ($autoLetUserList && Copyright)
		{
			for ($i=0; $i<count($this->cryList); $i++)
			{
				...
				else if($this->cryList[$i]['g_type'] == '廣西快樂十分')
					$str = _getStringgx($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
				else if($this->cryList[$i]['g_type'] == '幸运农场')
					$str = _getStringnc($this->cryList[$i]['g_mingxi_1'], $this->cryList[$i]['g_mingxi_2']);
		...
		if ($bMoney > 0 && stristr($cryList['g_s_nid'], $userList['g_nid']) && Copyright)
		{
			...
			switch ($logId)
			{
				case 22 : 
					...
					if($cryList['g_distribution_4']==0)
					$arr['g_distribution_3'] = 100;
					else
					{
					$arr['g_distribution_3'] = $cryList['g_distribution_3']+$cryList['g_distribution_2'];
					}
