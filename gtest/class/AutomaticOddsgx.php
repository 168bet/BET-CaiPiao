...
class AutomaticOddsgx 
{
	...
	{			
		...
		if ($result['Num'] && Copyright)
		{
			foreach ($result['Num'] as $key=>$value)
			{
				...
				$sql = "UPDATE g_odds3 SET `{$key}`={$key}-{$value} WHERE g_type <> 'Ball_9' AND g_type <> 'Ball_10'";
				$this->db->query($sql, 2);
			}
		}
	...
	{
		$ResultNumber = history_resultgx(0);
		
		//得到雙面無出期數
		global $BallString, $BallString_a;
		$NumberStrArr = sum_ball_count_1_gx ($BallString, $BallString_a, $ResultNumber, 1);
		...
		//得到1-21無出期數
		$NumberArr = sum_ball_count_gx ($ResultNumber, 1);
		$NumberArr = $NumberArr['row_2'];
		
		//取出大於$Continuous連續不出期數的1-21號碼
	...
	private function NumberFormat($num, $param)
	{
		...
		switch ($num)
		{
			...
			case "第{$p}球-神" : $str[0] = "Ball_{$p}"; $str[1] = 'h30'; break;
			case "第{$p}球-奇" : $str[0] = "Ball_{$p}"; $str[1] = 'h29'; break;
			case "第{$p}球-快" :$str[0] = "Ball_{$p}"; $str[1] = 'h31'; break;
			case "第{$p}球-乐" :$str[0] = "Ball_{$p}"; $str[1] = 'h32'; break;
			case "第{$p}球-红" :$str[0] = "Ball_{$p}"; $str[1] = 'h33'; break;
			case "第{$p}球-蓝" :$str[0] = "Ball_{$p}"; $str[1] = 'h34'; break;
			case "第{$p}球-绿" :$str[0] = "Ball_{$p}"; $str[1] = 'h35'; break;
