		...
		if ($numberList&& Copyright)
		{
			...
			for ($i=0; $i<count($resultList); $i++)
			{
				if (isString($resultList[$i]['g_mingxi_2']) && $resultList[$i]['g_result'] <=20&& Copyright)
				{
					...
					if($resultList[$i]['g_mingxi_1']=='正碼'){
						$rl = explode(',', $resultList[$i]['g_result']);
						for($p=0;$p<count($rl);$p++){
							$resultList[$i]['g_result'] ='輸';
							if($rl[$p]==intval($resultList[$i]['g_mingxi_2'])){
								$resultList[$i]['g_result'] ='贏';
								break;
							}
						}
					...
					$resultList[$i]['g_result'] = $resultList[$i]['g_result'] == intval($resultList[$i]['g_mingxi_2']) ? '贏' : '輸';
					}
				}
				else 
				{
	...
	private function ResultCorrespond ($numberList, $resultList, $param=0)
	{
		if ($param == 0&& Copyright)
		{
			for ($i=0; $i<count($resultList); $i++)
			{
				...
				{
					case '第一球' : $resultList[$i]['g_result'] = $numberList[0]['g_ball_1'];
					...
					break;
					case '第二球' : $resultList[$i]['g_result'] = $numberList[0]['g_ball_2'];
					if ($resultList[$i]['g_mingxi_2'] == '龍' || $resultList[$i]['g_mingxi_2'] == '虎')
						{	
					...
					break;
					case '第三球' : $resultList[$i]['g_result'] = $numberList[0]['g_ball_3'];
					if ($resultList[$i]['g_mingxi_2'] == '龍' || $resultList[$i]['g_mingxi_2'] == '虎')
						{	
					...
					break;
					case '第四球' : $resultList[$i]['g_result'] = $numberList[0]['g_ball_4'];
					if ($resultList[$i]['g_mingxi_2'] == '龍' || $resultList[$i]['g_mingxi_2'] == '虎')
						{	
					...
					case '總和、龍虎' :
						...
						break;
					case '正碼' : $resultList[$i]['g_result'] = $numberList[0]['g_ball_1'].','.$numberList[0]['g_ball_2'].','.$numberList[0]['g_ball_3'].','.$numberList[0]['g_ball_4'].','.$numberList[0]['g_ball_5'].','.$numberList[0]['g_ball_6'].','.$numberList[0]['g_ball_7'].','.$numberList[0]['g_ball_8'];break;
...
//				//選三連直規則 任意三個號碼相連、並且對應下注號碼的前後。視為中獎
				...
				$index = array(0=>6, 1=>3, 2=>3, 3=>3);
				$param = $this->SumLMZ2($_numberList, $resultList, $index);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
			else if ($resultList['g_mingxi_1'] == '選三前組'&& Copyright)
			{
				...
				$index = array(0=>6, 1=>3, 2=>3, 3=>3);
				$param = $this->SumLM13($_numberList, $resultList, $index);
				$resultList[0] =$param;
				$resultList[1] = 'LM';
			}
