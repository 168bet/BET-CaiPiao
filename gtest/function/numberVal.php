...
function postForms ($list)
{
	...
	$sql .= "VALUES (
					...
					'{$list['g_pan']}',
					'{$list['nowjiner']}')";//20180823jia
					
	$insertId = $db->query($sql, 4);
	
	$sql2 = "INSERT INTO `g_zhudan_copy` ( `g_s_nid`, `g_mumber_type`, `g_nid`, `g_date`, `g_type`, `g_qishu`, `g_mingxi_1`, `g_mingxi_1_str`, `g_mingxi_2`, `g_mingxi_2_str`, `g_odds`, `g_jiner`, `g_tueishui`, `g_tueishui_1`, `g_tueishui_2`, `g_tueishui_3`, `g_tueishui_4`, `g_distribution`, `g_distribution_1`, `g_distribution_2`, `g_distribution_3`,`g_distribution_4`,`g_pan`) "; 
	$sql2 .= "VALUES (
					'{$list['g_s_nid']}', 
					'{$list['g_mumber_type']}', 
					'{$list['g_nid']}', 
...
function isNumber ($type=null, $ball=null, $number=null)
{
	...
	{
		if ($type == "第一球" || $type == "第二球" || $type == "第三球" || $type == "第四球" || $type == "第五球" || $type == "第六球" || $type == "第七球" || $type == "第八球" || $type == "總和、龍虎" || $type == "連碼"||$type == "總和、家禽野兽" )
			$bool = true;
	}
	return $bool;
}
...
function isNumberk3 ($type=null, $ball=null, $number=null)
{
	$bool = false;
	if ($number != null)
	{
		$db = new DB();
		$bool = _isNumberIsNotNullk3 ($db, $ball, $number);
	}
	else if ($type != null)
	{
		if ($type == "三軍" || $type == "圍骰" || $type == "點數" || $type == "長牌" || $type == "短牌" )
			$bool = true;
	}
	return $bool;
}
...
function _isNumberIsNotNullk3 ($db, $ball, $number)
{
	$bool = false;
	$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan7` WHERE `g_lock` = 2 ORDER BY g_qishu DESC LIMIT 1 ", 1);
	$endTimes = strtotime($result[0]['g_feng_date']);
	$thisTime =  strtotime(date('Y-m-d H:i:s'));
	if ($result[0]['g_qishu'] != $number || $thisTime > $endTimes)
	{
	...
	else 
	{
		switch ($ball)
		{
			case '大' : $bool = true; break;
			case '小' : $bool = true; break;
			case '全骰' : $bool = true; break;
			case '1,2' : $bool = true; break;
			case '1,3' : $bool = true; break;
			case '1,4' : $bool = true; break;
			case '1,5' : $bool = true; break;
			case '1,6' : $bool = true; break;
			case '2,3' : $bool = true; break;
			case '2,4' : $bool = true; break;
			case '2,5' : $bool = true; break;
			case '2,6' : $bool = true; break;
			case '3,4' : $bool = true; break;
			case '3,5' : $bool = true; break;
			case '3,6' : $bool = true; break;
			case '4,5' : $bool = true; break;
			case '4,6' : $bool = true; break;
			case '5,6' : $bool = true; break;
			
			case '4點' : $bool = true; break;
			case '5點' : $bool = true; break;
			case '6點' : $bool = true; break;
			case '7點' : $bool = true; break;
			case '8點' : $bool = true; break;
			case '9點' : $bool = true; break;
			case '10點' : $bool = true; break;
			case '11點' : $bool = true; break;
			case '12點' : $bool = true; break;
			case '13點' : $bool = true; break;
			case '14點' : $bool = true; break;
			case '15點' : $bool = true; break;
			case '16點' : $bool = true; break;
			case '17點' : $bool = true; break;
			...
			case '1+1' : $bool = true; break;
			case '2+2' : $bool = true; break;
			case '3+3' : $bool = true; break;
			case '4+4' : $bool = true; break;
			case '5+5' : $bool = true; break;
			case '6+6' : $bool = true; break;
			case '7+7' : $bool = true; break;
			case '8+8' : $bool = true; break;
		}
	}
	return $bool;
}
...
function isNumberjx($type, $number)
{
	if ($type == 'Ball_1' || $type == 'Ball_2' || $type == 'Ball_3' || $type == 'Ball_4' || $type == 'Ball_5' || $type == 'Ball_6' || $type == 'Ball_7' || $type == 'Ball_8' || $type == 'Ball_9')
	{
		$db = new DB();
		$result = $db->query("SELECT `g_qishu`, `g_feng_date` FROM `g_kaipan4` WHERE `g_lock` = 2 AND g_qishu = '{$number}' LIMIT 1 ", 1);
		if (!$result || strtotime(date('Y-m-d H:i:s')) > strtotime($result[0]['g_feng_date']))
		{
			exit(alert_href('抱歉！第 '.$number.' 期已經封盤', '../left.php'));
		}
...
function isBallTypejxsm($type, $ball, $p=false)
{
...
function isBallTypejxsz($type, $ball, $p=false)
{
	$arr = array();
	switch ($type)
	{
