...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}//else{$ids="自选";}
...
echo "<script>alert('补仓成功!');window.location.href='index.php?action=pz_zx&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?=$kithe?>&class2="+arrTmp[14]+"&lx=0&id=12&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe?>&class2="+arrTmp[14]+"&id=12&class3="+arrTmp[12];
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbtitle">
    <td width="95%"><? require_once '3top.php';?></td>
    <td width="5%">&nbsp;</td>
  </tr>
  <tr >
    <td height="5"></td>
...
<table   border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%">
  <tr>
    <td width="72%" height="25" align="center" bordercolor="cccccc" bgcolor="#FDF4CA"><table width="100%" border="0" cellspacing="3" cellpadding="2">
      <tr>
        <td width="150" nowrap><?=$ids?>不中统计[<?=$kithe?>期]</td>
...
while($image = mysql_fetch_array($result)){
        ...
        <form name=form55 action="index.php?action=pz_zx&zsave=zsave&kithe=<?=$kithe?>&ids=<?=$ids?>" method=post>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_zx&save=save&ids=<?=$ids?>&kithe=<?=$kithe?>"    method="post" name="form1" > <?
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_zx&class1=自选&class2=<?=$ids?>&kithe=<?=$kithe?>')</script>
