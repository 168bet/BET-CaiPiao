...
$result=mysql_query("select * from ka_guan where id=".$_POST['guanid']."  order by id"); 
...
$exe=mysql_query("INSERT INTO ka_quota Set yg='".$yg[$I]."',ygb='".$ygb[$I]."',ygc='".$ygc[$I]."',ygd='".$ygd[$I]."',xx='".$xx[$I]."',xxx='".$xxx[$I]."',username='".$_POST['kauser']."',userid='".$SoftID."',lx=0,flag=0,guanid='".$_POST['guanid']."',zongid=0,danid=0,memid=0,ds='".$ds[$I]."',style='".$style[$I]."' ");
	...
	echo "<script>alert('总代添加成功!');window.location.href='index.php?action=zong_add';</script>"; 
exit;
...
$result=mysql_query("select id,kauser,sf,cs,tmb,rs  from ka_guan where  id=".$_GET['id']." and lx=1"); 
$row=mysql_fetch_array($result);
if ($row!=""){
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbtitle">
    <td width="29%"><span class="STYLE3">添加总代理用户</span></td>
    <td width="34%">&nbsp;</td>
    <td width="37%">&nbsp;</td>
  </tr>
  <tr >
...
<table width="99%"  border="1" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8">
 <form name=testFrm onSubmit="return SubChk()" method="post" action="index.php?action=zong_add&act=添加&id=<?=$_GET['id']?>"> <tr>
    <td height="30" align="right" bordercolor="#CCCCCC" bgcolor="#FDF4CA">上级用户：</td>
    <td height="30" colspan="3" bordercolor="#CCCCCC"><input name="guanid" type="hidden" value="<?=$guanid?>" />
        <select class="zaselect_ste" name="temppid" onchange="var jmpURL=this.options[this.selectedIndex].value ; if(jmpURL!='') {window.location=jmpURL;} else {this.selectedIndex=0 ;}">
          <option value="" ></option>
          <?
		$result = mysql_query("select id,kauser,sf,cs  from ka_guan  where lx=1");   
...
$result1 = mysql_query("Select SUM(cs) As sum_m  From ka_guan Where lx=2 and   guanid=".$image['id']." order by id desc");
  ...
  <tr>
    ...
    <td bordercolor="#CCCCCC"><input type="hidden" name="tv5" value="是">
        <img src="images/icon_21x21_selectboxon.gif" name="tv5_b" align="absmiddle" class="cursor" id="tv5_b" onclick="javascript:ra_select('tv5')" />(充许走飞/禁止走飞)<span class="STYLE2">*</span></td>
  ...
  <tr>
    <td height="30" colspan="4" bordercolor="#CCCCCC"><table width="100%" border="1" cellpadding="3" cellspacing="1" bordercolor="f1f1f1">
      ...
      <? 
					   ...
					   if ($guanid!=""){
					      $result = mysql_query("select * from  ka_quota where userid=".$guanid." and  lx=0  and flag=0 order by id"); 
					   }else{
...
?>
  ...
  <tr>
    ...
    <td colspan="3" bordercolor="#CCCCCC"><br />
      ...
      <input  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" type="submit" name="Submit" value="保存总代理" />
        <br />
        <table width="100" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="10"></td>
