...
<table border="1" align="center" cellspacing="1" height="87" cellpadding="5" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%">
    ...
    <tr>
      ...
      <TD width="30" align="center" bordercolor="cccccc" style="display:<? if($_SESSION['stadmin666']==1){echo ""; }else{ echo "none";}?>;" bgcolor="#FDF4CA" class="m_title_reall">操作</TD>
    </tr>
    <?php
$z_re=0;
		$z_sum=0;
...
?>
    <?php
$result = mysql_query("select id,kithe,username,adddate,sum_m,rate,user_ds,class1,class2,class3,abcd  from   ka_tan ".$vvv."  order by sum_m desc limit $start_limit, $perpage");   
...
?>
    <form action="index.php?action=x5&save=save&id=<?=$rs['id']?>&kithe=<?=$kithe?>&username=<?=$rs['username']?>" method="post" name="form" id="form"> <tr >
      ...
      <td align="center" nowrap="nowrap" style="display:<? if($_SESSION['stadmin666']==1){echo ""; }else{ echo "none";}?>;" bordercolor="cccccc"><input  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" type="submit" name="Submit" value="修改" />
