...
$result1 = mysql_query("Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*dagu_zc/10) as dagu_zc,sum(sum_m*guan_zc/10) as guan_zc,sum(sum_m*zong_zc/10) as zong_zc,sum(sum_m*dai_zc/10) as dai_zc,sum(sum_m-sum_m*user_ds/100) as sum_st from ka_tan   where Kithe='".$kithe."' and guan='".$rs['guan']."'");
...
?>
  <tr >
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><button onClick="javascript:location.href='index.php?action=x2&kithe=<?=$kithe?>&username=<?=$rs['guan']?>'"  class="headtd4" style="width:80;height:22" ;><font color=ff6600>
      <?=$Rs5['sum_m']?>
    </font></button></td>
