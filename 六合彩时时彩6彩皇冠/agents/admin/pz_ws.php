...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="尾数";}
...
$exe=mysql_query("update adad set ws='".$_POST['tm']."',ws1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('补仓成功!');window.location.href='index.php?action=pz_ws&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?=$kithe?>&class2=<?=$ids?>&lx=0&id=7&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe?>&class2=<?=$ids?>&id=7&class3="+arrTmp[12];
...
function UpdateRate(commandName,inputID,cellID,strPara)
{
	...
	switch(commandName)
	{
		case "MODIFYRATE":	//更新赔率
		 ...
		 makeRequest('index.php?action=server_ws&class1=尾数&class2=<?=$ids?>&kithe=<?=$kithe?>')
...
while($image = mysql_fetch_array($result)){
        ...
        <form name=form55 action="index.php?action=pz_ws&zsave=zsave&kithe=<?=$kithe?>&ids=<?=$ids?>" method=post>
          <td width="85" align="right" nowrap>最大亏损：</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?=$ws?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>退水：</td>
          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$ws1?>" size="5"></td>
        ...
        <td width="697"><div align="right">
          ...
          <button onClick="javascript:location.href='index.php?action=pz_ts&ids=头数&kithe=<?=$kithe?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm1 STYLE='color:000000;'>头数</span></button>
          <button onClick="javascript:location.href='index.php?action=pz_ws&ids=尾数&kithe=<?=$kithe?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:ff0000;'>尾数</span></button>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_ws&save=save&ids=<?=$ids?>&kithe=<?=$kithe?>"    method="post" name="form1" > <?
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_ws&class1=尾数&class2=<?=$ids?>&kithe=<?=$kithe?>')</script>
