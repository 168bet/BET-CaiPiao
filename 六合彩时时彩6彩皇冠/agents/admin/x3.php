...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbtitle">
    <td width="15%"><strong><font color="#FFFFFF">[<?=$username?>]总代理注单查询[<?=$kithe?>期]</font></strong></td>
    <td width="48%">&nbsp;</td>
...
<table id="tb"  border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%">
  ...
  $z_st=0;
		...
		$z_dai=0;
		 ...
		 $result = mysql_query("select distinct(dai)   from   ka_tan where Kithe='".$kithe."' and zong='$username' order by dai desc");   
$ii=0;
while($rs = mysql_fetch_array($result)){



$result1 = mysql_query("Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*dagu_zc/10) as dagu_zc,sum(sum_m*guan_zc/10) as guan_zc,sum(sum_m*zong_zc/10) as zong_zc,sum(sum_m*dai_zc/10) as dai_zc,sum(sum_m-sum_m*user_ds/100) as sum_st from ka_tan   where Kithe='".$kithe."' and dai='".$rs['dai']."'");
...
$z_zong+=$Rs5['zong_zc'];
$z_dai+=$Rs5['dai_zc'];
$result2=mysql_query("select * from ka_guan where  kauser='".$rs['dai']."' order by id"); 
...
?>
  <tr >
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><button onClick="javascript:location.href='index.php?action=x4&kithe=<?=$kithe?>&username=<?=$rs['dai']?>'"  class="headtd4" style="width:80;height:22" ;><font color=ff6600>
      <?=$Rs5['sum_m']?>
    </font></button></td>
