...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="半波";}
...
$exe=mysql_query("update adad set bb='".$_POST['tm']."',bb1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('补仓成功!');window.location.href='index.php?action=pz_bb&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
num4 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='半波'; return true; onMouseOut=this.className='headtd4';window.status='半波';return true;  onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>"; //字段num2的值
...
num11 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='半波'; return true; onMouseOut=this.className='headtd4';window.status='半波';return true;  onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>"; //字段num2的值
}else{
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_bb&save=save&ids=<?=$ids?>&kithe=<?=$kithe?>"    method="post" name="form1" > <?
include("d1.php");
?>
</form>
</div>
