...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="正肖";}
...
$exe=mysql_query("update adad set zhx='".$_POST['tm']."',zhx1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('补仓成功!');window.location.href='index.php?action=pz_zhx&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?=$kithe?>&class2=<?=$ids?>&lx=0&id=6111&class3="+arrTmp[12];
num4 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='正肖'; return true; onMouseOut=this.className='headtd4';window.status='正肖';return true;  onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>"; //字段num2的值
...
var zmzm2="index.php?action=look&lx=1&kithe=<?=$kithe?>&class2=<?=$ids?>&id=6111&class3="+arrTmp[12];
num11 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='正肖'; return true; onMouseOut=this.className='headtd4';window.status='正肖';return true;  onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>"; //字段num2的值
}else{
...
function UpdateRate(commandName,inputID,cellID,strPara)
{
	...
	switch(commandName)
	{
		...
		makeRequest('index.php?action=server_bb&class1=正肖&class2=<?=$ids?>&kithe=<?=$kithe?>')
...
while($image = mysql_fetch_array($result)){
        ...
        <form name=form55 action="index.php?action=pz_zhx&zsave=zsave&kithe=<?=$kithe?>&ids=<?=$ids?>" method=post>
          <td width="85" align="right" nowrap>最大亏损：</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?=$zhx?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>退水：</td>
          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?=$zhx1?>" size="5"></td>
        ...
        <td width="697"><div align="right">
          ...
          <button onClick="javascript:location.href='index.php?action=pz_zhx&ids=正肖&kithe=<?=$kithe?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:ff0000;'>正肖</span></button>
          <button onClick="javascript:location.href='index.php?action=pz_qsb&ids=七色波&kithe=<?=$kithe?>';"  class="but_c1" onMouseOut="this.className='but_c1'" onMouseOver="this.className='but_c1M'" style="height:22" ;><img src="images/add.gif" width="16" height="16" align="absmiddle"><SPAN id=rtm2 STYLE='color:000000;'>七色波</span></button>
        </div></td>
      </tr>
    </table></td>
  </tr>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_zhx&save=save&ids=<?=$ids?>&kithe=<?=$kithe?>"    method="post" name="form1" > <?
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_zhx&class1=正肖&class2=<?=$ids?>&kithe=<?=$kithe?>')</script>
