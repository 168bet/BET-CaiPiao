...
if ($_GET['act']=="修改") {
    ...
    $exe=mysql_query("update ka_bl  set adddate='". $text."',rate=".$num." where class1='头数' and class2='".$class2."' and  class3='".$class3."'");
	$exe=mysql_query("update ka_bl  set adddate='". $text."',rate=".$num." where class1='尾数' and class2='".$class2."' and  class3='".$class3."'");
    ...
    echo "<script>alert('修改成功!');window.location.href='index.php?action=rake_ts&ids=".$ids."';</script>"; 
    exit;
	
}//结速

$sql_ts="Select * from (Select rate,class3,class2,locked,0 as bl from ka_bl where class1='头数'   Order By ID) as a";
$sql_ws="Select * from (Select rate,class3,class2,locked,1 as bl from ka_bl where class1='尾数'   Order By ID) as b";
		...
		case "MODIFYRATE":	//更新赔率
			{
				...
				if (strResult!="")
				{
					makeRequest('index.php?action=server&class1=头尾数&class2=<?=$ids?>')
       ...
       <table width="100%" border="0" cellspacing="0" cellpadding="5">
         ...
         <tr >
          ...
          <table border="1" align="center" cellspacing="1" cellpadding="2" bordercolor="f1f1f1" bordercolordark="#FFFFFF" width="99%">
           <form name="form1" method="post" action="index.php?action=rake_ts&act=修改&ids=<?=$ids?>"> <TBODY>
              <TR align=center>
...
{?> 
                ...
                <TD align="center" bordercolor="cccccc"><table border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    ...
                    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','lm<?=$I+1?>','bl<?=$I?>','class1=<? if($ShowTable[$I][4]==0)echo "头数";else echo "尾数";?>&ids=<?=$ShowTable[$I][2]?>&sqq=sqq&lxlx=1&qtqt=0.01&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_01.gif"   width="19" height="17" border="0"></a></td>
                        </tr>
                        <tr>
                          <td><a style="cursor:hand" onClick="UpdateRate('MODIFYRATE','lm<?=$I+1?>','bl<?=$I?>','class1=<? if($ShowTable[$I][4]==0)echo "头数";else echo "尾数";?>&ids=<?=$ShowTable[$I][2]?>&sqq=sqq&lxlx=0&qtqt=0.01&class3=<?=$ShowTable[$I][1]?>');"><img src="images/bvbv_02.gif" width="19" height="17" border="0"  ></a></td>
                        </tr>
                    </table></td>
                    <td><input name="checkbox" type=checkbox id=lock<?=$I?> style="zoom:95%" title="关闭该项" onClick="UpdateRate('LOCK','lock<?=$I?>','','class1=<? if($ShowTable[$I][4]==0)echo "头数";else echo "尾数";?>&ids=<?=$ids?>&sqq=sqq&class3=<?=$ShowTable[$I][1]?>&lock='+this.checked);" value="TRUE"  <? if ($ShowTable[$I][3]==1){echo "checked";}?>></td>
                  </tr>
                </table></TD>
      ...
      <SCRIPT language=javascript>
 makeRequest('index.php?action=server&class1=头尾数&class2=<?=$ids?>')</script>
