...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="五行";}
...
$exe=mysql_query("update adad set ys='".$_POST['tm']."',ys1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('补仓成功!');window.location.href='index.php?action=pz_wx&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_wx&save=save&ids=<?=$ids?>&kithe=<?=$kithe?>"    method="post" name="form1" > <?
include("d1.php");
?>
</form>
</div>
