...
if (empty($_POST['Num_'.$tt])) {       
 ...
 $class3=$_POST['class3_'.$tt];
  ...
  if ($class2=="正码1"){ $class22="正1特"; }
  if ($class2=="正码2"){ $class22="正2特"; }
   if ($class2=="正码3"){ $class22="正3特"; }
    if ($class2=="正码4"){ $class22="正4特"; }
	 if ($class2=="正码5"){ $class22="正5特"; }
	  if ($class2=="正码6"){ $class22="正6特"; }
	  
  $text=date("Y-m-d H:i:s");
$exe=mysql_query("update ka_bl  set adddate='". $text."',rate=".$num." where class1='过关' and class2='".$class2."' and  class3='".$class3."'");

$exe=mysql_query("update ka_bl  set adddate='". $text."',rate=".$num." where class1='正特' and class2='".$class22."'  and  class3='".$class3."'");
...
$result=mysql_query("Select rate,class3,class2,locked from ka_bl where class1='过关'   Order By ID");
$ShowTable = array();
$y=0;
...
{?>
      ...
      style="HEIGHT: 18px"  class="input1" maxlength="6" size="4" value="<?=$ShowTable[$I+35-1][0]?>" name="Num_<?=$I+35?>" /></td>
          ...
          <SCRIPT language=javascript>
 makeRequest('index.php?action=server&class1=过关&class2=<?=$ids?>')</script>
