...
$mysql= "update match_sports set type='FT' where type='FU' AND  M_Start<now()";
mysql_query($mysql);
...
$html_date=$curl->fetch_url("".$site."/app/member/result/result.php?game_type=FT&list_date=$list_date&uid=$sid&langx=zh-tw");
...
for ($i=1;$i<sizeof($data);$i++){
	...
	for ($j=0;$j<sizeof($abcde)-1;$j++){		
		...
		if (sizeof($score)==7){
			...
			if ($tg_inball=='賽事無pk/加時'){
				$mb_inball='-7';
				$tg_inball='-7';				
			}
			if ($tg_inball_hr=='賽事無pk/加時'){
				$mb_inball_hr='-7';
				$tg_inball_hr='-7';
			}
			if ($tg_inball==$Score8){
			...
			$sql="select MID,MB_Inball from match_sports where Type='FT' and MB_MID=".(int)$mb_mid." and M_Date='".$list_date."'";
			$result = mysql_query( $sql);
			$cou=mysql_num_rows($result);
			$row = mysql_fetch_array($result);
			if ($cou==0){
				$sql="select MID from match_sports where Type='FT' and TG_MID=".(int)$mb_mid." and M_Date='".$list_date."'";
				$result = mysql_query( $sql);
				$row = mysql_fetch_array($result);		
			}
			$mid=$row['MID'];
			if ($row['MB_Inball']==""){
			    $mysql="update match_sports set MB_Inball='$mb_inball',TG_Inball='$tg_inball',MB_Inball_HR='$mb_inball_hr',TG_Inball_HR='$tg_inball_hr' where Type='FT' and M_Date='".$list_date."' and MID=".(int)$mid;
			}else if ($row['MB_Inball']<0){
			    $mysql="update match_sports set MB_Inball='$mb_inball',TG_Inball='$tg_inball',MB_Inball_HR='$mb_inball_hr',TG_Inball_HR='$tg_inball_hr',Cancel=1 where Type='FT' and M_Date='".$list_date."' and MID=".(int)$mid;				
            }else{
				$m_sql="select MID,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from match_sports where Type='FT' and MID='".(int)$mid."' and M_Date='".$list_date."'";
				...
				if ($a!=$b){
				$check=1;
				$mysql="update match_sports set MB_Inball='".(int)$mb_inball."',TG_Inball='".(int)$tg_inball."',MB_Inball_HR='".(int)$mb_inball_hr."',TG_Inball_HR='".(int)$tg_inball_hr."',Checked='".$check."' where Type='FT' and M_Date='".$list_date."' and MID=".(int)$mid;
				}else{
				$mysql="update match_sports set MB_Inball='".(int)$mb_inball."',TG_Inball='".(int)$tg_inball."',MB_Inball_HR='".(int)$mb_inball_hr."',TG_Inball_HR='".(int)$tg_inball_hr."' where Type='FT' and M_Date='".$list_date."' and MID=".(int)$mid;
				}
			}	
