...
$mysql="select ID,MID,Active,M_Name,LineType,OpenType,BetTime,ShowType,Mtype,Gwin,TurnRate,BetType,M_Place,M_Rate,Middle,BetScore,A_Rate,B_Rate,C_Rate,D_Rate,A_Point,B_Point,C_Point,D_Point,Pay_Type,Checked from web_report_data where M_Date='$date' and LineType=8 and (Active=5 or Active=55) and cancel=0";
$result = mysql_query( $mysql);
...
$k++;	
	...
	$count=0;
	for($i=0;$i<$cou;$i++){
		$sql="select MB_Inball,TG_Inball from match_sports where Type='VB' and M_Date='$date' and MID=".$mid[$i];
...
<table width="220" height="190" border="0" cellpadding="0" cellspacing="0">
  <tr> 
	...
	<input type=button name=button value="排球过关刷新" onClick="window.location.reload()"></td>
  </tr>
</table>
</body>
</html>
