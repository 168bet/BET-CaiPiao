...
$mmysql = "select MID,MB_MID,TG_MID,MB_Team,TG_Team,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from `match_sports` where `Type`='VB' and `M_Date` ='$mDate' and MB_Inball!='' and Score=0 order by M_Start,MID";	
...
while ($mrow=mysql_fetch_array($mresult1)){
	...
	while ($row = mysql_fetch_array($result)){
		...
		if($mb_in_score<0 and $mb_in_score_v<0){
           ...
           $sql="update web_report_data set VGOLD='0',M_Result='0',D_Result='0',C_Result='0',B_Result='0',A_Result='0',T_Result='0',Cancel=1,Checked=1,Confirmed='$mb_in_score' where MID='$gid' and (active=5 or active=55) and LineType!=8";
		}else{
	       if ($row['Checked']==0){	
		       if ($row['Pay_Type']==1){
	...
	$mysql="update match_sports set Score=1 where Type='VB' and MID='$gid'";
	mysql_query($mysql) or die ("error!!");
}
?>
<html>
...
<table width="220" height="190" border="0" cellpadding="0" cellspacing="0">
  <tr> 
	...
	<input type=button name=button value="排球刷新" onClick="window.location.reload()"></td>  
  </tr>
</table>
</body>
</html>
