...
//echo ($time*60*60)."-".$date."-".$mDate;exit;
$mmysql = "select MID,MB_MID,TG_MID,MB_Team,TG_Team,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from `match_sports` where `Type`='FT' and `M_Date` ='$mDate' and MB_Inball!='' and Score=0 order by M_Start,MID";
$mresult1 = mysql_db_query($dbname,$mmysql);
		...
		if($mb_in_score<0 and $mb_in_score_v<0){
	       if ($row['Checked']==0){
		       if ($row['Pay_Type']==1){
				   ...
				   mysql_db_query($dbname,$mysql) or die ("error!");
		       }
	  	   }		
