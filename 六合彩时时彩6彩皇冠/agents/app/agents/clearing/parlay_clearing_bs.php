...
$mysql="select ID,MID,Active,M_Name,LineType,OpenType,BetTime,ShowType,Mtype,Gwin,TurnRate,BetType,M_Place,M_Rate,Middle,BetScore,A_Rate,B_Rate,C_Rate,D_Rate,A_Point,B_Point,C_Point,D_Point,Pay_Type,Checked from web_report_data where M_Date='$date' and LineType=8 and (Active=3 or Active=33) and cancel=0";
$result = mysql_query( $mysql);
...
$k++;	
	...
	$count=0;
	for($i=0;$i<$cou;$i++){
		$sql="select MB_Inball,TG_Inball from match_sports where Type='BS' and M_Date='$date' and MID=".$mid[$i];
		...
		echo $graded.'<br>';
        if ($graded<1 and $graded>0){
		      $winrate=$winrate*(1+$rate[$i]*$graded);
		}elseif($graded<=-0.05 and $graded>=-0.95){	
			  if ($count>9){
					$winrate=0;
				}else{
					$winrate=$winrate*(1+$graded);
				}
				$count=$count+1;
		}else if($graded==1){
...
<table width="220" height="190" border="0" cellpadding="0" cellspacing="0">
  <tr> 
	...
	<input type=button name=button value="棒球过关刷新" onClick="window.location.reload()"></td>
  </tr>
</table>
</body>
</html>
