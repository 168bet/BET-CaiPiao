...
$mysql = "select datasite_en as datasite_en as datasite,Uid_en,udp_ft_tw from web_system_data where ID=1";
...
$curl->set_referrer("".$site."/app/member/browse_FS/loadgame_R.php?rtype=fs&uid=$uid&langx=en-us&mtype=3");

$allcount=0;

//for($page_no=0;$page_no<10;$page_no++){

$html_data=$curl->fetch_url("".$site."/app/member/browse_FS/reloadgame_R.php?uid=$uid&langx=en-us&rtype=fs");
...
for($i=0;$i<$cou;$i++){
	...
	for($t=0;$t<$pp-1;$t++)
	{
		...
		if(!empty($datainfo)){
			$sql = "update match_crown set MB_Team_en='$MB_Team',M_League_en='$M_League',M_Item_en='$M_Item' where MID=$datainfo[0] and Gid='$tid'";
