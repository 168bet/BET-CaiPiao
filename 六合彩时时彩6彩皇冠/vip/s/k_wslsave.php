...
if ($n<2 or $n>8){echo "<script>alert('对不起，请选择二-八个尾数!');window.location.href='index.php?action=left';</script>"; 
exit;}
$zs=$n*($n-1)/2;
...
if ($n<3 or $n>8){echo "<script>alert('对不起，请选择三-八个尾数!'); window.location.href='index.php?action=left';</script>"; 
exit;}
$zs=$n*($n-1)*($n-2)/6;
...
if ($n<4 or $n>8){echo "<script>alert('对不起，请选择四-八个尾数!'); window.location.href='index.php?action=left';</script>"; 
exit;}
$zs=$n*($n-1)*($n-2)*($n-3)/24;
  ...
  <tr>
	...
	?>
          ...
          <tr>
            ...
            <td  class=t_td_text><span class="STYLE4">
              <?=$Current_Kithe_Num?>
              期</span></td>
          </tr>
