...
$XF=25;
function ka_kk1($i){   
   $result=mysql_query("select sum(sum_m) as sum_mm from ka_tan where kithe='".$Current_Kithe_Num."' and  username='".$_SESSION['kauser']."' and class1='半半波' and class2='".$ids."' and class3='".$i."' order by id desc"); 
...
show_student163_time(); 
	...
	?>
	<tr>
      <td width="41" height="25" align="center"  bgcolor="#FDF4CA"><span class="STYLE4"><font color=<? if ($ii<4){echo "ff0000";}elseif($ii<8){echo "077305";}elseif ($ii<12){echo "0000ff";}elseif ($ii<14){echo "ff0000";}elseif ($ii<16){echo "077305";}elseif ($ii<18){echo "0000ff";}?> ><?=$rs['class3']?></span></td>
      <td width="50" height="25" align="center" valign="middle" class="ball_ff"><b><span id=bl<?=$ii?>> 0 </span><span> </span></b></td>
      <td width="55" height="25" align="center" bgcolor="#FFFFFF"><input onKeyPress="return CheckKey();" 
                        onBlur="return CountGold(this,'blur','SP','<?=ka_kk1($rs['class3'])?>','1');" 
