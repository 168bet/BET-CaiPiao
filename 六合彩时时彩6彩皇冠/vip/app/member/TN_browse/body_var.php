...
case "re":
	...
	for($i=0;$i<$cou;$i++){
		...
		$opensql = "select * from `match_sports` where Type='TN' and  MID='$datainfo[0]'";
	...
	break;
case "pd":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,ShowTypeR from `match_sports` where Type='TN' and `M_Start` > now( ) AND `M_Date` ='$m_date'".$league." and PD_Show=1 and MB2TG0!=0 and $mb_team!='' order by m_start,mid";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=20;
	$page_count=$cou_num/$page_size;
	...
	break;
case "p":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_P_Win_Rate,TG_P_Win_Rate,MB_MID,TG_MID,ShowTypeP from `match_sports` where Type='TN' and `M_Start` > now( ) AND `M_Date` ='$m_date' and P_Show=1 and $mb_team!='' order by m_start,mid";	
	$result = mysql_db_query($dbname, $mysql);
	$cou=mysql_num_rows($result);
	echo "parent.retime=0;\n";
	echo "parent.gamount=$cou;\n";
	...
	break;
case "pr":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_MID,TG_MID,ShowTypeP,M_P_LetB,MB_P_LetB_Rate,TG_P_LetB_Rate,MB_P_Dime,TG_P_Dime,MB_P_Dime_Rate,TG_P_Dime_Rate from `match_sports` where Type='TN' and `m_start` > now( ) AND `M_Date` ='$m_date' ".$league." and PR_Show=1 and $mb_team!='' order by M_Start,MB_MID desc";	
	$result = mysql_db_query($dbname, $mysql);
	$cou=mysql_num_rows($result);
	echo "parent.retime=0;\n";
	echo "parent.gamount=$cou;\n";
