...
?>
<!--<script>if (top.game_alert.indexOf('VB')==-1){alert("<?=$mem_msg?>"); top.game_alert+='VB,'}</script>-->
<script> 
var show_ior = '100';
</script>
<html>
	...
	with(obj_table){
		...
		for(i=0; i<data_amount; i++){
			...
			with(nowTR){
                ...
                if(GameData[i][7] == 'H') //強隊是主隊
                	...
                	tmpStr += '<tr><td align=\"center\" width=\"68%\">&nbsp;</td>';
                tmpStr += '<td '+checkRatio(i,9)+'><a href=\"../VB_order/VB_order_re.php?gid='+GameData[i][0]+'&uid='+uid+'&odd_f_type='+odd_f_type+'&type=H&gnum='+GameData[i][3]+'&strong='+GameData[i][7]+'\" target=\"mem_order\" title=\"'+GameData[i][5]+'\">'+R_ior[0]+'</a></td>'+
                '</tr></table>';
                nowTD.innerHTML = tmpStr;
                //大小盤主隊
                nowTD = insertCell();
		...
		if(langx=="en-us"||langx=="th-tis"){
			title_str="Over";
                ...
                nowTD.innerHTML = '<font '+checkRatio_font(i,11)+'>'+GameData[i][11]+'</font>&nbsp;&nbsp;'+
                '&nbsp;&nbsp;<a href=\"../VB_order/VB_order_rou.php?gid='+GameData[i][0]+'&uid='+uid+'&odd_f_type='+odd_f_type+'&type=C&gnum='+GameData[i][4]+'\" target=\"mem_order\" title=\"'+title_str+'\"><font '+checkRatio_font(i,14)+'>'+OU_ior[1]+'</A></font>&nbsp;';
			}//主隊TR結束
			...
			with(nowTR){
				...
				nowTD.innerHTML = GameData[i][6];
		                ...
		                tmpStr += '<td '+checkRatio(i,10)+'><a href=\"../VB_order/VB_order_re.php?gid='+GameData[i][0]+'&uid='+uid+'&odd_f_type='+odd_f_type+'&type=C&gnum='+GameData[i][4]+'\" target=\"mem_order\"  title=\"'+GameData[i][6]+'\">'+R_ior[1]+'</a></td>'+
		                '</tr></table>';
		                nowTD.innerHTML = tmpStr;
		                //大小盤主隊
		                nowTD = insertCell();
				...
				nowTD.innerHTML = '<font '+checkRatio_font(i,12)+'>'+GameData[i][12]+'</font>&nbsp;&nbsp;'+
		                '&nbsp;&nbsp;<a href=\"../VB_order/VB_order_rou.php?gid='+GameData[i][0]+'&uid='+uid+'&odd_f_type='+odd_f_type+'&type=H&gnum='+GameData[i][3]+'\" target=\"mem_order\" title=\"'+title_str+'\"><font '+checkRatio_font(i,13)+'>'+OU_ior[0]+'</A></font>&nbsp;';
			}//客隊TR結束
			...
			with(nowTR){
				...
				if (top.casino == "SI2") {
					if (GameData[i][31] != "" && GameData[i][31] != "null" && GameData[i][31] != undefined) {	//判斷是否有轉播
						tmpStr+= VideoFun(GameData[i][31], GameData[i][32], GameData[i][33], "VB");
					}
				}
				tmpStr+= "</td></tr></table>";
				tmpStr+= "</td>";
