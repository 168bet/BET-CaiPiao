...
case "pd":
	...
	break;
case "hpd":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB1TG0H,MB2TG0H,MB2TG1H,MB3TG0H,MB3TG1H,MB3TG2H,MB4TG0H,MB4TG1H,MB4TG2H,MB4TG3H,MB0TG0H,MB1TG1H,MB2TG2H,MB3TG3H,MB4TG4H,UP5H,MB0TG1H,MB0TG2H,MB1TG2H,MB0TG3H,MB1TG3H,MB2TG3H,MB0TG4H,MB1TG4H,MB2TG4H,MB3TG4H,ShowTypeR,MB_MID,TG_MID from `match_sports` where `Type`='FU' and `M_Date` >'$m_date' and `HPD_Show`=1 and `MB2TG1H`!=0 ".$date." and `Open`=1 order by M_Start,MB_MID";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=18;
	$page_count=$cou_num/$page_size;
	...
	break;
case "t":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate,M_Flat_Rate,S_0_1,S_2_3,S_4_6,S_7UP,ShowTypeR,MB_MID,TG_MID from `match_sports` where `Type`='FU' and `M_Date` >'$m_date' and `T_Show`=1 ".$date." and `Open`=1 order by m_start,MB_MID";	
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=25;
	$page_count=$cou_num/$page_size;
	...
	while ($row=mysql_fetch_array($result)){
	    ...
	    $m_date=strtotime($row['M_Date']);
	    $dates=date("m-d",$m_date);	
		echo "parent.GameFT[$K]=new Array('$row[MID]','$dates<br>$row[M_Time]','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]','$row[TG_Team]','$row[ShowTypeR]','0','0','$row[S_0_1]','$row[S_2_3]','$row[S_4_6]','$row[S_7UP]','$MB_Win_Rate','$TG_Win_Rate','$M_Flat_Rate');\n";		
		$K=$K+1;
	}
	break;	
case "f":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MBMB,MBFT,MBTG,FTMB,FTFT,FTTG,TGMB,TGFT,TGTG,MB_MID,TG_MID,ShowTypeR from `match_sports` where `Type`='FU' and `M_Date` >'$m_date' and `F_Show`=1 ".$date." and `MBMB`>0 and `Open`=1 order by m_start,MB_MID";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=16;
	$page_count=$cou_num/$page_size;
