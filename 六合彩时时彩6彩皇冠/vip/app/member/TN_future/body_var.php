...
switch ($rtype){
case "r":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate,M_LetB,MB_LetB_Rate,TG_LetB_Rate,MB_Dime,TG_Dime,MB_Dime_Rate,TG_Dime_Rate,MB_MID,TG_MID,ShowTypeR,S_Single_Rate,S_Double_Rate,M_Type from `match_sports` where Type='TU' and `M_Date` >'$m_date' and S_Show=1 and $mb_team<>'' ".$date." order by m_start,mid";
	...
	break;
case "pd":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,ShowTypeR from `match_sports` where Type='TU' and `M_Date` >'$m_date' and PD_Show=1 and MB2TG1!=0 and $mb_team<>'' ".$date." order by m_start,mid";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=20;
	$page_count=$cou_num/$page_size;
	...
	break;
case "p":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_P_Win_Rate,TG_P_Win_Rate,MB_MID,TG_MID,ShowTypeP from `match_sports` where Type='TU' and `M_Date` >'$m_date' and P_Show=1 and $mb_team<>'' order by m_start,mid";	
	$result = mysql_db_query($dbname, $mysql);
	$cou=mysql_num_rows($result);
	...
	break;
case "pr":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_MID,TG_MID,ShowTypeP,M_P_LetB,MB_P_LetB_Rate,TG_P_LetB_Rate,MB_P_Dime,TG_P_Dime,MB_P_Dime_Rate,TG_P_Dime_Rate from `match_sports` where Type='TU' and `M_Date` >'$m_date' and PR_Show=1 and $mb_team<>'' order by m_start,mid";	
	$result = mysql_db_query($dbname, $mysql);
	$cou=mysql_num_rows($result);
	echo "parent.retime=0;\n";
	echo "parent.gamount=$cou;\n";
	while ($row=mysql_fetch_array($result)){
		...
		echo "parent.GameFT[$K]= Array('$row[MID]','$pdate','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]','$row[TG_Team]','$row[ShowTypeP]','$row[M_PR_LetB]','$MB_PR_LetB_Rate','$TG_PR_LetB_Rate','$row[MB_PR_Dime]','$row[TG_PR_Dime]','$MB_PR_Dime_Rate','$TG_PR_Dime_Rate');\n";
		$K=$K+1;	
	}
	break;
}
