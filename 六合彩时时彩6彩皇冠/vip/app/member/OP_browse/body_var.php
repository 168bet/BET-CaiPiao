...
case "hr":
	...
	while ($row=mysql_fetch_array($result)){
		...
		if ($row['M_Type']==1){
		echo "parent.GameFT[$K]=new Array('$row[MID]','$date<br>$row[M_Time]<br><font color=red>Running Ball</font>','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]<font color=gray> - [$Order_1st_Half]</font>','$row[TG_Team]<font color=gray> - [$Order_1st_Half]</font>','$row[ShowTypeHR]','$row[M_LetB_H]','$MB_LetB_Rate','$TG_LetB_Rate','$row[MB_Dime_H]','$row[TG_Dime_H]','$TG_Dime_Rate','$MB_Dime_Rate','$MB_Win_Rate_hr','$TG_Win_Rate_hr','$M_Flat_Rate_hr');\n";
		}else{
		echo "parent.GameFT[$K]=new Array('$row[MID]','$date<br>$row[M_Time]','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]<font color=gray> - [$Order_1st_Half]</font>','$row[TG_Team]<font color=gray> - [$Order_1st_Half]</font>','$row[ShowTypeHR]','$row[M_LetB_H]','$MB_LetB_Rate','$TG_LetB_Rate','$row[MB_Dime_H]','$row[TG_Dime_H]','$TG_Dime_Rate','$MB_Dime_Rate','$MB_Win_Rate_hr','$TG_Win_Rate_hr','$M_Flat_Rate_hr');\n";
		}
		$K=$K+1;	
...
case "re":
	...
	for($i=0;$i<$cou;$i++){
		...
		$opensql = "select * from `match_sports` where  MID='$datainfo[0]' and `Type`='OP'";
		//echo $opensql;
...
case "pd":
	...
	break;
case "hpd":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB1TG0H,MB2TG0H,MB2TG1H,MB3TG0H,MB3TG1H,MB3TG2H,MB4TG0H,MB4TG1H,MB4TG2H,MB4TG3H,MB0TG0H,MB1TG1H,MB2TG2H,MB3TG3H,MB4TG4H,UP5H,MB0TG1H,MB0TG2H,MB1TG2H,MB0TG3H,MB1TG3H,MB2TG3H,MB0TG4H,MB1TG4H,MB2TG4H,MB3TG4H,ShowTypeR from `match_sports` where Type='OP' and `M_Start` > now( ) AND `M_Date` ='$m_date' and HPD_Show=1 and MB2TG1!=0 and $mb_team<>''  order by M_Start,MID";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=60;
	$page_count=$cou_num/$page_size;
	...
	break;
case "t":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate,M_Flat_Rate,S_0_1,S_2_3,S_4_6,S_7UP,MB_MID,TG_MID,ShowTypeR from `match_sports` where Type='OP' and `M_Start` > now( ) AND `M_Date` ='$m_date' and T_Show=1 order by M_Start,MID";
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=60;
	$page_count=$cou_num/$page_size;
	...
	break;	
case "f":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MBMB,MBFT,MBTG,FTMB,FTFT,FTTG,TGMB,TGFT,TGTG,MB_MID,TG_MID,ShowTypeR from `match_sports` where Type='OP' and `M_Start` > now( ) AND `M_Date` ='$m_date' and F_Show=1 order by M_Start,MID";	
	$result = mysql_db_query($dbname, $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=60;
	$page_count=$cou_num/$page_size;
